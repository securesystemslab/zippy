...
public class PySetInJavaTest {
    ...
    public static void testPySetAsJavaSet() {
        ...
        check(!s.add(v));
        String[] asArray = (String[]) s.toArray(new String[0]);// The array type
                                                               // should be the
                                                               // same
        ...
        // Add a Random as it should be wrapped in a generic PyObject; go
        // through addAll to give it
        // a little exercise
        ...
        for (Object object : naiveArray) {
            if (object instanceof String) {
                ...
                check(object == rand, "Should be 'value' or rand, not "
                        + object);
            }
        }
        check(!s.remove(new Random()),
                "The Random in the set shouldn't match a new Random");
        check(s.remove(rand));
    ...
    public static void accessAndRemovePySetItems(Set<Object> items) {
        check(items instanceof PySet, "The set shouldn't be __tojava'd into "
                + items.getClass());
        check(items.size() == 3, "Should be 3 items, not " + items.size());
        check(items.contains("value"),
                "The set from Python should contain 'value'");
        check(!items.contains(new Random()),
                "The set contains a particular Random");
        Iterator<Object> it = items.iterator();
        while (it.hasNext()) {
            Object object = it.next();
            check(items.contains(object),
                    "The set should contain all items from its iterator");
            if (object instanceof String) {
                check(object.equals("value"),
                        "The string should be 'value', not '" + object + "'");
            } else {
