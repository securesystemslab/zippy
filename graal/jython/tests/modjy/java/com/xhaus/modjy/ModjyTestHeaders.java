...
public class ModjyTestHeaders extends ModjyTestBase {
    ...
    public void testInvalidStatusCode() throws Exception {
        ...
        assertTrue("Could not find exception 'BadArgument' in output: "
                + getOutput(), getOutput().indexOf("BadArgument") != -1);
    }

    public void testNonLatin1StatusString() throws Exception {
        // We'll let this one pass:
        // 1. The integer status code can't be anything but an ASCII-encoded
        // integer
    ...
    public void doBadHeadersListTest(String appName) throws Exception {
        for (int i = 1; i < 5; i++) {
            ...
            assertTrue("Could not find exception 'BadArgument' in output: "
                    + firstLine, firstLine.indexOf("BadArgument") != -1);
        }
    }

    public void testHeadersNotList() throws Exception {
        // Ensure that setting headers to anything other than a list raises an
        // error
    ...
    public void testHeadersListContainsWrongLengthTuples() throws Exception {
        // Ensure that setting headers to anything other than a list of 2-tuples
        // raises an error
    ...
    public void testHeadersListContainsNonLatin1Values() throws Exception {
        // Ensure that setting header values to non-latin1 strings raises an
        // error
    ...
    public void testHeadersListContainsAccentedLatin1Values() throws Exception {
        // Ensure that setting header values to 8-bit latin1 strings works
        // properly
        String headerName = "x-latin1-header";
        String headerValue = "\u00e1\u00e9\u00ed\u00f3\u00fa";
        String headerQString = headerName + "=" + headerValue;
        doHeaderTest("test_headers_list_contains_accented_latin1_values",
                headerQString);
        assertEquals(
                "Status code != 200: ServerError, =='" + getStatus() + "'",
                200, getStatus());
        assertTrue("Header '" + headerName + "' not returned: ", getResponse()
    ...
    public void testHopByHopHeaders() throws Exception {
        ...
        for (String hopByHopHeader : hop_by_hop_headers) {
            ...
            assertEquals("Status code != 500: ServerError, =='" + getStatus()
                    + "'", 500, getStatus());
            assertTrue(
                    "Could not find exception 'HopByHopHeaderSet' in output",
