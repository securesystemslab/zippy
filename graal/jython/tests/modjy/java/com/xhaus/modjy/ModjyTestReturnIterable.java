...
public class ModjyTestReturnIterable extends ModjyTestBase {
    ...
    public void doReturnTest(String appName, String queryString)
            throws Exception {
        returnTestSetUp();
        setAppName(appName);
        createServlet();
    ...
    public void doWrongReturnTypeTest(String appName, String typeString)
            throws Exception {
        // Try to return not-iterables
        doReturnTest(appName, typeString);
        assertEquals(
                "Status code != 500: ServerError, =='" + getStatus() + "'",
                500, getStatus());
        String firstLine = getOutput().split("\n")[0];
        assertTrue("Could not find exception 'ReturnNotIterable' in output: "
                + firstLine, firstLine.indexOf("ReturnNotIterable") != -1);
    }
    ...
    public void doIterableContainsWrongReturnTypeTest(String appName,
        ...
        assertEquals(
                ...
                500, getStatus());
        String firstLine = getOutput().split("\n")[0];
        assertTrue("Could not find exception 'NonStringOutput' in output: "
                + firstLine, firstLine.indexOf("NonStringOutput") != -1);
    }

    public void testReturnIterableContainingInt() throws Exception {
        doIterableContainsWrongReturnTypeTest(
                "test_iterable_containing_non_strings_return", "int");
    }

    public void testReturnIterableContainingFloat() throws Exception {
        doIterableContainsWrongReturnTypeTest(
                "test_iterable_containing_non_strings_return", "float");
    }

    public void testReturnIterableContainingNone() throws Exception {
        doIterableContainsWrongReturnTypeTest(
                "test_iterable_containing_non_strings_return", "none");
    }
    ...
    public void testStartResponseNotCalled() throws Exception {
        ...
        String firstLine = getOutput().split("\n")[0];
        assertTrue(
                "Could not find exception 'StartResponseNotCalled' in output: "
                        + firstLine,
    ...
    public void doCorrectIterableTest(String appName) throws Exception {
        ...
        assertEquals(
                "Status code != 200: ServerError, =='" + getStatus() + "'",
                200, getStatus());
