...
public class Name extends expr implements Context {
    ...
    private final static PyString[] fields = new PyString[] {
            new PyString("id"), new PyString("ctx") };
    ...
    public Object executeGeneric(VirtualFrame frame) {
        if (ctx == expr_contextType.Load || ctx == expr_contextType.AugStore) {
            ;
            if (ASTInterpreter.debug) {
                System.out.println("GET " + frame + " slot " + slot + " name " + id);
            }

            if (slot == null) {
                return GlobalScope.getInstance().get(id);
            } else if (slot instanceof EnvironmentFrameSlot) {
                Object val = frame.getCaller().unpack().getObject(slot);
                return val;
            } else {
                // This is where PyObjectContainer got unpacked
                Object val = frame.getObject(slot);
                if (val instanceof PyObjectContainer) {
                    val = ((PyObjectContainer) val).unpack();
                }
                return val;
            }
        }
