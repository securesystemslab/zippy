...
public class Tuple extends expr implements Context {
    ...
    public void setElts(PyObject elts) {
        ...
        if (Options.specialize) {
            ...
            for (int i = 0; i < ((List) elts).size(); i++) {
                this.elts.add(adoptChild(AstAdapters.py2expr(this.elts.get(i))));
            }
        } else {
    ...
    private final static PyString[] fields = new PyString[] {
            new PyString("elts"), new PyString("ctx") };
    ...
    public Tuple(Integer ttype, Token token, java.util.List<expr> elts,
    ...
    public Tuple(PythonTree tree, java.util.List<expr> elts,
            expr_contextType ctx) {
        super(tree);
        this.elts = elts;
        if (elts == null) {
    ...
    public Object executeGeneric(VirtualFrame frame) {
        List<PyObject> elements = new ArrayList<PyObject>();

        for (expr e : elts) {
            elements.add((PyObject) e.executeGeneric(frame));
        }

        return new PyTuple(elements.toArray(new PyObject[elements.size()]));
    }
