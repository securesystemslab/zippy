...
public class imp {
    ...
    private static class ModuleInfo {
        ...
        ModuleInfo(PyObject file, String filename, String suffix, String mode,
                int type) {
            this.file = file;
    ...
    /**
     * This needs to be consolidated with the code in (@see
     * org.python.core.imp).
     * 
     * @param name
     *            module name
     * @param entry
     *            a path String
     * @param findingPackage
     *            if looking for a package only try to locate __init__
     * @return null if no module found otherwise module information
     */
    static ModuleInfo findFromSource(String name, String entry,
            boolean findingPackage, boolean preferSource) {
        ...
        File dir = findingPackage ? new File(directoryName) : new File(
                directoryName, name);
        File sourceFile = new File(dir, sourceName);
        File compiledFile = new File(dir, compiledName);

        boolean pkg = dir.isDirectory() && caseok(dir, name)
                && (sourceFile.isFile() || compiledFile.isFile());

        if (!findingPackage) {
            if (pkg) {
                return new ModuleInfo(Py.None,
                        new File(displayDirName, name).getPath(), "", "",
                        PKG_DIRECTORY);
            } else {
        ...
        if (sourceFile.isFile() && caseok(sourceFile, sourceName)) {
            if (!preferSource && compiledFile.isFile()
                    && caseok(compiledFile, compiledName)) {
                Py.writeDebug("import",
                        "trying precompiled " + compiledFile.getPath());
                long pyTime = sourceFile.lastModified();
                long classTime = compiledFile.lastModified();
                if (classTime >= pyTime) {
                    return new ModuleInfo(newFile(compiledFile), new File(
                            displayDirName, compiledName).getPath(), ".class",
                            "rb", PY_COMPILED);
                }
            }
            return new ModuleInfo(newFile(sourceFile), new File(displayDirName,
                    sourceName).getPath(), ".py", "r", PY_SOURCE);
        }
        ...
        if (compiledFile.isFile() && caseok(compiledFile, compiledName)) {
            return new ModuleInfo(newFile(compiledFile), new File(
                    displayDirName, compiledName).getPath(), ".class", "rb",
                    PY_COMPILED);
        }
        return null;
    }
    ...
    public static PyObject load_source(String modname, String filename,
        ...
        if (file == null) {
            // XXX: This should load the accompanying byte code file instead, if
            // it exists
        ...
        String compiledFilename = org.python.core.imp.makeCompiledFilename(sys
                .getPath(filename));
        mod = org.python.core.imp.createFromSource(modname.intern(),
                (InputStream) o, filename, compiledFilename);
    ...
    public static PyObject load_compiled(String name, String pathname,
        ...
        return org.python.core.imp.loadFromCompiled(name.intern(), stream,
                sourceName, pathname);
    }
    ...
    public static PyObject find_module(String name, PyObject path) {
        if (path == Py.None && PySystemState.getBuiltin(name) != null) {
            return new PyTuple(Py.None, Py.newString(name), new PyTuple(
                    Py.EmptyString, Py.EmptyString, Py.newInteger(C_BUILTIN)));
        }
        ...
        for (PyObject p : path.asIterable()) {
            ...
            return new PyTuple(mi.file, new PyString(mi.filename), new PyTuple(
                    new PyString(mi.suffix), new PyString(mi.mode),
                    Py.newInteger(mi.type)));
        }
        throw Py.ImportError("No module named " + name);
    }

    public static PyObject load_module(String name, PyObject file,
            PyObject filename, PyTuple data) {
        PyObject mod = Py.None;
        PySystemState sys = Py.getSystemState();
        ...
        while (mod == Py.None) {
            ...
            case PY_SOURCE:
                ...
                compiledName = org.python.core.imp
                        .makeCompiledFilename(resolvedFilename);
                if (name.endsWith(".__init__")) {
                    name = name.substring(0,
                            name.length() - ".__init__".length());
                ...
                mod = org.python.core.imp.createFromSource(name.intern(),
                        (InputStream) o, filename.toString(), compiledName,
                        mtime);
                break;
            case PY_COMPILED:
    ...
    public static PyObject get_suffixes() {
        return new PyList(new PyObject[] {
                new PyTuple(new PyString(".py"), new PyString("r"),
                        Py.newInteger(PY_SOURCE)),
                new PyTuple(new PyString("$py.class"), new PyString("rb"),
