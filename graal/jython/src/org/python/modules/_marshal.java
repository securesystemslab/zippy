    ...
    public static class Marshaller extends PyObject {
        ...
        private void write_object(PyObject v, int depth) {
            if (depth >= MAX_MARSHAL_STACK_DEPTH) {
                throw Py.ValueError("Maximum marshal stack depth"); // XXX - fix
                                                                    // this
                                                                    // exception
            } else if (v == null) {
                ...
                write_object(Py.newString(new String(code.co_lnotab)),
                        depth + 1);
            } else {
    ...
    public static class Unmarshaller extends PyObject {
        ...
        public PyObject load() {
            try {
                ...
                // convert from our PyIOFile abstraction to what marshal in
                // CPython returns
        ...
        private PyObject read_object(int depth) {
            ...
            case TYPE_CODE: {
                // XXX - support restricted execution mode? not certain if this
                // is just legacy
                ...
                PyObject consts[] = ((PyTuple) read_object_notnull(depth + 1))
                        .getArray();
                 ...
                 * ModularVM
                 */
                PyBytecode pb = new PyBytecode(argcount, nlocals, stacksize,
                flags, code, consts, names, varnames, filename, name,
                firstlineno, lnotab, cellvars, freevars);

//                if (Options.subroutineInterpreter) {
//                    return new PyThreadedCodeInterpreter(pb);
//                }
                return pb;
            }
            default:
