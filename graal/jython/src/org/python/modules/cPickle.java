 ...
 * arbitrary Python objects. This is the act of converting objects to a stream
 * of bytes (and back: ``unpickling''). This is a more primitive notion than
 * persistency -- although <tt>cPickle.java</tt> reads and writes file objects,
 * it does not handle the issue of naming persistent objects, nor the (even more
 * complicated) area of concurrent access to persistent objects. The
 * <tt>cPickle.java</tt> module can transform a complex object into a byte
 * stream and it can transform the byte stream into an object with the same
 * internal structure. The most obvious thing to do with these byte streams is
 * to write them onto a file, but it is also conceivable to send them across a
 * network or store them in a database. The module <tt>shelve</tt> provides a
 ...
 * <tt>Unpickler</tt> are factory functions, not classes (so they cannot be used
 * as base classes for inheritance). This limitation is similar for the original
 * cPickle.c version.
 * 
 * <P>
 ...
 * the advantage that there are no restrictions imposed by external standards
 * such as XDR (which can't represent pointer sharing); however it means that
 ...
 * representation. The big advantage of using printable ASCII (and of some other
 * characteristics of <tt>cPickle.java</tt>'s representation) is that for
 * debugging or recovery purposes it is possible for a human to read the pickled
 ...
 * compatibility with the Python 1.4 pickle module. In a future version, the
 * default may change to binary.
 * 
 * <P>
 ...
 * it supports the notion of a reference to an object outside the pickled data
 * stream. Such objects are referenced by a name, which is an arbitrary string
 * of printable ASCII characters. The resolution of such names is not defined by
 * the <tt>cPickle.java</tt> module -- the persistent object module will have to
 * implement a method <tt>persistent_load()</tt>. To write references to
 * persistent objects, the persistent module must define a method
 * <tt>persistent_id()</tt> which returns either <tt>None</tt> or the persistent
 * ID of the object.
 * 
 * <P>
 ...
 * 
 * <P>
 * When a pickled class instance is unpickled, its <tt>__init__()</tt> method is
 * normally <i>not</i> invoked. <b>Note:</b> This is a deviation from previous
 * versions of this module; the change was introduced in Python 1.5b2. The
 * reason for the change is that in many cases it is desirable to have a
 * constructor that requires arguments; it is a (minor) nuisance to have to
 * provide a <tt>__getinitargs__()</tt> method.
 * 
 * <P>
 * If it is desirable that the <tt>__init__()</tt> method be called on
 * unpickling, a class can define a method <tt>__getinitargs__()</tt>, which
 * should return a <i>tuple</i> containing the arguments to be passed to the
 * class constructor (<tt>__init__()</tt>). This method is called at pickle
 * time; the tuple it returns is incorporated in the pickle for the instance.
 * <P>
 * Classes can further influence how their instances are pickled -- if the class
 * defines the method <tt>__getstate__()</tt>, it is called and the return state
 * is pickled as the contents for the instance, and if the class defines the
 * method <tt>__setstate__()</tt>, it is called with the unpickled state. (Note
 * that these methods can also be used to implement copying class instances.) If
 * there is no <tt>__getstate__()</tt> method, the instance's <tt>__dict__</tt>
 * is pickled. If there is no <tt>__setstate__()</tt> method, the pickled object
 * must be a dictionary and its items are assigned to the new instance's
 * dictionary. (If a class defines both <tt>__getstate__()</tt> and
 * <tt>__setstate__()</tt>, the state object needn't be a dictionary -- these
 * methods can do what they want.) This protocol is also used by the shallow and
 * deep copying operations defined in the <tt>copy</tt> module.
 * <P>
 * Note that when class instances are pickled, their class's code and data are
 * not pickled along with them. Only the instance data are pickled. This is done
 * on purpose, so you can fix bugs in a class or add methods and still load
 * objects that were created with an earlier version of the class. If you plan
 * to have long-lived objects that will see many versions of a class, it may be
 * worthwhile to put a version number in the objects so that suitable
 * conversions can be made by the class's <tt>__setstate__()</tt> method.
 * 
 * <P>
 ...
 * Therefore, the restriction that the class must be defined at the top level in
 * a module applies to pickled classes as well.
 * 
 * <P>
 * 
 * <P>
 ...
 * (with an integer argument) and <tt>f.readline()</tt> (without argument), both
 * returning a string. It is explicitly allowed to pass non-file objects here,
 * as long as they have the right methods.
 * 
 * <P>
 * The constructor for the <tt>Pickler</tt> class has an optional second
 * argument, <i>bin</i>. If this is present and nonzero, the binary pickle
 * format is used; if it is zero or absent, the (less efficient, but backwards
 * compatible) text pickle format is used. The <tt>Unpickler</tt> class does not
 * have an argument to distinguish between binary and text pickle formats; it
 ...
 * 
 * <P>
 * Attempts to pickle unpicklable objects will raise the <tt>PicklingError</tt>
 * exception; when this happens, an unspecified number of bytes may have been
 * written to the file.
 * 
 * <P>
 * It is possible to make multiple calls to the <tt>dump()</tt> method of the
 * same <tt>Pickler</tt> instance. These must then be matched to the same number
 * of calls to the <tt>load()</tt> method of the corresponding
 * <tt>Unpickler</tt> instance. If the same object is pickled by multiple
 * <tt>dump()</tt> calls, the <tt>load()</tt> will all yield references to the
 * same object. <i>Warning</i>: this is intended for pickling multiple objects
 * without intervening modifications to the objects or their parts. If you
 * modify an object and then pickle it again using the same <tt>Pickler</tt>
 * instance, the object is not pickled again -- a reference to it is pickled and
 * the <tt>Unpickler</tt> will return the old value, not the modified one.
 * (There are two problems here: (a) detecting changes, and (b) marshalling a
 * minimal set of changes. I have no answers. Garbage Collection may also become
 * a problem here.)
 * 
 * <P>
 * Apart from the <tt>Pickler</tt> and <tt>Unpickler</tt> classes, the module
 * defines the following functions, and an exception:
 * 
 * <P>
 * <dl>
 ...
 * <tt>Pickler(<i>file</i>, <i>bin</i>).dump(<i>object</i>)</tt>". If the
 * optional <i>bin</i> argument is present and nonzero, the binary pickle format
 * is used; if it is zero or absent, the (less efficient) text pickle format is
 * used.
 * </dl>
 * 
 * <P>
 ...
 * writing it to a file. If the optional <i>bin</i> argument is present and
 * nonzero, the binary pickle format is used; if it is zero or absent, the (less
 ...
 * <dd>
 * Read a pickled object from a string instead of a file. Characters in the
 ...
 * <p>
 * 
 * The module is based on both original pickle.py and the cPickle.c version,
...
public class cPickle implements ClassDictInit {
     ...
     */
    public static final String[] compatible_formats = new String[] { "1.0",
            "1.1", "1.2", "1.3", "2.0" };
    ...
    private static PyType BuiltinCallableType = PyType
            .fromClass(PyBuiltinCallable.class);

    private static PyType ReflectedFunctionType = PyType
            .fromClass(PyReflectedFunction.class);
    ...
    public static void classDictInit(PyObject dict) {
        ...
        extension_registry = (PyDictionary) copyreg
                .__getattr__("_extension_registry");
        inverted_registry = (PyDictionary) copyreg
                .__getattr__("_inverted_registry");

        PickleError = Py.makeClass("PickleError", Py.Exception, _PickleError());
        PicklingError = Py.makeClass("PicklingError", PickleError,
                exceptionNamespace());
        UnpickleableError = Py.makeClass("UnpickleableError", PicklingError,
                _UnpickleableError());
        UnpicklingError = Py.makeClass("UnpicklingError", PickleError,
                exceptionNamespace());
        BadPickleGet = Py.makeClass("BadPickleGet", UnpicklingError,
                exceptionNamespace());
    }
    ...
    public static PyObject _PickleError() {
        ...
        dict.__setitem__("__str__",
                getJavaFunc("__str__", "_PickleError__str__"));
        return dict;
    }

    public static PyString _PickleError__str__(PyObject self, PyObject[] args,
            String[] kwargs) {
    ...
    public static PyObject _UnpickleableError() {
        dict = exceptionNamespace();
        dict.__setitem__("__str__",
                getJavaFunc("__str__", "_UnpickleableError__str__"));
        return dict;
    }

    public static PyString _UnpickleableError__str__(PyObject self,
            PyObject[] args, String[] kwargs) {
        PyObject selfArgs = self.__getattr__("args");
        PyObject a = selfArgs.__len__() > 0 ? selfArgs.__getitem__(0)
                : new PyString("(what)");
    ...
    /**
     * @param file
     *            a file-like object, can be a cStringIO.StringIO, a PyFile or
     *            any python object which implements a <i>write</i> method. The
     *            data will be written as text.
     ...
     * @param file
     *            a file-like object, can be a cStringIO.StringIO, a PyFile or
     *            any python object which implements a <i>write</i> method.
     * @param protocol
     *            pickle protocol version (0 - text, 1 - pre-2.3 binary, 2 -
     *            2.3)
     * @return a new Pickler instance.
     */
     ...
     * @param file
     *            a file-like object, can be a cStringIO.StringIO, a PyFile or
     *            any python object which implements a <i>read</i> and
     *            <i>readline</i> method.
    ...
    /**
     * Shorthand function which unpickles a object from the file and returns the
     * new object.
     * 
     * @param file
     *            a file-like object, can be a cStringIO.StringIO, a PyFile or
     ...
     * 
     * @param str
     *            a strings which must contain a pickled object representation.
     * @return a new object.
     */
    ...
    static public class Pickler {
        ...
        /**
         * To write references to persistent objects, the persistent module must
         * assign a method to persistent_id which returns either None or the
         * persistent ID of the object. For the benefit of persistency modules
         * written using pickle, it supports the notion of a reference to an
         * object outside the pickled data stream. Such objects are referenced
         * by a name, which is an arbitrary string of printable ASCII
         * characters.
         */
        ...
        private static final int get_id(PyObject o) {
            // we don't pickle Java instances so we don't have to consider that
            // case
        ...
        private void save(PyObject object, boolean pers_save) {
            if (!pers_save && persistent_id != null
                    && save_pers(object, persistent_id)) {
                return;
            }
            ...
            if (!pers_save && inst_persistent_id != null
                    && save_pers(object, inst_persistent_id)) {
                return;
            }
            ...
            if (!(tup instanceof PyTuple)) {
                throw new PyException(PicklingError, "Value returned by "
                        + reduce.__repr__() + " must be a tuple");
            }

            int l = tup.__len__();
            if (l < 2 || l > 5) {
                throw new PyException(PicklingError, "tuple returned by "
                        + reduce.__repr__()
        ...
        private void save_unicode(PyObject object) {
            if (protocol > 0) {
                String str = codecs.PyUnicode_EncodeUTF8(object.toString(),
                        "struct");
                ...
                file.write(UNICODE);
                file.write(codecs.PyUnicode_EncodeRawUnicodeEscape(
                        object.toString(), "strict", true));
                file.write("\n");
            }
            put(putMemo(get_id(object), object));
        }
         ...
         * possibly temporary objects are kept alive by referencing them. We
         * store a reference at the id of the memo, which should normally not be
         * used unless someone tries to deepcopy the memo itself...
         */
     ...
     * integers as keys and stores both an integer and an object as value. It is
     * very private! And should only be used thread-confined.
     */
    static private class PickleMemo {
        // Table of primes to cycle through
        private final int[] primes = { 13, 61, 251, 1021, 4093, 5987, 9551,
                15683, 19609, 31397, 65521, 131071, 262139, 524287, 1048573,
    ...
    static public class Unpickler {
         ...
         * such names is not defined by the pickle module -- the persistent
         * object module will have to add a method persistent_load().
         */
        ...
        /**
         * Unpickle and return an instance of the object represented by the
         * file.
         */
        public PyObject load() {
            ...
            while (true) {
                ...
                case STOP:
                    return load_stop();
                default:
                    throw new PyException(UnpicklingError, String.format(
                            "invalid load key, '%s'.", key));
                }
            }
        }
        ...
        private int read_binint() {
            ...
            return s.charAt(0) | (s.charAt(1) << 8) | (s.charAt(2) << 16)
                    | (s.charAt(3) << 24);
        }
        ...
        final private void load_binfloat() {
            ...
            long bits = s.charAt(7) | ((long) s.charAt(6) << 8)
                    | ((long) s.charAt(5) << 16) | ((long) s.charAt(4) << 24)
                    | ((long) s.charAt(3) << 32) | ((long) s.charAt(2) << 40)
        ...
        final private void load_string() {
            ...
            value = PyString.decode_UnicodeEscape(line, 1, n - 1, "strict",
                    false);
        ...
        final private PyObject find_class(String module, String name) {
            if (find_global != null) {
                ...
                return find_global.__call__(new PyString(module), new PyString(
                        name));
            }
            ...
            if (global == null) {
                throw new PyException(Py.SystemError, "Failed to import class "
        ...
        private void load_build() {
            ...
            // A default __setstate__. First see whether state embeds a slot
            // state dict
            ...
            if (state != Py.None) {
                ...
                for (PyObject item : ((PyDictionary) state).iteritems()
                        .asIterable()) {
            ...
            if (slotstate != null) {
                ...
                for (PyObject item : ((PyDictionary) slotstate).iteritems()
                        .asIterable()) {
