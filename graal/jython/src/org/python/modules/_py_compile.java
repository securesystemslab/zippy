...
public class _py_compile {
    public static PyList __all__ = new PyList(new PyString[] { new PyString(
            "compile") });

    public static boolean compile(String filename, String cfile, String dfile) {
        // Resolve relative path names. dfile is only used for error messages
        // and should not be
        // resolved
        ...
        byte[] bytes = org.python.core.imp.compileSource(name, file, dfile,
                cfile);
    ...
    public static final String getModuleName(File f) {
        ...
        // Make the compiled classfile's name fully qualified with a package by
        // walking up the
        // directory tree looking for __init__.py files. Don't check for
        // __init__$py.class since
