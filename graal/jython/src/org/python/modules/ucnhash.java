...
public class ucnhash implements ucnhashAPI {
    ...
    private static final char[] charmap = " ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-"
            .toCharArray();
    ...
    private static int compare(byte[] a1, int off1, int len1, byte[] a2,
            int off2, int len2) {
    ...
    private static int binarysearch(byte[] rawlist, int start, int end) {
        ...
        long lengths = ((long) rawindex[tmp++] << 48)
                ...
                | ((long) rawindex[tmp++] << 16) | rawindex[tmp++];
    ...
    private static int lookup(String name, int start, int end) {
        ...
        while (true) {
            ...
            boolean isWord = hash >= 0 && ridx - rbegin > 1
