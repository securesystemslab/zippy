...
public class zipimporter extends importer<PyObject> {
    ...
    final void zipimporter___init__(PyObject[] args, String[] kwds) {
        ArgParser ap = new ArgParser("__init__", args, kwds,
                new String[] { "path" });
    ...
    /**
     * Return the uncompressed data for the file at the specified path as a
     * String.
     * 
     * @param path
    ...
    final boolean zipimporter_is_package(String fullname) {
        ...
        if (moduleInfo == ModuleInfo.NOT_FOUND) {
            throw zipimport.ZipImportError(String.format(
                    "can't find module '%s'", fullname));
        }
    ...
    /**
     * Return the source code for the module as a string (using newline
    ...
    /**
     * Given a path to a compressed file in the archive, return the file's
     * (uncompressed) data stream in a ZipBundle.
     * 
     * @param datapath
     *            file's filename inside of the archive
     * @return a ZipBundle with an InputStream to the file's uncompressed data
     */
    @Override
    public ZipBundle makeBundle(String datapath, PyObject entry) {
        ...
        try {
            ...
            throw zipimport.ZipImportError("zipimport: can not open file: "
                    + archive);
        }

        ZipEntry dataEntry = zipArchive.getEntry(datapath);
        try {
            return new ZipBundle(zipArchive,
                    zipArchive.getInputStream(dataEntry));
        } catch (IOException ioe) {
     ...
     * 
     * Given a path to a Zip archive, build a dict, mapping file names (local to
     * the archive, using SEP as a separator) to toc entries.
     * 
     * @param archive
     ...
     * 
     * (__file__, # value to use for __file__, available for all files compress,
     * # compression kind; 0 for uncompressed data_size, # size of compressed
     * data on disk file_size, # size of decompressed data file_offset, # offset
     * of file header from start of archive (or -1 in Jython) time, # mod time
     * of file (in dos format) date, # mod data of file (in dos format) crc, #
     * crc checksum of the data )
     * 
     * Directories can be recognized by the trailing SEP in the name, data_size
     * and file_offset are 0.
     * 
     * @param zipFile
     *            ZipFile to read
     * @param files
    ...
    private void readZipFile(ZipFile zipFile, PyObject files) {
        for (Enumeration<? extends ZipEntry> zipEntries = zipFile.entries(); zipEntries
            ...
            // file_offset is a CPython optimization; it's used to seek directly
            // to the
            // file when reading it later. Jython doesn't do this nor is the
            // offset
            // available
            ...
            PyTuple entry = new PyTuple(__file__, compress, data_size,
    ...
    /**
     * Convert the date/time values found in the Zip archive to a long time (in
     * milliseconds) value.
     * 
     * @param dostime
    ...
    private long dosTimeToEpoch(int dosTime, int dosDate) {
        Date d = new Date(((dosDate >> 9) & 0x7f) + 80,
                ((dosDate >> 5) & 0x0f) - 1, dosDate & 0x1f,
                (dosTime >> 11) & 0x1f, (dosTime >> 5) & 0x3f,
                (dosTime & 0x1f) * 2);
        return d.getTime();
    }
    ...
    /**
     * ZipBundle is a ZipFile and one of its InputStreams, bundled together so
     * the ZipFile can be closed when finished with its InputStream.
     */
