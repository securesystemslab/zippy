...
public class _codecs {
    ...
    public static PyTuple utf_8_decode(String str, String errors, boolean final_) {
        ...
        return decode_tuple(
                codecs.PyUnicode_DecodeUTF8Stateful(str, errors, consumed),
    ...
    public static PyTuple utf_7_encode(String str, String errors) {
        int size = str.length();
        return encode_tuple(
                codecs.PyUnicode_EncodeUTF7(str, false, false, errors), size);
    }
    ...
    public static PyTuple escape_decode(String str, String errors) {
        return decode_tuple_str(PyString.decode_UnicodeEscape(str, 0,
                str.length(), errors, true), str.length());
    }
    ...
    public static PyTuple charmap_decode(String str, String errors,
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (ch > 0xFF) {
                i = codecs.insertReplacementAndGetResume(v, errors, "charmap",
                        str, i, i + 1, "ordinal not in range(255)") - 1;
                continue;
            }
            ...
            if (x == null) {
                if (ignoreUnmapped) {
                    ...
                    i = codecs.insertReplacementAndGetResume(v, errors,
                            "charmap", str, i, i + 1, "no mapping found") - 1;
                }
                continue;
            }
            /* Apply mapping */
            if (x instanceof PyInteger) {
                ...
                i = codecs.insertReplacementAndGetResume(v, errors, "charmap",
                        str, i, i + 1, "character maps to <undefined>") - 1;
    ...
    public static PyObject translateCharmap(PyUnicode str, String errors,
            PyObject mapping) {
        StringBuilder buf = new StringBuilder(str.toString().length());

        for (Iterator<Integer> iter = str.newSubsequenceIterator(); iter
                .hasNext();) {
            ...
            if (result == null) {
                ...
                // XXX: We don't support the fancier error handling CPython does
                // here of
                // capturing regions of chars removed by the None mapping to
                // optionally
                // pass to an error handler. Though we don't seem to even use
                // this
    ...
    public static PyTuple charmap_encode(String str, String errors,
        ...
        return charmap_encode_internal(str, errors, mapping, new StringBuilder(
                str.length()), true);
    }

    private static PyTuple charmap_encode_internal(String str, String errors,
            PyObject mapping, StringBuilder v, boolean letLookupHandleError) {
        EncodingMap encodingMap = mapping instanceof EncodingMap ? (EncodingMap) mapping
                : null;
        int size = str.length();
        for (int i = 0; i < size; i++) {
            char ch = str.charAt(i);
    ...
    private static int handleBadMapping(String str, String errors,
            PyObject mapping, StringBuilder v, int size, int i) {
        if (errors != null) {
        ...
        PyObject replacement = codecs.encoding_error(errors, "charmap", str, i,
                i + 1, "character maps to <undefined>");
    ...
    public static PyTuple latin_1_encode(String str, String errors) {
        int size = str.length();
        return encode_tuple(codecs.PyUnicode_EncodeLatin1(str, size, errors),
                size);
    }
    ...
    public static PyTuple utf_16_ex_decode(String str, String errors,
            int byteorder, boolean final_) {
        int[] bo = new int[] { 0 };
        int[] consumed = final_ ? null : new int[1];
        String decoded = decode_UTF16(str, errors, bo, consumed);
        return new PyTuple(Py.newString(decoded), Py.newInteger(final_ ? str
                .length() : consumed[0]), Py.newInteger(bo[0]));
    }
    ...
    private static String decode_UTF16(String str, String errors,
        ...
        for (i = 0; i < size; i += 2) {
            ...
            if (i + 1 == size) {
                ...
                i = codecs.insertReplacementAndGetResume(v, errors, "utf-16",
                        str, i, i + 1, "truncated data");
                continue;
            }
            ...
            if (W1 < 0xD800 || W1 > 0xDFFF) {
                ...
                i = codecs.insertReplacementAndGetResume(v, errors, "utf-16",
                        str, i, i + 1, "illegal UTF-16 surrogate");
                continue;
            }
    ...
    public static PyTuple raw_unicode_escape_encode(String str, String errors) {
        return encode_tuple(
                codecs.PyUnicode_EncodeRawUnicodeEscape(str, errors, false),
                str.length());
    }
    ...
    public static PyTuple raw_unicode_escape_decode(String str, String errors) {
        return decode_tuple(
                codecs.PyUnicode_DecodeRawUnicodeEscape(str, errors),
                str.length());
    }
    ...
    public static PyTuple unicode_escape_decode(String str, String errors) {
        int n = str.length();
        return decode_tuple(
                PyString.decode_UnicodeEscape(str, 0, n, errors, true), n);
    }
     ...
     * 
     * Uses a trie structure instead of a dictionary; the speedup primarily
     * comes from not creating integer objects in the process. The trie is
     * created by inverting the encoding map.
     */
    ...
    public static class EncodingMap extends PyObject {
        ...
        private EncodingMap(char[] level1, char[] level23, int count2,
                int count3) {
            this.level1 = level1;
         ...
         * Returns a PyDictionary if the mapping isn't easily optimized.
         * 
         * @param string
         *            a 256 length unicode mapping
        ...
        public static PyObject buildEncodingMap(PyObject string) {
            ...
            if (needDict) {
                ...
                for (i = 0; i < 256; i++) {
                    result.__setitem__(Py.newInteger(decode.charAt(i)),
                            Py.newInteger(i));
                }
                return result;
            }
