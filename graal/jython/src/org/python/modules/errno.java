...
public class errno implements ClassDictInit {
    ...
    public static final PyString __doc__ = Py
            .newString("This module makes available standard errno system symbols.\n\n"
     ...
     * 
     * Windows replaced the BSD/POSIX socket errnos with its own Winsock
     * equivalents (e.g. EINVAL -> WSAEINVAL). We painstakenly map the missing
     * constants to their WSA equivalent values and expose the WSA constants on
     * their own.
     */
    private static void initWindows(PyObject dict) {
        ...
        // Fill the gaps by searching through every possible constantine Errno
        // first
        // checking if it's defined on Windows, then falling back to the WSA
        // prefixed
        ...
        for (Constant lastError : lastErrors) {
            if (lastError.name().startsWith("WSA")) {
                addCode(dict, lastError.name(), lastError.value(),
                        lastError.toString());
            }
        }
    }
    ...
    private static void initPosix(PyObject dict) {
        for (Constant constant : ConstantSet.getConstantSet("Errno")) {
            addCode(dict, constant.name(), constant.value(),
                    constant.toString());
        }
    }

    private static void addCode(PyObject dict, String name, int code,
            String message) {
    ...
    /**
     * @deprecated Use
     *             org.python.core.constantine.Errno.valueOf(code).toString()
     *             (or os.strerror from Python) instead.
     */
    @Deprecated
