...
public class ReferenceType extends AbstractReference {
    ...
    @ExposedNew
    static final PyObject weakref___new__(PyNewWrapper new_, boolean init,
        ...
        if (new_.for_type == subtype) {
            // NOTE: CPython disallows weakrefs to many builtin types (e.g.
            // dict, list)
            // and would check weakrefability here. We aren't as strict since
            // the JVM can
            // weakref anything. Our types' needs_weakref flag only refers to
            // whether it
            // has a __weakref__ descriptor, not weakrefability
            if (callback == null) {
                ReferenceType ret = (ReferenceType) gref
                        .find(ReferenceType.class);
                if (ret != null) {
    ...
    /**
     * Return an ArgParser setup to ignore keyword args (allowing them to
     * passthru).
     * 
     * @param funcName
     *            the name of the caller
     * @param args
     *            PyObject array of args
     * @param keywords
     *            String array of keywords
     * @return an ArgParser instance
     */
    private static ArgParser parseInitArgs(String funcName, PyObject[] args,
            String[] keywords) {
    ...
    public String toString() {
        ...
        if (nameObj != null) {
            return String.format("<weakref at %s; to '%.50s' at %s (%s)>",
                    Py.idstr(this), obj.getType().fastGetName(), Py.idstr(obj),
                    nameObj);
        }
        return String.format("<weakref at %s; to '%.50s' at %s>",
                Py.idstr(this), obj.getType().fastGetName(), Py.idstr(obj));
    }
}
