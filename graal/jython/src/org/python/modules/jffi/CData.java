...
public abstract class CData extends PyObject {
    ...
    /**
     * Wraps up this object in a pointer that can be passed to native code. The
     * byref() return value cannot be used as anything other than a parameter.
     * 
    ...
    public PyObject byref(PyObject offset) {
        return new ByReference(ctype, (DirectMemory) getReferenceMemory()
                .slice(offset.asInt()));
    }
    ...
    public PyObject pointer(PyObject pytype) {
        ...
        return new PointerCData((PyType) pytype, CType.typeOf(pytype),
                getReferenceMemory(), getMemoryOp());
    }
    ...
    /**
     * Returns the memory used when creating a reference to this instance. e.g.
     * via byref(obj)
     * 
    ...
    protected DirectMemory allocateReferenceMemory() {
        DirectMemory m = AllocatedNativeMemory.allocate(getCType().size(),
                false);
    ...
    protected static final DirectMemory findInDll(PyObject lib, PyObject name) {
        if (!(lib instanceof DynamicLibrary)) {
            throw Py.TypeError("expected library, not "
                    + lib.getType().fastGetName());
        }
        DynamicLibrary.Symbol sym = (DynamicLibrary.Symbol) ((DynamicLibrary) lib)
                .find_symbol(name);
