...
public final class HeapMemory implements Memory {
    ...
    private static final ArrayIO getArrayIO() {
        if (ByteOrder.nativeOrder().equals(ByteOrder.BIG_ENDIAN)) {
            return Platform.getPlatform().addressSize() == 64 ? newBE64ArrayIO()
                    : newBE32ArrayIO();
        } else {
            return Platform.getPlatform().addressSize() == 64 ? newLE64ArrayIO()
                    : newLE32ArrayIO();
        }
    }
    ...
    public HeapMemory slice(long offset) {
        ...
        return offset == 0 ? this : new HeapMemory(array(), arrayOffset()
                + (int) offset, arrayLength() - (int) offset);
    }
