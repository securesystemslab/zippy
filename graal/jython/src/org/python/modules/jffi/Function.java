...
public class Function extends BasePointer implements Pointer {
    ...
    @ExposedNew
    public static PyObject Function_new(PyNewWrapper new_, boolean init,
    ...
    private synchronized Invoker tryCompilation() {
        ...
        if (jitHandle == null) {
            jitHandle = JITCompiler.getInstance().getHandle(cResultType,
                    cParameterTypes, CallingConvention.DEFAULT, false);
        }
        ...
        if (jffiFunction == null) {
            ...
            jffiFunction = new com.kenai.jffi.Function(
                    getMemory().getAddress(), jffiReturnType, jffiParamTypes);
        }

        if (defaultInvoker == null) {
            Invoker invoker = DefaultInvokerFactory.getFactory().createInvoker(
                    jffiFunction, restype, argtypes);
            defaultInvoker = errcheck != Py.None ? new ErrCheckInvoker(invoker,
                    errcheck) : invoker;
        }

        Invoker invoker = jitHandle.compile(jffiFunction, null,
                new NativeDataConverter[0]);
        if (invoker != null) {
            return compiledInvoker = errcheck != Py.None ? new ErrCheckInvoker(
                    invoker, errcheck) : invoker;
        }
