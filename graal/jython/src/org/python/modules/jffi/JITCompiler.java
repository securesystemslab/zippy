...
class JITCompiler {
    ...
    private final Map<Class, JITHandle> classes = Collections
            .synchronizedMap(new WeakHashMap<Class, JITHandle>());

    private final ReferenceQueue referenceQueue = new ReferenceQueue();

    private final JITHandle failedHandle = new JITHandle(this,
            new JITSignature(NativeType.VOID, new NativeType[0], false,
                    new boolean[0], CallingConvention.DEFAULT, false), true);
    ...
    private static final class HandleRef extends WeakReference<JITHandle> {
        ...
        public HandleRef(JITHandle handle, JITSignature signature,
    ...
    JITHandle getHandle(PyObject resultType, PyObject[] parameterTypes,
        ...
        if (resultType instanceof CType.Builtin) {
            ...
            /*
             * } else if (resultType instanceof MappedType) { nativeResultType =
             * ((MappedType) resultType).getRealType().getNativeType();
             */
        } else {
            return failedHandle;
        }
        ...
        JITSignature jitSignature = new JITSignature(nativeResultType,
                nativeParameterTypes, hasResultConverter,
        ...
        synchronized (this) {
            ...
            if (handle == null) {
                ...
                handles.put(jitSignature, new HandleRef(handle, jitSignature,
                        referenceQueue));
            }
