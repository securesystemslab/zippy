 ...
 * variable defaultFactory. The remaining functionality is the same as for the
 * dict class.
 * 
 * collections.defaultdict([defaultFactory[, ...]]) - returns a new
 * dictionary-like object. The first argument provides the initial value for the
 * defaultFactory attribute; it defaults to None. All remaining arguments are
 * treated the same as if they were passed to the dict constructor, including
 * keyword arguments.
 */
...
public class PyDefaultDict extends PyDictionary {
    ...
    public PyDefaultDict(PyType subtype) {
        super(subtype, false);
        backingMap = new MapMaker()
                .makeComputingMap(new Function<PyObject, PyObject>() {
    ...
    /**
     * This method is NOT called by the __getitem__ method of the dict class
     * when the requested key is not found. It is simply here as an alternative
     * to the atomic construction of that factory. (We actually inline it in.)
     */
    @ExposedMethod
    ...
    final String defaultdict_toString() {
        return String.format("defaultdict(%s, %s)", defaultFactory,
                super.toString());
    }
