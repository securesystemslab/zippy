...
public abstract class Exposer implements Opcodes, PyTypes {
    ...
    public void generate(ClassVisitor visitor) {
        ...
        cv.visit(V1_5, ACC_PUBLIC, getInternalName(), null,
        ...
        public void pushArgs() {
            if (types.length > 0) {
                throw new IllegalStateException(
                        "If the constuctor takes types as indicated by "
    ...
    protected void callConstructor(Type onType, Type... args) {
        mv.visitMethodInsn(INVOKESPECIAL, onType.getInternalName(), "<init>",
     ...
     */
    protected void call(Type onType, String methodName, Type returnType,
            Type... args) {
        mv.visitMethodInsn(INVOKEVIRTUAL, onType.getInternalName(), methodName,
     ...
     * types.
     */
    protected void callStatic(Type onType, String methodName, Type returnType,
            Type... args) {
        mv.visitMethodInsn(INVOKESTATIC, onType.getInternalName(), methodName,
                methodDesc(returnType, args));
    }
    ...
    protected void get(String fieldName, Type ofType) {
        ...
        mv.visitFieldInsn(GETFIELD, getInternalName(), fieldName,
                ofType.getDescriptor());
    }

    /**
     * Turns an object of inputType on the top of the stack into an equivalent
     * Py type. Handles primitives, void, and String. If void, the top item on
     * the stack isn't touched.
     */
    protected void toPy(Type inputType) {
        ...
        } else if (inputType.equals(INT) || inputType.equals(BYTE)
                || inputType.equals(SHORT)) {
    ...
    protected void getStatic(Type onType, String fieldName, Type ofType) {
        mv.visitFieldInsn(GETSTATIC, onType.getInternalName(), fieldName,
                ofType.getDescriptor());
    }
}
