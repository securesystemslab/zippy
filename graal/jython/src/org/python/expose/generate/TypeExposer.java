...
public class TypeExposer extends Exposer {
    ...
    public TypeExposer(Type onType, Type baseType, boolean isBaseType,
            String doc, String name, Collection<MethodExposer> methods,
            Collection<DescriptorExposer> descriptors, Exposer ne) {
    ...
    private void throwDupe(String exposedName) {
        throw new InvalidExposingException(
                "Only one item may be exposed on a type with a given name[name="
    ...
    protected void generate() {
        ...
        for (MethodExposer exposer : methods) {
            for (final String name : exposer.getNames()) {
                ...
                mv.visitLdcInsn(i++);
                instantiate(exposer.getGeneratedType(),
                        new Instantiator(STRING) {
        ...
        superConstructor(STRING, CLASS, CLASS, BOOLEAN, STRING,
                ABUILTIN_METHOD, ADATA_DESCR, PYNEWWRAPPER);
        endConstructor();
    }
}
