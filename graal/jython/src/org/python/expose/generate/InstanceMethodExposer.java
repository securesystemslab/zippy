...
public class InstanceMethodExposer extends MethodExposer {
    ...
    public InstanceMethodExposer(Type onType, int access, String methodName,
            String desc, String typeName) {
        this(onType, access, methodName, desc, typeName, new String[0],
                new String[0], MethodType.DEFAULT, "");
    }

    public InstanceMethodExposer(Type onType, int access, String methodName,
            String desc, String typeName, String[] asNames, String[] defaults,
            MethodType type, String doc) {
        super(onType, methodName, Type.getArgumentTypes(desc), Type
                .getReturnType(desc), typeName, asNames, defaults,
                isWide(desc) ? PyBuiltinMethod.class
                        : PyBuiltinMethodNarrow.class, doc);
    ...
    private void checkCmpResult() {
        ...
        instantiate(STRING_BUILDER, new Instantiator(STRING) {
            ...
            public void pushArgs() {
                mv.visitLdcInsn(typeName + ".__cmp__(x,y) requires y to be '"
                        + typeName + "', not a '");
            }
        });
        ...
        // throw a type error with our excellent message since this was of the
        // wrong type.
