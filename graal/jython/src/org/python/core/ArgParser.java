...
public class ArgParser {
     ...
     * provided, which may be <code>null</code>. If the returned value is not
     * <code>null</code>, it must be of the specified type.
     * 
     * @param pos
     *            the position of the argument. First argument is numbered 0
     * @param type
    ...
    private static PyObject checkedForType(PyObject arg, int pos, PyType type) {
        if (arg == null || Py.isInstance(arg, type))
            return arg;
        throw Py.TypeError(String.format("argument %d must be %s, not %s",
                pos + 1, type.fastGetName(), arg.getType().fastGetName()));
    }
    ...
    public void noKeywords() {
        if (kws.length > 0) {
            throw Py.TypeError(String.format(
                    "%s does not take keyword arguments", funcname));
        }
    }
    ...
    private Object getArg(int pos, Class clss, String classname, Object def) {
        ...
        if (ret == Py.NoConversion) {
            throw Py.TypeError("argument " + (pos + 1) + ": expected "
                    + classname + ", " + value.getType().fastGetName()
                    + " found");
        }
        return ret;
    }
