...
public class PyBaseException extends PyObject {
    ...
    final void BaseException___init__(PyObject[] args, String[] keywords) {
        ArgParser ap = new ArgParser(getType().getName(), args, keywords,
                "args");
        ap.noKeywords();
    ...
    final PyUnicode BaseException___unicode__() {
        // CPython issue6108: if __str__ has been overridden in the subclass,
        // unicode()
        ...
        if (str != null && where[0] != TYPE) {
            // Unlike str(), __str__ can return unicode (i.e. return the
            // equivalent
    ...
    public PyObject getMessage() {
        ...
        if (__dict__ != null
                && (message = __dict__.__finditem__("message")) != null) {
            return message;
        }
