...
/**
 * A wrapper for all python exception. Note that the wellknown python exception
 * are <b>not</b> subclasses of PyException. Instead the python exception class
 * is stored in the <code>type</code> field and value or class instance is
 * stored in the <code>value</code> field.
 */
public class PyException extends RuntimeException {

    /**
     * The python exception class (for class exception) or identifier (for
     * string exception).
     */
    public PyObject type;

    /**
     * The exception instance (for class exception) or exception value (for
     * string exception).
     */
    ...
    /**
     * Whether the exception was re-raised, such as when a traceback is
     * specified to 'raise', or via a 'finally' block.
     */
    ...
    public Throwable fillInStackTrace() {
        return Options.includeJavaStackInExceptions ? super.fillInStackTrace()
                : this;
    }
    ...
    public void tracebackHere(PyFrame here, boolean isFinally) {
        if (!isReRaise && here != null) {
            // the frame is either inapplicable or already registered (from a
            // finally)
            // during a re-raise
            traceback = new PyTraceback(traceback, here);
        }
        // finally blocks immediately tracebackHere: so they toggle isReRaise to
        // skip the
        ...
        if (isExceptionClass(type)) {
            ...
            if (!isExceptionInstance(pye.value)) {
                throw Py.TypeError(String
                        .format("calling %s() should have returned an instance of BaseException, not '%s'",
                                pye.type, pye.value));
            }
            return pye;
    ...
    public boolean match(PyObject exc) {
        ...
        if (exc == Py.IOError) {
            if (__builtin__.isinstance(value,
                    PyType.fromClass(IOException.class))) {
                return true;
            }
        }
    ...
    public static String exceptionClassName(PyObject obj) {
        return obj instanceof PyClass ? ((PyClass) obj).__name__
                : ((PyType) obj).fastGetName();
    }
}
