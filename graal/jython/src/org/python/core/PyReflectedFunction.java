...
public class PyReflectedFunction extends PyObject {
    ...
    public PyReflectedFunction(Method... methods) {
        ...
        if (nargs == 0) {
            String msg = String.format(
                    "Attempted to make Java method visible, but it isn't "
    ...
    public PyObject _doget(PyObject container, PyObject wherefound) {
        ...
        return new PyMethod(calledStatically ? copyWithCalledStatically(false)
                : this, container, wherefound);
    }

    private ReflectedArgs makeArgs(Method m) {
        return new ReflectedArgs(m, m.getParameterTypes(),
                m.getDeclaringClass(), Modifier.isStatic(m.getModifiers()),
                m.isVarArgs());
    }
    ...
    private PyReflectedFunction copyWithCalledStatically(
    ...
    public void addMethod(Method m) {
        ...
        if (isPackagedProtected(m.getDeclaringClass())) {
             ...
             * that reflected calls to the method in subclasses of the package
             * protected class will succeed. Yes, it's convoluted.
             * 
             * This fails when done through reflection due to Sun JVM bug
             * 4071957(http://tinyurl.com/le9vo). 4533479 actually describes the
             * problem we're seeing, but there are a bevy of reflection bugs
             * that stem from 4071957. Supposedly it'll be fixed in Dolphin but
             * it's been promised in every version since Tiger so don't hold
             * your breath.
             */
            try {
                m.setAccessible(true);
            } catch (SecurityException se) {
                // This case is pretty far in the corner, so don't scream if we
                // can't set the method
                // accessible due to a security manager. Any calls to it will
                // fail with an
                // IllegalAccessException, so it'll become visible there. This
                // way we don't spam
                // people who aren't calling methods like this from Python with
                // warnings if a
    ...
    public static boolean isPackagedProtected(Class<?> c) {
        ...
        return !(Modifier.isPublic(mods) || Modifier.isPrivate(mods) || Modifier
                .isProtected(mods));
    }
    ...
    public PyObject __call__(PyObject self, PyObject[] args, String[] keywords) {
        ...
        // If this is a direct call to a Java class instance method with a
        // PyProxy instance as the
        // arg, use the super__ version to actually route this through the
        // method on the class.
    ...
    protected void throwBadArgError(int errArg, int nArgs, boolean self) {
        ...
        for (int i = 0; i < nargs; i++) {
            ...
            // I'm still a little worried about non-static methods called with
            // an explicit self...
