...
public class PyUnicode extends PyString implements Iterable {
    ...
    public PyUnicode(PyType subtype, PyString pystring) {
        this(subtype, pystring instanceof PyUnicode ? pystring.string
                : pystring.decode().toString());
    }
    ...
    public boolean isBasicPlane() {
        if (plane == Plane.BASIC) {
            ...
            plane = (getString().length() == getCodePointCount()) ? Plane.BASIC
                    : Plane.ASTRAL;
        }
        return plane == Plane.BASIC;
    }

    // RETAIN THE BELOW CODE, it facilitates testing astral support more
    // completely
    ...
    final static PyObject unicode_new(PyNewWrapper new_, boolean init,
        ...
        ArgParser ap = new ArgParser("unicode", args, keywords, new String[] {
        ...
        if (new_.for_type == subtype) {
            ...
            if (S instanceof PyString) {
                if (S.getType() != PyString.TYPE && encoding == null
                        && errors == null) {
                    return S.__unicode__();
                }
                PyObject decoded = codecs
                        .decode((PyString) S, encoding, errors);
    ...
    final PyObject unicode___getslice__(PyObject start, PyObject stop,
            PyObject step) {
    ...
    protected PyObject getslice(int start, int stop, int step) {
        ...
        for (Iterator<Integer> iter = newSubsequenceIterator(start, stop, step); iter
                .hasNext();) {
            buffer.appendCodePoint(iter.next());
        }
    ...
    // XXX: Parameterize SubsequenceIteratorImpl and friends (and make them
    // Iterable)
    public Iterator<Integer> newSubsequenceIterator() {
        return new SubsequenceIteratorImpl();
    }

    public Iterator<Integer> newSubsequenceIterator(int start, int stop,
            int step) {
        if (step < 0) {
    ...
    private static class StripIterator implements Iterator {
        ...
        public StripIterator(PyUnicode sep, Iterator<Integer> iter) {
            ...
            if (sep != null) {
                ...
                for (Iterator<Integer> sepIter = sep.newSubsequenceIterator(); sepIter
                        .hasNext();) {
    ...
    final PyObject unicode_strip(PyObject sepObj) {
        ...
        return new PyUnicode(new ReversedIterator(new StripIterator(sep,
                new ReversedIterator(new StripIterator(sep,
    ...
    private class LineSplitIterator implements Iterator {
        ...
        private final PeekIterator<Integer> iter = new PeekIterator(
                newSubsequenceIterator());
    ...
    protected PyString fromSubstring(int begin, int end) {
        assert (isBasicPlane()); // can only be used on a codepath from str_
                                 // equivalents
    ...
    final int unicode_count(PyObject subObj, PyObject start, PyObject end) {
        ...
        int count = 0;
        for (Iterator<Integer> mainIter = newSubsequenceIterator(indices[0],
                indices[1], 1); mainIter.hasNext();) {
            int matched = sub.getCodePointCount();
            for (Iterator<Integer> subIter = sub.newSubsequenceIterator(); mainIter
                    .hasNext() && subIter.hasNext();) {
    ...
    final PyObject unicode_ljust(int width, String padding) {
        ...
        if (n <= 0) {
            ...
            return new PyUnicode(getString()
                    + padding(n, parse_fillchar("ljust", padding)));
        }
    }
    ...
    final PyObject unicode_rjust(int width, String padding) {
        ...
        if (n <= 0) {
            ...
            return new PyUnicode(padding(n, parse_fillchar("ljust", padding))
                    + getString());
        }
    }
    ...
    final PyObject unicode_center(int width, String padding) {
        ...
        return new PyUnicode(padding(half, pad) + getString()
                + padding(n - half, pad));
    }
    ...
    final PyObject unicode_replace(PyObject oldPieceObj, PyObject newPieceObj,
            int maxsplit) {
        PyUnicode newPiece = coerceToUnicode(newPieceObj);
        PyUnicode oldPiece = coerceToUnicode(oldPieceObj);
        if (isBasicPlane() && newPiece.isBasicPlane()
                && oldPiece.isBasicPlane()) {
        ...
        if (oldPiece.getCodePointCount() == 0) {
            ...
            if (iter.getEndsWithSeparator()
                    && (maxsplit == -1 || numSplits <= maxsplit)) {
                buffer.append(newPiece.getString());
            }
            return new PyUnicode(buffer);
        }
    ...
    @ExposedMethod(defaults = { "null", "null" }, doc = BuiltinDocs.unicode___getslice___doc)
    final boolean unicode_startswith(PyObject prefix, PyObject start,
            PyObject end) {
    ...
    // these tests need to be UTF-16 aware because they are
    ...
    final boolean unicode_isalnum() {
        ...
        for (Iterator<Integer> iter = newSubsequenceIterator(); iter.hasNext();) {
            int codePoint = iter.next();
            if (!(Character.isLetterOrDigit(codePoint) || Character
                    .getType(codePoint) == Character.LETTER_NUMBER)) {
                return false;
            }
        }
        return true;
    ...
    final PyObject unicode_format(PyObject[] args, String[] keywords) {
        try {
            return new PyUnicode(buildFormattedString(getString(), args,
                    keywords, null));
    ...
    /**
     * Encode unicode into a valid decimal String. Throws a UnicodeEncodeError
