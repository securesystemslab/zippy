 ...
 */
@ExposedType(name = "xrange", base = PyObject.class, isBaseType = false, doc = BuiltinDocs.xrange_doc)
public class PyXRange extends PySequence {
    ...
    @ExposedNew
    static final PyObject xrange___new__(PyNewWrapper new_, boolean init,
            PyType subtype, PyObject[] args, String[] keywords) {
        ArgParser ap = new ArgParser("xrange", args, keywords, new String[] {
                "ilow", "ihigh", "istep" }, 1);
        ap.noKeywords();
    ...
    /**
     * Return number of items in range/xrange (lo, hi, step). step > 0 required.
     * Return a value < 0 if & only if the true value is too large to fit in a
     * Java int.
     * 
     * @param lo
     *            int value
    ...
    protected String unsupportedopMessage(String op, PyObject o2) {
        // always return the default unsupported messages instead of
        // PySequence's
        return null;
    }
