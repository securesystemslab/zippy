...
public class PyInteger extends PyObject {
    ...
    public static final BigInteger MIN_INT = BigInteger
            .valueOf(Integer.MIN_VALUE);

    /** The maximum value of an int represented by a BigInteger */
    public static final BigInteger MAX_INT = BigInteger
    ...
    public Object __tojava__(Class<?> c) {
        if (c == Integer.TYPE || c == Number.class || c == Object.class
                || c == Integer.class || c == Serializable.class) {
    ...
    /**
     * Coercion logic for int. Implemented as a final method to avoid invocation
     * of virtual methods from the exposed coerced.
     */
    ...
    private static long divide(long x, long y) {
        ...
        // If the signs of x and y differ, and the remainder is non-0, C89
        // doesn't define
        // whether xdivy is now the floor or the ceiling of the infinitely
        // precise
        // quotient. We want the floor, and we have it iff the remainder's sign
        // matches
        // y's.
    ...
    final PyObject int___divmod__(PyObject right) {
        ...
        return new PyTuple(Py.newInteger(xdivy), Py.newInteger(modulo(v,
                rightv, xdivy)));
    }
    ...
    final PyObject int___rdivmod__(PyObject left) {
        ...
        return new PyTuple(Py.newInteger(xdivy), Py.newInteger(modulo(leftv, v,
                xdivy)));
    }
    ...
    static PyObject formatImpl(Object value, PyObject formatSpec) {
        ...
        try {
            ...
            InternalFormatSpec spec = new InternalFormatSpecParser(specString)
                    .parse();
    ...
    /**
     * Formats an integer or long number according to a PEP-3101 format
     * specification.
     * 
     * @param value
     *            Integer or BigInteger object specifying the value to format.
     * @param spec
     *            parsed PEP-3101 format specification.
    ...
    public static String formatIntOrLong(Object value, InternalFormatSpec spec) {
        ...
        if (spec.type == 'c') {
            if (spec.sign != '\0') {
                throw new IllegalArgumentException(
                        "Sign not allowed with integer format "
                                + "specifier 'c'");
            }
            if (value instanceof Integer) {
                int intValue = (Integer) value;
