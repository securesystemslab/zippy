...
public class ContextGuard implements ContextManager {
     ...
     * 
     * @contextmanager def my_manager(): print "setup" try: yield finally: print
     * "done"
     */
            ...
            if (code instanceof PyBaseCode) {
                ...
                if (pyCode.co_flags.isFlagSet(CodeFlag.CO_GENERATOR)) {
                    return new PyFunction(function.__globals__,
                            function.func_defaults, new ContextCode(pyCode),
                            function.__doc__,
                            (function.func_closure == null) ? null
                                    : ((PyTuple) function.func_closure)
                                            .getArray());
                }
            }
        }
    ...
    private static class ContextCode extends PyBaseCode {
        ...
        ContextCode(PyBaseCode code) {
            ...
            for (CodeFlag flag : CodeFlag.values()) {
                if (code.co_flags.isFlagSet(flag)
                        && flag != CodeFlag.CO_GENERATOR) {
        ...
        @SuppressWarnings("serial")
        @Override
        public PyObject call(ThreadState ts, PyFrame frame,
                final PyObject closure) {
            frame.f_back = null;
            return new GeneratorContextManager(frame) {
                @Override
                PyObject body(ThreadState ts) {
    ...
    @SuppressWarnings("serial")
    private static abstract class GeneratorContextManager extends PyObject
