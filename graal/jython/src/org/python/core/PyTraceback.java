...
public class PyTraceback extends PyObject {
    ...
    private String tracebackInfo() {
        ...
        return String.format("  File \"%.500s\", line %d, in %.500s\n%s",
                tb_frame.f_code.co_filename, tb_lineno,
                tb_frame.f_code.co_name, line == null ? "" : "    " + line);
    }
    ...
    private String getLine(String filename, int lineno) {
        ...
        try {
            if (!file.isFile() || !file.canRead()) {
                // XXX: We should run through sys.path until the filename is
                // found
                return null;
            }
        } catch (SecurityException e) {
            return null; // If we don't have read access to the file, return
                         // null
        }
