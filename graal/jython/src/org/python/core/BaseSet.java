...
public abstract class BaseSet extends PyObject implements Set {
    ...
    final PyObject baseset_difference(PyObject[] args) {
        ...
        for (PyObject item : args) {
            BaseSet bs = args[0] instanceof BaseSet ? (BaseSet) item
                    : new PySet(item);
    ...
    final PyObject baseset_symmetric_difference(PyObject other) {
        BaseSet bs = other instanceof BaseSet ? (BaseSet) other : new PySet(
                other);
    ...
    final PyObject baseset___lt__(PyObject other) {
        ...
        return Py.newBoolean(size() < bs.size()
    ...
    final PyObject baseset_intersection(PyObject other) {
        ...
        PyObject common = __builtin__.filter(big.__getattr__("__contains__"),
                little);
    ...
    /**
     * Casts other as BaseSet, throwing a TypeError tailored for the rich
     * comparison methods when not applicable.
     * 
     * @param other
     *            a PyObject
     * @return a BaseSet
     ...
     * BaseSet and pye is a TypeError.
     * 
     * WARNING: The PyFrozenSet returned is only intended to be used temporarily
     * (and internally); since its contents are shared with value, it could be
     * mutated!
     * 
     * This is better than special-casing behavior based on isinstance, because
     * a Python subclass can override, say, __hash__ and all of a sudden you
     * can't assume that a non-PyFrozenSet is unhashable anymore.
     * 
     * @param pye
