...
public abstract class PyBaseCode extends PyCode {
    ...
    public PyObject call(ThreadState ts, PyFrame frame, PyObject closure) {
        ...
        try {
             ...
             */
            long start = 0;
            if (co_name.equals("<module>")) {
                start = Options.startTimer(Options.timeInterpret);
            }
            ...
            if (co_name.equals("<module>")) {
                Options.stopTimer(Options.timeInterpret, start,
                        Options.TIMER.INTERPRET);
            }
        ...
        // on Jython
        if (ts.systemState._systemRestart
    ...
    public PyObject call(ThreadState state, PyObject globals,
        ...
        if (co_argcount != 0 || varargs || varkwargs)
            return call(state, Py.EmptyObjects, Py.NoKeywords, globals,
                    defaults, closure);
    ...
    public PyObject call(ThreadState state, PyObject arg1, PyObject globals,
        ...
        if (co_argcount != 1 || varargs || varkwargs)
            return call(state, new PyObject[] { arg1 }, Py.NoKeywords, globals,
                    defaults, closure);
        PyFrame frame = new PyFrame(this, globals);
    ...
    public PyObject call(ThreadState state, PyObject arg1, PyObject arg2,
        ...
        if (co_argcount != 2 || varargs || varkwargs)
            return call(state, new PyObject[] { arg1, arg2 }, Py.NoKeywords,
                    globals, defaults, closure);
        PyFrame frame = new PyFrame(this, globals);
        frame.f_fastlocals[0] = arg1;
    ...
    public PyObject call(ThreadState state, PyObject args[], String kws[],
            PyObject globals, PyObject[] defs, PyObject closure) {
        PyFrame frame = new PyFrame(this, globals);
        ...
        if (co_argcount > 0 || (varargs || varkwargs)) {
            ...
            if (argcount > co_argcount) {
                if (!varargs) {
                    ...
                    String msg = String.format(
                            ...
                            co_name, defcount > 0 ? "at most" : "exactly",
                            co_argcount, kws.length > 0 ? "non-keyword " : "",
                            co_argcount == 1 ? "" : "s", argcount);
            ...
            for (i = 0; i < kws.length; i++) {
                ...
                // {
                // throw
                // Py.TypeError(String.format("%.200s() keywords must be strings",
                // co_name));
                // }
                ...
                if (j >= co_argcount) {
                    if (kwdict == null) {
                        throw Py.TypeError(String
                                .format("%.200s() got an unexpected keyword "
                                        + "argument '%.400s'", co_name, keyword));
                    }
                    kwdict.__setitem__(keyword, value);
                } else {
                    if (fastlocals[j] != null) {
                        throw Py.TypeError(String.format(
                                "%.200s() got multiple values for "
                                        + "keyword argument '%.400s'", co_name,
                                keyword));
                    }
                    fastlocals[j] = value;
                }
            ...
            if (argcount < co_argcount) {
                ...
                for (i = argcount; i < m; i++) {
                    if (fastlocals[i] == null) {
                        String msg = String.format(
                                "%.200s() takes %s %d %sargument%s (%d given)",
                                co_name, (varargs || defcount > 0) ? "at least"
                                        : "exactly", m,
            ...
            throw Py.TypeError(String
                    .format("%.200s() takes no arguments (%d given)", co_name,
                            argcount));
        }
    ...
    public String toString() {
        return String.format(
                "<code object %.100s at %s, file \"%.300s\", line %d>",
