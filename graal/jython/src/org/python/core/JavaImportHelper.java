...
public class JavaImportHelper {
     ...
     * <p>
     * This is handy in cases where the package scan cannot run, or when the
     * initial classpath does not contain all .jar files (such as in J2EE
     * containers).
     * <p>
     * There is some self-healing in the sense that a correct, explicit import
     * of a java class will succeed even if sys.modules already contains a
     * Py.None entry for the corresponding java package.
     * 
     * @param packageName
     *            The dotted name of the java package
     * @param fromlist
     *            A tuple with the from names to import. Can be null or empty.
     * 
     * @return <code>true</code> if a java package was doubtlessly identified
     *         and added, <code>false</code> otherwise.
     */
    protected static boolean tryAddPackage(final String packageName,
            PyObject fromlist) {
        ...
        if (packageName != null) {
            ...
            if (!packageAdded) {
                ...
                do {
                    ...
                    if (dotPos > 0) {
                        parentPackageName = parentPackageName.substring(0,
                                dotPos);
                        if (isLoadedPackage(parentPackageName, packages)) {
                            packageAdded = addPackage(parentPackageName,
                                    packageAdded);
                        }
                    }
                } while (dotPos > 0);
     ...
     * <p>
     * May return <code>false</code> even if the given package name is a valid
     * java package !
     * 
     * @param packageName
     * 
     * @return <code>true</code> if the package with the given name is already
     *         loaded by the VM, <code>false</code> otherwise.
     */
    ...
    private static final List<String> getFromListAsStrings(PyObject fromlist) {
        ...
        if (fromlist != null && fromlist != Py.EmptyTuple
                && fromlist instanceof PyTuple) {
     ...
     * @param packageName
     * @param packages
     *            A Map containing all packages actually known to the VM. Such a
     *            Map can be obtained using {@link
     *            JavaImportHelper.buildLoadedPackagesTree()}
     * 
     * @return <code>true</code> if the package with the given name is already
     *         loaded by the VM, <code>false</code> otherwise.
     */
    private static boolean isLoadedPackage(String javaPackageName,
     ...
     * <code>java</code>, <code>java.lang</code>, <code>java.lang.reflect</code>,
     */
     ...
     * 
     * @return <code>true</code> if something was really added,
    ...
    private static boolean addPackage(String packageName, boolean packageAdded) {
        ...
        // a previously failed import could have created a Py.None entry in
        // sys.modules
