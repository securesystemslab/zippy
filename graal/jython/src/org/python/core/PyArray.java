...
public class PyArray extends PySequence implements Cloneable {
    ...
    @ExposedNew
    static final PyObject array_new(PyNewWrapper new_, boolean init,
        ...
        // Initialize the typecode (and validate type) before creating the
        // backing Array
        class2char(type);
        ...
        } else if (initial instanceof PyString
                && !(initial instanceof PyUnicode)) {
    ...
    /**
     * Create a PyArray storing <em>ctype</em> types and being initialised with
     * <em>initialiser</em>.
     * 
     * @param init
    ...
    final PyObject array___getslice__(PyObject start, PyObject stop,
            PyObject step) {
        return seq___getslice__(start, stop, step);
    }

    @ExposedMethod(defaults = "null")
    final void array___setslice__(PyObject start, PyObject stop, PyObject step,
            PyObject value) {
    ...
    public Object __tojava__(Class<?> c) {
        if (c == Object.class
            ...
            if (delegate.capacity != delegate.size) {
                // when unboxing, need to shrink the array first, otherwise
                // incorrect
                // results to Java
    ...
    /**
     * Delete the slice defined by <em>start</em> to <em>stop</em> from the
     * array.
     * 
     * @param start
     ...
     * Default behaviour is to defer to {@link #extendInternalIter(PyObject)
     * extendInternalIter }
     * 
     * @param iterable
     ...
     * that were available are still inserted into the array. <em>f</em> must be
     * a real built-in file object; something else with a read() method won't
     * do.
     * 
     * @param f
    ...
    /**
     * Getter for the type code of the array. {@link #char2class(char)
     * char2class} describes the possible type codes and their meaning.
     * 
    ...
    /**
     * Removes the item with the index <em>index</em> from the array and returns
     * it. The optional argument defaults to -1, so that by default the last
        ...
        if (type == Byte.TYPE) {
            ...
            if (isSigned() && value instanceof PyInteger) {
                if (((PyInteger) value).getValue() < 0) {
                    throw Py.OverflowError("value too small for "
                            + type.getName());
                }
    ...
    private boolean isSigned() {
        return typecode.length() == 1
                && typecode.equals(typecode.toUpperCase());
    }
            ...
            if (value instanceof PyString && type == Byte.TYPE) {
                ...
                if (step == 1) {
                    ...
                    try {
                        ...
                        throw Py.TypeError("Slice typecode '"
                                + array.typecode
                ...
                throw Py.TypeError(String.format(
                        "can only assign array (not \"%.200s\") to array "
