...
public class PySystemState extends PyObject implements ClassDictInit {
    ...
    public static final PyTuple subversion = new PyTuple(
            new PyString("Jython"), Py.newString(""), Py.newString(""));

    public static final int hexversion = ((Version.PY_MAJOR_VERSION << 24)
            | (Version.PY_MINOR_VERSION << 16)
            | (Version.PY_MICRO_VERSION << 8) | (Version.PY_RELEASE_LEVEL << 4) | (Version.PY_RELEASE_SERIAL << 0));
    ...
    public static final PyObject copyright = Py
            .newString("Copyright (c) 2000-2009 Jython Developers.\n"
                    + "All rights reserved.\n\n" +
    ...
    // XXX - from Jython code, these statics are immutable; we may wish to
    // consider
    ...
    // Automatically close resources associated with a PySystemState when they
    // get GCed
    ...
    public PySystemState() {
        ...
        stdin = __stdin__ = new PyFile(System.in, "<stdin>", "r" + mode,
                buffering, false);
        stdout = __stdout__ = new PyFile(System.out, "<stdout>", "w" + mode,
                buffering, false);
        stderr = __stderr__ = new PyFile(System.err, "<stderr>", "w" + mode, 0,
                false);
        initEncoding();
        ...
        modules.__setitem__("__builtin__", new PyModule("__builtin__",
                getDefaultBuiltins()));
    ...
    private static void checkMustExist(String name) {
        if (name == "__dict__" || name == "__class__" || name == "registry"
                || name == "exec_prefix" || name == "platform"
                || name == "packageManager" || name == "builtins"
                || name == "warnoptions") {
            throw Py.TypeError("readonly attribute");
        }
    }
    ...
    private void initEncoding() {
        ...
        for (PyFile stdStream : new PyFile[] { (PyFile) this.stdin,
                (PyFile) this.stdout, (PyFile) this.stderr }) {
            if (stdStream.isatty()) {
    ...
    /**
     * Resolve a path. Returns the full path taking the current working
     ...
     * consulted for the current working directory when it's necessary (when the
     * path is relative).
     * 
     * @param path
     *            a path String
    ...
    private static String getPath(PySystemState sys, String path) {
        ...
        // Python considers r'\Jython25' and '/Jython25' abspaths on Windows,
        // unlike
        // java.io.File
        if (!file.isAbsolute()
                && (!Platform.IS_WINDOWS || !(path.startsWith("\\") || path
                        .startsWith("/")))) {
            if (sys == null) {
        ...
        // This needs to be performed always to trim trailing backslashes on
        // Windows
        return file.getPath();
    }
    ...
    private static String findRoot(Properties preProperties,
            Properties postProperties, String jarFileName) {
        String root = null;
        try {
        ...
        if (root == null) {
            ...
            if (classpath != null) {
                ...
                if (jarIndex >= 0) {
                    ...
                    // in case JYTHON_JAR is referenced from a MANIFEST inside
                    // another jar on the
                    // classpath
    ...
    private static void initRegistry(Properties preProperties,
            Properties postProperties, boolean standalone, String jarFileName) {
        if (registry != null) {
    ...
    private static void addRegistryFile(File file) {
        if (file.exists()) {
            if (!file.isDirectory()) {
                ...
                System.err.println("warning: " + file.toString()
    ...
    public static synchronized void initialize(Properties preProperties,
        ...
        initialize(preProperties, postProperties, argv, classLoader,
                new ClassicPyObjectAdapter());
    }
    ...
    public static synchronized void initialize(Properties preProperties,
        ...
        try {
            ...
            if (context != null) {
                if (initialize(preProperties, postProperties, argv,
                        classLoader, adapter, context)) {
                    return;
                }
            } else {
                Py.writeDebug("initializer",
                        "Context class loader null, skipping");
            }
            ClassLoader sysStateLoader = PySystemState.class.getClassLoader();
            if (sysStateLoader != null) {
                if (initialize(preProperties, postProperties, argv,
                        classLoader, adapter, sysStateLoader)) {
                    return;
                }
            } else {
                Py.writeDebug("initializer",
                        "PySystemState.class class loader null, skipping");
            }
            ...
            // Must be running in a security environment that doesn't allow
            // access to the class
            // loader
        } catch (Exception e) {
            Py.writeWarning("initializer",
    ...
    /**
     * Attempts to read a SystemStateInitializer service from the given
     * classloader, instantiate it, and initialize with it.
     * 
     * @throws UnsupportedCharsetException
     *             if unable to load UTF-8 to read a service definition
     * @return true if a service is found and successfully initializes.
     */
    private static boolean initialize(Properties pre, Properties post,
            String[] argv, ClassLoader sysClassLoader,
            ExtensiblePyObjectAdapter adapter,
            ClassLoader initializerClassLoader) {
        InputStream in = initializerClassLoader
                .getResourceAsStream(INITIALIZER_SERVICE);
        if (in == null) {
            Py.writeDebug("initializer", "'" + INITIALIZER_SERVICE
                    + "' not found on " + initializerClassLoader);
            return false;
        }
        ...
        try {
            ...
            Py.writeWarning("initializer", "Failed reading '"
                    + INITIALIZER_SERVICE + "' from " + initializerClassLoader);
            e.printStackTrace(System.err);
            return false;
        }
        Class<?> initializer;
        try {
            ...
            Py.writeWarning("initializer", "Specified initializer class '"
                    + className + "' not found, continuing");
            return false;
        }
        try {
            ...
            Py.writeWarning("initializer", "Failed initializing with class '"
    ...
    private static void initStaticFields() {
        ...
        _mercurial = new PyTuple(Py.newString("Jython"), Py.newString(Version
                .getHGIdentifier()), Py.newString(Version.getHGVersion()));
    ...
    private static void initCacheDirectory(Properties props) {
        ...
        cachedir = new File(props.getProperty(PYTHON_CACHEDIR,
                CACHEDIR_DEFAULT_NAME));
        if (!cachedir.isAbsolute()) {
            cachedir = new File(prefix == null ? null : prefix.toString(),
                    cachedir.getPath());
        }
    }
    ...
    /**
     * Determine the default sys.executable value from the registry. Returns
     * Py.None is no executable can be found.
     * 
     * @param props
    ...
    private static PyList initPath(Properties props, boolean standalone,
            String jarFileName) {
        ...
        if (standalone) {
            // standalone jython: add the /Lib directory inside JYTHON_JAR to
            // the path
    ...
    protected static String getJarFileNameFromURL(URL url) {
        ...
        if (url != null) {
            try {
                // escape plus signs, since the URLDecoder would turn them into
                // spaces
                final String plus = "\\+";
    ...
    /**
     * Add a classpath directory to the list of places that are searched for
     * java packages.
     * <p>
     * <b>Note</b>. Classes found in directory and subdirectory are not made
     * available to jython by this call. It only makes the java package found in
     * the directory available. This call is mostly usefull if jython is
     * embedded in an application that deals with its own classloaders. A
     * servlet container is a very good example. Calling
     * add_classdir("<context>/WEB-INF/classes") makes the java packages in
     * WEB-INF classes available to jython import. However the actual
    ...
    /**
     * Add a .jar & .zip directory to the list of places that are searched for
     * java .jar and .zip files. The .jar and .zip files found will not be
     * cached.
     * <p>
     * <b>Note</b>. Classes in .jar and .zip files found in the directory are
     * not made available to jython by this call. See the note for
     ...
     *            The name of a directory.
     * @param cache
     *            Controls if the packages in the zip and jar file should be
     *            cached.
     * 
     * @see #add_classdir
     */
     ...
     * @exception Py.SystemExit
     *                always throws this exception. When caught at top level the
     *                program will exit.
     */
    ...
    public static PyTuple exc_info() {
        ...
        return new PyTuple(exc.type, value == null ? Py.None : value,
                tb == null ? Py.None : tb);
    }
    ...
    private static class PySystemStateCloser {
        ...
        private PySystemStateCloser(PySystemState sys) {
            ...
            WeakReference<PySystemState> ref = new WeakReference<PySystemState>(
                    sys, systemStateQueue);
        ...
        private synchronized void cleanup() {
            ...
            // close this thread so we can unload any associated classloaders in
            // cycle
            // with this instance
            if (shutdownHook != null) {
                try {
                    ...
                    // JVM is already shutting down, so we cannot remove this
        ...
        // Python scripts expect that files are closed upon an orderly cleanup
        // of the VM.
        private Thread initShutdownCloser() {
            try {
                ...
                Py.writeDebug("PySystemState",
                        "Can't register cleanup closer hook");
                return null;
            }
        }
        ...
        private class ShutdownCloser extends Thread {
            ...
            public synchronized void run() {
                ...
                for (Callable<Void> callable : resourceClosers) {
                    try {
                        callable.call(); // side effect of being removed from
                                         // this set
                    } catch (Exception e) {
 ...
 * deleted. Used only in PySystemState for now.
 */
...
class FloatInfo extends PyTuple {
    ...
    static public FloatInfo getInfo() {
        ...
        return new FloatInfo(Py.newFloat(Double.MAX_VALUE), // DBL_MAX
