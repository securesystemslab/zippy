...
public class StreamIO extends RawIOBase {
     ...
     * @param closefd
     *            boolean whether the underlying file is closed on close()
     *            (defaults to True)
     */
    ...
    private static FileDescriptor getInputFileDescriptor(InputStream stream)
            throws IOException {
        if (stream == null) {
            return null;
        }
    ...
    private static FileDescriptor getOutputFileDescriptor(OutputStream stream)
            throws IOException {
        if (stream == null) {
            return null;
        }
        ...
        if (stream instanceof FilterOutputStream) {
            ...
            try {
                ...
                return getOutputFileDescriptor((OutputStream) outField
                        .get(stream));
            } catch (Exception e) {
                // XXX: masking other exceptions
            } finally {
