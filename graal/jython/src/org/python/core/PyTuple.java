...
public class PyTuple extends PySequenceList implements List {
    ...
    @ExposedNew
    final static PyObject tuple_new(PyNewWrapper new_, boolean init,
            PyType subtype, PyObject[] args, String[] keywords) {
        ArgParser ap = new ArgParser("tuple", args, keywords,
    ...
    protected PyObject repeat(int count) {
        ...
        if (size == 0 || count == 1) {
            if (getType() == TYPE) {
                // Since tuples are immutable, we can return a shared copy in
                // this case
                return this;
            }
            if (size == 0) {
    ...
    final PyObject tuple___add__(PyObject generic_other) {
        ...
        if (generic_other instanceof PyTuple) {
            ...
            PyObject[] newArray = new PyObject[array.length
                    + other.array.length];
            System.arraycopy(array, 0, newArray, 0, array.length);
            System.arraycopy(other.array, 0, newArray, array.length,
                    other.array.length);
    ...
    public ListIterator listIterator(final int index) {
        return new ListIterator() {
            ...
            private final ListIterator<PyObject> iter = getList().listIterator(
                    index);
