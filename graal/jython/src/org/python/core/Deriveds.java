...
public class Deriveds {
    ...
    private static final PyObject objectGetattribute = PyObject.TYPE
            .__findattr__("__getattribute__");

    public static void dispatch__init__(PyObject self, PyObject[] args,
            String[] keywords) {
        ...
        if (result != Py.None) {
            throw Py.TypeError(String.format(
                    "__init__() should return None, not '%.200s'", result
                            .getType().fastGetName()));
        }
        self.proxyInit();
    }
     ...
     * 
     * This resides here (in org.python.core) because it manipulates PyType, and
     * doesn't call any of the Derived classes' superclass methods.
     */
    public static PyObject __findattr_ex__(PyObject self, String name) {
        ...
        try {
            if (type.getUsesObjectGetattribute()) {
                // Fast path: don't bother calling through the descriptor if
                // using the
                ...
                if (getattribute == null) {
                    ...
                    throw Py.SystemError(String.format(
                            "__getattribute__ not found on type %s",
                            type.getName()));
                }
            ...
            if (!pye.match(Py.AttributeError)) {
                ...
                // saved to avoid swallowing custom AttributeErrors, and pass
                // through to
                // __getattr__
        ...
        if (getattr != null) {
            return getattr.__get__(self, type).__call__(
                    pyName != null ? pyName : PyString.fromInterned(name));
        }
