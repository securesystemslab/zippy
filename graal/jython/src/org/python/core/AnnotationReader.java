 ...
 * class annotation for APIVersion, which can then be retrieved by a call to
 * getVersion().
 * 
 * Hopefully the use of ClassReader in this implementation is not too expensive.
 * I suspect it is not since EmptyVisitor is just a bag of empty methods so
 * shouldn't cost too much. If it turns out to cost too much, we will want to
 * implement a special purpose ClassReader that only reads out the APIVersion
 * annotation I think.
 */
    ...
    public AnnotationReader(byte[] data) throws IOException {
        ...
        try {
            ...
            IOException ioe = new IOException(
                    "Malformed bytecode: not enough data");
            ioe.initCause(e);// IOException didn't grow a constructor that could
                             // take a cause till
