...
public class PyBytecode extends PyBaseCode {
    ...
    // for debugging
    private int count = 0; // total number of opcodes run so far in this code
                           // obj
    private int maxCount = -1; // if -1, no cap on number of opcodes than can be
                               // run
    ...
    protected static synchronized PyObject get_opname() {
        if (opname == null) {
    ...
    public PyBytecode(int argcount, int nlocals, int stacksize, int flags,
            String codestring, PyObject[] constants, String[] names,
            String varnames[], String filename, String name, int firstlineno,
            String lnotab) {
        this(argcount, nlocals, stacksize, flags, codestring, constants, names,
                varnames, filename, name, firstlineno, lnotab, null, null);
    }
    ...
    public PyBytecode(int argcount, int nlocals, int stacksize, int flags,
            String codestring, PyObject[] constants, String[] names,
            String varnames[], String filename, String name, int firstlineno,
            String lnotab, String[] cellvars, String[] freevars) {
        debug = defaultDebug;
    ...
    protected PyBytecode(PyBytecode pyBytecode, boolean debug) {
        this.debug = debug;

        co_argcount = nargs = pyBytecode.co_argcount;
        co_varnames = pyBytecode.co_varnames;
        co_nlocals = pyBytecode.co_nlocals; // maybe assert = varnames.length;
        co_filename = pyBytecode.co_filename;
        co_firstlineno = pyBytecode.co_firstlineno;
        co_cellvars = pyBytecode.co_cellvars;
        co_freevars = pyBytecode.co_freevars;
        co_name = pyBytecode.co_name;
        co_flags = pyBytecode.co_flags;
        varargs = pyBytecode.varargs;
        varkwargs = pyBytecode.varkwargs;

        co_stacksize = pyBytecode.co_stacksize;
        co_consts = pyBytecode.co_consts;
        co_names = pyBytecode.co_names;
        co_code = pyBytecode.co_code;
        co_lnotab = pyBytecode.co_lnotab;
    }

    /*
     * ModularVM
     */
    PyBytecode(boolean debug) {
        co_stacksize = 0;
        co_names = null;
        co_lnotab = null;
        co_consts = null;
    ...
    private static final String[] __members__ = { "co_name", "co_argcount",
            "co_varnames", "co_filename", "co_firstlineno", "co_flags",
            "co_cellvars", "co_freevars", "co_nlocals", "co_code", "co_consts",
            "co_names", "co_lnotab", "co_stacksize" };
    ...
    protected static class PyStackWhy extends PyObject {
    ...
    protected static class PyStackException extends PyObject {
        ...
        @Override
        public String toString() {
            return String.format("PyStackException<%s,%s,%.100s>",
    ...
    private void print_debug(int count, int next_instr, int line, int opcode, int oparg, PyStack stack, PyFrame f) {
        if (debug) {
            System.err.println(next_instr + " " + co_name + " " + line + ":" + count + ","
                    + f.f_lasti + "> "
                    + get_opname().__getitem__(Py.newInteger(opcode))
                    + (opcode >= Opcode.HAVE_ARGUMENT ? " " + oparg : "")
                    + ", stack: " + stack.toString());
//                    + ", stack: " + stack.toString() + ", blocks: "
//                    + stringify_blocks(f));
        }
    }

    /*protected static PyBlock popBlock(PyFrame f) {
        return f.f_blocks.pop();
    }

    protected static void pushBlock(PyFrame f, PyBlock block) {
        if (f.f_blocks == null) {
            f.f_blocks = new Stack<PyBlock>();
        }

        f.f_blocks.push(block);
    }

    protected boolean blocksLeft(PyFrame f) {
        if (f.f_blocks != null) {
            return !(f.f_blocks.isEmpty());
        } else {
            return false;
        }
    }*/
    ...
    protected static PyBlock popBlock(PyFrame f) {
        return f.f_blocks.remove(f.f_blocks.size() - 1);
    }

    protected static void pushBlock(PyFrame f, PyBlock block) {
        if (f.f_blocks == null) {
            f.f_blocks = new ArrayList<PyBlock>();
        }
        
        f.f_blocks.add(block);
    }
    ...
    @Override
    public PyObject interpret(PyFrame f, ThreadState ts) {
        ...
        // in a shadow version of the frame that we copy back to on entry/exit
        // and downcalls

        if (debug) {
            System.err.println(co_name + ":" + f.f_lasti + "/" + co_code.length
                    + ", cells:" + Arrays.toString(co_cellvars) + ", free:"
                    + Arrays.toString(co_freevars));
            int i = 0;
        ...
        while (!debug || (maxCount == -1 || count < maxCount)) { // XXX -
                                                                 // replace with
                                                                 // while(true)
            if (f.tracefunc != null || debug) {
                if (lineCache == null) {
                    ...
                    if (debug) {
                        System.err
                                .println("LineCache: " + lineCache.toString());
                    }
                }
                line = lineCache.getline(next_instr); // XXX - should also
                                                      // return the range this
                                                      // is valid to avoid an
                                                      // unnecessary bisect
                if (line != last_line) {
                    f.setline(line);
                }
            ...
            try {
                ...
                if (opcode >= Opcode.HAVE_ARGUMENT) {
                    next_instr += 2;
                    oparg = (getUnsigned(co_code, next_instr) << 8)
                 ...
                 * maxine stuff
                 */
                //bytecode histogram stuff
//                Opcode.countOpcode(opcode);
                ...
                case Opcode.SLICE + 2:
                case Opcode.SLICE + 3: {
                    PyObject stop = (((opcode - Opcode.SLICE) & 2) != 0) ? stack
                            .pop() : null;
                    PyObject start = (((opcode - Opcode.SLICE) & 1) != 0) ? stack
                            .pop() : null;
                ...
                case Opcode.BUILD_CLASS: {
                    ...
                    PyObject bases[] = ((PySequenceList) (stack.pop()))
                            .getArray();
                ...
                case Opcode.LOAD_CLOSURE: {
                    ...
                    if (cell.ob_ref == null) {
                        ...
                        // XXX - consider some efficient lookup mechanism, like
                        // a hash :),
                        // at least if co_varnames is much greater than say a
                        // certain
                        // size (but i would think, it's not going to happen in
                        // real code. profile?)
                ...
                case Opcode.COMPARE_OP: {
                    ...
                    case Opcode.PyCmp_EXC_MATCH:
                        if (a instanceof PyStackException) {
                            ...
                            stack.push(Py.newBoolean(new PyException(a)
                                    .match(b)));
                        }
                        break;
                ...
                case Opcode.IMPORT_NAME: {
                    PyObject __import__ = f.f_builtins
                            .__finditem__("__import__");
                    if (__import__ == null) {
                ...
                case Opcode.SETUP_FINALLY:
                    pushBlock(f, new PyBlock(opcode, next_instr + oparg, stack.size()));
                    break;

                case Opcode.WITH_CLEANUP: {
                    /*
                     * TOP is the context.__exit__ bound method. Below that are
                     * 1-3 values indicating how/why we entered the finally
                     * clause: - SECOND = None - (SECOND, THIRD) =
                     * (WHY_{RETURN,CONTINUE}), retval - SECOND = WHY_*; no
                     * retval below it - (SECOND, THIRD, FOURTH) = exc_info() In
                     * the last case, we must call TOP(SECOND, THIRD, FOURTH)
                     * otherwise we must call TOP(None, None, None)
                     *
                     * In addition, if the stack represents an exception,and*
                     * the function call returns a 'true' value, we "zap" this
                     * information, to prevent END_FINALLY from re-raising the
                ...
                case Opcode.CALL_FUNCTION_VAR_KW: {
                    ...
                    call_function(na, nk, (flags & CALL_FLAG_VAR) != 0,
                            (flags & CALL_FLAG_KW) != 0, stack);
                    break;
                }
                ...
                case Opcode.MAKE_FUNCTION: {
                    ...
                    PyFunction func = new PyFunction(f.f_globals, defaults,
                            code, doc);
                    stack.push(func);
                    break;
                }
                ...
                case Opcode.EXTENDED_ARG:
                    ...
                    next_instr += 2;
                    oparg = oparg << 16
                            | ((getUnsigned(co_code, next_instr) << 8) + getUnsigned(
                                    co_code, next_instr - 1));
                    break;

                default:
                    Py.print(Py.getSystemState().stderr, Py.newString(String
                            .format("XXX lineno: %d, opcode: %d\n", f.f_lasti,
                                    opcode)));
            ...
            // do some trace handling here, but for now just convert to
            // EXCEPTION
            if (why == Why.RERAISE) {
                why = Why.EXCEPTION;
            }
            ...
            while (why != Why.NOT && blocksLeft(f)) {
                ...
                if (b.b_type == Opcode.SETUP_FINALLY
                        || (b.b_type == Opcode.SETUP_EXCEPT && why == Why.EXCEPTION)) {
                    if (why == Why.EXCEPTION) {
                        ...
                        stack.push(new PyStackException(exc)); // instead of
                                                               // stack.push(exc.type),
                                                               // like CPython
                    } else {
                        if (why == Why.RETURN || why == Why.CONTINUE) {
        ...
        if (debug) {
            System.err.println(count + "," + f.f_lasti + "> Returning from "
                    + why + ": " + retval + ", stack: " + stack.toString()
        ...
        if (co_flags.isFlagSet(CodeFlag.CO_GENERATOR) && why == Why.RETURN
                && retval == Py.None) {
            f.f_lasti = -1;
        }
    ...
    protected static void call_function(int na, PyStack stack) {
        switch (na) {
        case 0: {
    ...
    protected static void call_function(int na, int nk, PyStack stack) {
        int n = na + nk * 2;
    ...
    protected static void call_function(int na, int nk, boolean var,
            boolean kw, PyStack stack) {
        int n = na + nk * 2;
    ...
    protected static void unpack_iterable(int oparg, PyStack stack) {
        PyObject v = stack.pop();
        int i = oparg;
    ...
    static class PyStack {

        final PyObject[] stack;
        int top = -1;

        PyStack() {
            stack = new PyObject[0];
        }
...
//                PyObject item = stack[N - i - 1];
        ...
        static String upto(String x) {
            return upto(x, 100);
        }

        static String upto(String x, int n) {
            x = x.replace('\n', '|');
            if (x.length() > n) {
    ...
    /**
     * ModularVM
     * We use PyBlock instead of PyTryBlock
     * PyTryBlock causes the Null Pointer Exception in Maxine
     * So we temporarily solve the Null Pointer Exception
     */
    protected static class PyBlock { // purely to sit on top
        ...
        PyBlock(int type, int handler, int level) {
            b_type = type;
            b_handler = handler;
            b_level = level;
        }
    ...
    private class LineCache {
        ...
        List<Integer> lines = new ArrayList<Integer>(); // length should be one
                                                        // more than
                                                        // addr_breakpoints
    ...
    // Utility functions to enable storage of unsigned bytes in co_code,
    // co_lnotab byte[] arrays
    static char getUnsigned(byte[] x, int i) {
        byte b = x[i];
        if (b < 0) {
            return (char) (b + 256);
        } else {
