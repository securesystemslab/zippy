...
public class PyClassMethodDescr extends PyMethodDescr {
    ...
    public static final PyType TYPE = PyType
            .fromClass(PyClassMethodDescr.class);
        ...
        if (type == null || type == Py.None) {
            if (obj != null) {
                ...
                throw Py.TypeError(String.format(
                        "descriptor '%s' for type '%s' needs either an "
                                + " object or a type", name,
                        dtype.fastGetName()));
            }
        } else if (!(type instanceof PyType)) {
            throw Py.TypeError(String.format(
                    "descriptor '%s' for type '%s' needs a type, not a"
                            + " '%s' as arg 2", name, dtype.fastGetName(), type
                            .getType().fastGetName()));
        }
