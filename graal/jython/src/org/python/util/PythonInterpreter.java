...
public class PythonInterpreter {
    ...
    /**
     * Initializes the Jython runtime. This should only be called once, before
     * any other Python objects (including PythonInterpreter) are created.
     * 
     * @param preProperties
     ...
     *            be added to this property set. postProperties override system
     *            properties and registry properties.
     * @param argv
     ...
     * namespace for each thread (set by invoking setLocals()).
     * 
     * @param dict
     *            a Python mapping object (e.g., a dictionary) for use as the
     *            default namespace
     */
    ...
    public void exec(String s) {
        setSystemState();
        Py.exec(Py.compile_flags(s, "<string>", CompileMode.exec, cflags),
                getLocals(), null);
        Py.flushLine();
    }
    ...
    public PyCode compile(Reader reader, String filename) {
        mod node = ParserFacade.parseExpressionOrModule(reader, filename,
                cflags);
        setSystemState();
     ...
     * @param value
     *            the object to set the variable to (as converted to an
