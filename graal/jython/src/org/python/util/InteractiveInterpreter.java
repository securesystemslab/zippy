...
public class InteractiveInterpreter extends PythonInterpreter {
     ...
     * (SyntaxError or OverflowError). A syntax traceback will be printed by
     * calling the showsyntaxerror() method.
     * 
     * 2) The input is incomplete, and more input is required; compile_command()
     * returned None. Nothing happens.
     * 
     * 3) The input is complete; compile_command() returned a code object. The
     * code is executed by calling self.runcode() (which also handles run-time
     * exceptions, except for SystemExit).
     * 
     * The return value is 1 in case 2, 0 in the other cases (unless an
     * exception is raised). The return value can be used to decide whether to
     * use sys.ps1 or sys.ps2 to prompt the next line.
     **/
    ...
    public boolean runsource(String source, String filename, CompileMode kind) {
        PyObject code;
        try {
            code = Py.compile_command_flags(source, filename, kind, cflags,
                    true);
     ...
     * traceback. All exceptions are caught except SystemExit, which is
     * reraised.
     * 
     ...
     * and then continue the code hoping that Jython will get control to do the
     * break;
     **/
