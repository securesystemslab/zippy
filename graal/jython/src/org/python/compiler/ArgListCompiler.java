...
public class ArgListCompiler extends Visitor {
    ...
    public void visitArgs(arguments args) throws Exception {
        for (int i = 0; i < args.getInternalArgs().size(); i++) {
            ...
            if (args.getInternalArgs().get(i) instanceof Tuple) {
                ...
                Assign ass = new Assign(args.getInternalArgs().get(i), targets,
                        new Name(args.getInternalArgs().get(i), name,
        ...
        for (int i = 0; i < defaults.size(); i++) {
            if (defaults.get(i) == null)
                throw new ParseException(
                        "non-default argument follows default argument", args
                                .getInternalArgs().get(
                                        args.getInternalArgs().size()
                                                - defaults.size() + i));
        }
    }
    ...
    public Object visitName(Name node) throws Exception {
        ...
        if (node.getInternalCtx() != expr_contextType.Store
                && node.getInternalCtx() != expr_contextType.Param) {
            return null;
        }
