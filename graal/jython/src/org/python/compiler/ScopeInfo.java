...
public class ScopeInfo extends Object implements ScopeConstants {
    ...
    // Resolve the names used in the given scope, and mark any freevars used in
    // the up scope
    public void cook(ScopeInfo up, int distance, CompilationContext ctxt)
            throws Exception {
        if (up == null)
    ...
    private void dynastuff_trouble(boolean inner_free, CompilationContext ctxt)
        ...
        if (unqual_exec && from_import_star) {
            ...
            illegal.append("unqualified exec is not allowed in function '")
                    .append(scope_name).append("'");
        } else {
            illegal.append("import * is not allowed in function '")
                    .append(scope_name).append("'");
        }
        if (inner_free) {
