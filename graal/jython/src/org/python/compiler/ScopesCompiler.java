...
public class ScopesCompiler extends Visitor implements ScopeConstants {
    ...
    public ScopesCompiler(CompilationContext code_compiler,
            Hashtable<PythonTree, ScopeInfo> nodeScopes) {
    ...
    public void endScope() throws Exception {
        ...
        // Go into the stack to find a non class containing scope to use making
        // the closure
        // See PEP 227
        int dist = 1;
    ...
    public void parse(PythonTree node) throws Exception {
        try {
            ...
            throw ParserFacade.fixParseError(null, t,
    ...
    public Object visitGeneratorExp(GeneratorExp node) throws Exception {
        ...
        if (node.getInternalGenerators() != null
                && node.getInternalGenerators().size() > 0) {
        ...
        ac.visitArgs(new arguments(node, args, null, null,
                new ArrayList<expr>()));
                ...
                if (node.getInternalGenerators().get(i) != null) {
                    if (i == 0) {
                        visit(node.getInternalGenerators().get(i)
                                .getInternalTarget());
                        if (node.getInternalGenerators().get(i)
                                .getInternalIfs() != null) {
                            for (expr cond : node.getInternalGenerators()
                                    .get(i).getInternalIfs()) {
                                if (cond != null) {
                                    visit(cond);
                                }
                            }
