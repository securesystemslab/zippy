...
class PyIntegerConstant extends Constant implements ClassConstants, Opcodes {
    ...
    void put(Code c) throws IOException {
        ...
        c.iconst(value);
        c.invokestatic(p(Py.class), "newInteger",
                sig(PyInteger.class, Integer.TYPE));
    ...
    public boolean equals(Object o) {
        if (o instanceof PyFloatConstant) {
            ...
            if (ZERO == value) {
                ...
                return oVal == value
                        && Double.toString(oVal).equals(Double.toString(value));
            }
            return oVal == value;
        }
        return false;
...
class PyComplexConstant extends Constant implements ClassConstants, Opcodes {
    ...
    void put(Code c) throws IOException {
        ...
        c.ldc(new Double(value));
        c.invokestatic(p(Py.class), "newImaginary",
                sig(PyComplex.class, Double.TYPE));
...
class PyStringConstant extends Constant implements ClassConstants, Opcodes {
    ...
    void put(Code c) throws IOException {
        ...
        c.ldc(value);
        c.invokestatic(p(PyString.class), "fromInterned",
                sig(PyString.class, String.class));
...
class PyUnicodeConstant extends Constant implements ClassConstants, Opcodes {
    ...
    void put(Code c) throws IOException {
        ...
        c.ldc(value);
        c.invokestatic(p(PyUnicode.class), "fromInterned",
                sig(PyUnicode.class, String.class));
...
class PyCodeConstant extends Constant implements ClassConstants, Opcodes {
    ...
    PyCodeConstant(mod tree, String name, boolean fast_locals,
            String className, boolean classBody, boolean printResults,
    ...
    // XXX: this can probably go away now that we can probably just copy the
    // list.
    ...
    void put(Code c) throws IOException {
        ...
        c.invokestatic(
                p(Py.class),
                "newCode",
                sig(PyCode.class, Integer.TYPE, String[].class, String.class,
                        String.class, Integer.TYPE, Boolean.TYPE, Boolean.TYPE,
                        PyFunctionTable.class, Integer.TYPE, String[].class,
...
public class Module implements Opcodes, ClassConstants, CompilationContext {
    ...
    PyCodeConstant codeConstant(mod tree, String name, boolean fast_locals,
            String className, boolean classBody, boolean printResults,
            int firstlineno, ScopeInfo scope, CompilerFlags cflags)
            throws Exception {
        return codeConstant(tree, name, fast_locals, className, null,
                classBody, printResults, firstlineno, scope, cflags);
    }

    PyCodeConstant codeConstant(mod tree, String name, boolean fast_locals,
            String className, Str classDoc, boolean classBody,
            boolean printResults, int firstlineno, ScopeInfo scope,
        ...
        compiler.parse(tree, c, fast_locals, className, classDoc, classBody,
                scope, cflags);
        return code;
    }
    ...
    public void addInit() throws IOException {
        Code c = classfile.addMethod("<init>", sig(Void.TYPE, String.class),
                ACC_PUBLIC);
        c.aload(0);
    ...
    public void addMain() throws IOException {
        ...
        c.invokestatic(p(CodeLoader.class),
                CodeLoader.SIMPLE_FACTORY_METHOD_NAME,
                sig(CodeBootstrap.class, PyCode.class));
        c.aload(0);
        c.invokestatic(p(Py.class), "runMain",
                sig(Void.TYPE, CodeBootstrap.class, String[].class));
        c.return_();
    }
    ...
    public void addBootstrap() throws IOException {
        ...
        c.invokestatic(p(PyRunnableBootstrap.class),
    ...
    public void addFunctions() throws IOException {
        Code code = classfile.addMethod(
                "call_function",
                sig(PyObject.class, Integer.TYPE, PyFrame.class,
                        ThreadState.class), ACC_PUBLIC);
        ...
        if (!err) {
            try {
                Py.warning(Py.SyntaxWarning, msg,
                        (sfilename != null) ? sfilename : "?", node.getLine(),
                        null, Py.None);
                return;
            } catch (PyException e) {
    ...
    public static void compile(mod node, OutputStream ostream, String name,
            String filename, boolean linenumbers, boolean printResults,
            CompilerFlags cflags) throws Exception {
        compile(node, ostream, name, filename, linenumbers, printResults,
                cflags, org.python.core.imp.NO_MTIME);
    }

    public static void compile(mod node, OutputStream ostream, String name,
            String filename, boolean linenumbers, boolean printResults,
            CompilerFlags cflags, long mtime) throws Exception {
        ...
        Constant main = module.codeConstant(node, "<module>", false, null,
                false, printResults, 0, module.getScopeInfo(node), cflags);
