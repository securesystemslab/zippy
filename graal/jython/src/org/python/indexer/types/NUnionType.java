...
public class NUnionType extends NType {
     ...
     * 
     * @see http
     *      ://www.cs.kuleuven.ac.be/~dtai/projects/ALP/newsletter/archive_93_96
     *      /net/impl/occur.html for an interesting and highly relevant
     *      discussion.
     */
    ...
    public static NType union(NType u, NType v) {
        ...
        if (wu.isTupleType() && wv.isTupleType()) {
            ...
            if (tu.getElementTypes().size() == tv.getElementTypes().size()) {
                ...
                for (int i = 0; i < tu.getElementTypes().size(); i++) {
                    ret.add(union(tu.getElementTypes().get(i), tv
        ...
        if (wu.isListType() && wv.isListType()) {
            return new NListType(union(wu.asListType().getElementType(), wv
                    .asListType().getElementType()));
        }
        ...
        if (wu.isFuncType() && wv.isFuncType()) {
            return new NFuncType(NUnionType.union(wu.asFuncType()
                    .getReturnType(), wv.asFuncType().getReturnType()));
        }

        // XXX: see comments in NInstanceType
        if (wu.isFuncType() && wv.isClassType()) {
            // NUnknownType.point(wu.asFuncType().getReturnType(), new
            // NInstanceType(wv));
        ...
        if (wu.isClassType() && wv.isFuncType()) {
            // NUnknownType.point(wv.asFuncType().getReturnType(), new
            // NInstanceType(wu));
    ...
    /**
     * Returns the first alternate whose type is not unknown and is not
     * {@link Indexer.idx.builtins.None}.
     * 
     * @return the first non-unknown, non-{@code None} alternate, or
