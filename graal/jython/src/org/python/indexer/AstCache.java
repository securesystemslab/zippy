...
public class AstCache {
    ...
    public static final String CACHE_DIR = Util.getSystemTempDir()
            + "jython/ast_cache/";

    private static final Logger LOG = Logger.getLogger(AstCache.class
            .getCanonicalName());
    ...
    /**
     * Returns the syntax tree for {@code path}. May find and/or create a cached
     * copy in the mem cache or the disk cache.
     * 
     * @param path
    ...
    /**
     * Returns the syntax tree for {@code path} with {@code contents}. Uses the
     * memory cache but not the disk cache. This method exists primarily for
     * unit testing.
     * 
     * @param path
    ...
    private NModule generateAST(mod ast, String path) throws Exception {
        if (ast == null) {
            Indexer.idx
                    .reportFailedAssertion("ANTLR returned NULL for " + path);
            return null;
        }
    ...
    void serialize(NModule ast) throws Exception {
        String path = getCachePath(ast.getMD5(),
                new File(ast.getFile()).getName());
