...
public class NName extends NNode {
    ...
    /**
     * Create a temporary binding and definition for this name. If we later
     * encounter a true definition we'll remove this node from the defs and add
     * it to the refs.
     */
    private NBinding makeTempBinding(Scope s) {
        Scope scope = s.getScopeSymtab();

        NBinding b = scope.put(id, this, new NUnknownType(),
                NBinding.Kind.SCOPE);
    ...
    /**
     * Returns {@code true} if this name node is the {@code attr} child (i.e.
     * the attribute being accessed) of an {@link NAttribute} node.
     */
