...
public class NImport extends NNode {
     ...
     * 
     * @param mt
     *            the module that is actually bound to the imported name. for
     *            {@code import x.y.z as foo}, it is {@code z}, a sub-module of
     *            {@code x.y}. For {@code import x.y.z} it is {@code x}.
     */
    private void importName(Scope s, NAlias a, NModuleType mt) throws Exception {
        if (a.aname != null) {
            if (mt.getFile() != null) {
                ...
                // XXX: seems like the url should be set in loadModule, not
                // here.
                // Can't the moduleTable store url-keyed modules too?
                s.update(a.aname.id, new NUrl(Builtins.LIBRARY_URL
                        + mt.getTable().getPath() + ".html"), mt,
                        NBinding.Kind.SCOPE);
            }
        }
