 ...
 * <code>/usr/lib/python2.4/email</code>
 * 
 * <pre>
 * ant jar &amp;&amp; java -classpath ./dist/jython.jar org.python.indexer.demos.HtmlDemo /usr/lib/python2.4 /usr/lib/python2.4/email
 * </pre>
 * 
 * Fully indexing the Python standard library may require a more complete build
 * to pick up all the dependencies:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * You can alternately use Jython's version of the Python library. The following
 * command will index the whole thing:
 * 
 * <pre>
...
public class HtmlDemo {
    ...
    private void generateHtml() throws Exception {
        ...
        for (String path : indexer.getLoadedFiles()) {
            ...
            File destFile = Util.joinPath(OUTPUT_DIR,
                    path.substring(rootLength));
        ...
        info("wrote " + indexer.getLoadedFiles().size() + " files to "
                + OUTPUT_DIR);
    }
    ...
    private String markup(String path) throws Exception {
        ...
        List<StyleRun> styles = new Styler(indexer, linker).addStyles(path,
                source);
        ...
        return "<html><head title=\""
                ...
                + outline + "</td><td>" + "<pre>" + addLineNumbers(source)
