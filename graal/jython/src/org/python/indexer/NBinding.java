 ...
 * represented as a {@link NUnionType}.
 * <p>
 * 
 * Each qname has a set of one or more definitions, and a set of zero or more
 * references. Definitions and references correspond to code locations.
 * <p>
 */
public class NBinding implements Comparable<Object> {
    ...
    /**
     * Sets the binding's qualified name. This should in general be the same as
     * {@code binding.getType().getTable().getPath()}.
     */
     ...
     * automatically (when appropriate) by adding the binding to a {@link Scope}
     * .
     */
     ...
     * automatically (when appropriate) by adding the binding to a {@link Scope}
     * . If {@code node} is an {@link NUrl}, and this is the binding's only
     * definition, it will be marked as a builtin.
     */
    ...
    /**
     * Returns the first definition, which by convention is treated as the one
     * that introduced the binding.
     */
    ...
    /**
     * Returns the type, first calling {@link NUnknownType#follow} on it. For
     * external consumers of the index, this is usually preferable to calling
     * {@link #getType}.
     */
    ...
    public int compareTo(Object o) {
        return getSignatureNode().start()
                - ((NBinding) o).getSignatureNode().start();
    }
     ...
     * 
     * @return the filename associated with the type (if present) or the first
     *         definition (if present), otherwise a string describing what is
     *         known about the binding's source.
     */
