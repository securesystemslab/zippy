...
public class PyStatement extends PyObject {
    ...
    /**
     * Denotes a PreparedStatement either explicitly created by the user, or
     * from a cursor (due to the presence of bind parameters).
     */
    ...
    public PyString __str__() {
        return Py.newString(__unicode__().encode(codecs.getDefaultEncoding(),
                "replace"));
    }

    @Override
    public String toString() {
        return String.format("<PyStatement object at %s for [%s]",
                Py.idstr(this), __unicode__());
    }
     ...
     * @throws SQLException
     */
    private void prepare(PyCursor cursor, PyObject params, PyObject bindings)
            throws SQLException {
        ...
        final PreparedStatement preparedStatement = (PreparedStatement) statement;
        final Procedure procedure = style == STATEMENT_CALLABLE ? (Procedure) sql
                : null;

        if (style != STATEMENT_CALLABLE) {
            columns = params.__len__();
            // clear the statement so all new bindings take affect only if not a
            // callproc
            // this is because Procedure already registered the OUT parameters
            // and we
            // don't want to lose those
            preparedStatement.clearParameters();
        } else {
            columns = procedure.columns == Py.None ? 0 : procedure.columns
                    .__len__();
        }
            ...
            if (bindings != Py.None) {
                ...
                if (binding != null) {
                    try {
                        ...
                        datahandler.setJDBCObject(preparedStatement, column,
                                param, bindingValue);
                    } catch (PyException e) {
                        throw zxJDBC.makeException(zxJDBC.ProgrammingError,
