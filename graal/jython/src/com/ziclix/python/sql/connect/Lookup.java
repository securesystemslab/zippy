...
/**
 * Establish a connection through a JNDI lookup. The bound object can be either
 * a <code>DataSource</code>, <code>ConnectionPooledDataSource</code>,
 * <code>Connection</code> or a <code>String</code>. If it's a
 * <code>String</code> the value is passed to the DriverManager to obtain a
 * connection, otherwise the <code>Connection</code> is established using the
 * object.
 * 
 * @author brian zimmer
 */
public class Lookup extends PyObject {
    ...
    private static final PyString _doc = new PyString(
    ...
    /**
     * Expects a single PyString argument which is the JNDI name of the bound
     * Connection or DataSource. If any keywords are passed, an attempt is made
     * to match the keyword to a static final Field on javax.naming.Context. If
     * the Field is found, the value of the Field is substituted as the key and
     * the value of the keyword is the value put in the Hashtable environment.
     * If the Field is not found, the key is the keyword with no substitutions.
     */
    @Override
    public PyObject __call__(PyObject[] args, String[] keywords) {
        ...
        if ((jndiName == null) || (jndiName == Py.NoConversion)) {
            throw zxJDBC.makeException(zxJDBC.DatabaseError,
                    "lookup name is null");
        }
        ...
        if (ref == null) {
            throw zxJDBC.makeException(zxJDBC.ProgrammingError, "object ["
                    + jndiName + "] not found in JNDI");
        }
        ...
        try {
            if (ref instanceof String) {
                ...
                connection = ((ConnectionPoolDataSource) ref)
