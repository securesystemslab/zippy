...
/**
 * A database source. Given a PyConnection and information about the query,
 * produce the data.
 * 
 * @author brian zimmer
 */
public class DBSource extends BaseDB implements Source {
     ...
     */
    public DBSource(PyConnection connection, Class dataHandler,
            String tableName, String where, PyObject include, PyObject params) {
    ...
    /**
     * Return the next row in the result set. The first row returned will be
     * column information.
     */
    public PyObject next() {
        ...
        if (this.sentHeader) {
            ...
            // Py.None will be sent when all done, so this will close down the
            // queue
            return this.cursor.fetchone();
        } else {
            this.cursor.execute(Py.newString(this.sql), this.params, Py.None,
                    Py.None);
