...
/**
 * A database consumer. All data transferred will be inserted into the
 * appropriate table.
 * 
 * @author brian zimmer
 */
public class DBSink extends BaseDB implements Sink {
     ...
     * @param dataHandler
     *            a custom DataHandler for the cursor, can be None
     * @param tableName
     ...
     * @param batchsize
     *            the optional batchsize for the inserts
     */
    public DBSink(PyConnection connection, Class dataHandler, String tableName,
            PyObject exclude, PyObject bindings, int batchsize) {
        ...
        if (exclude != Py.None) {
            for (int i = 0; i < exclude.__len__(); i++) {
                PyObject lowered = Py.newString(((PyString) exclude
                        .__getitem__(i)).lower());
    ...
    /**
     * Return true if the key (converted to lowercase) is not found in the
     * exclude list.
     */
    ...
    protected void createSql(PyObject row) {
        ...
        /*
         * Iterate through the columns and pull out the names for use in the
         * insert statement and the types for use in the bindings. The tuple is
         * of the form (column name, column type).
         */
        for (int i = 0; i < len - 1; i++) {
            ...
            if (!this.excluded(col)) {
                ...
                try {
                    this.indexedBindings.__setitem__(pyIndex,
                            this.bindings.__getitem__(col));
                } catch (Exception e) {
        ...
        if (index == 0) {
            throw zxJDBC.makeException(zxJDBC.ProgrammingError,
                    zxJDBC.getString("excludedAllCols"));
        }
    ...
    /**
     * Handle the row. Insert the data into the correct table and columns. No
     * updates are done.
     */
    public void row(PyObject row) {
        ...
        if (this.sql != Py.None) {
            if (this.batchsize <= 0) {
                ...
                this.cursor.execute(this.sql, row, this.indexedBindings,
                        Py.None);
                ...
                if (len % this.batchsize == 0) {
                    this.cursor.execute(this.sql, this.rows,
                            this.indexedBindings, Py.None);
                    this.connection.commit();
    ...
    public void end() {
        ...
        try {
            ...
            if (len > 0) {
                this.cursor.execute(this.sql, this.rows, this.indexedBindings,
                        Py.None);
                this.connection.commit();
            }
        } finally {
