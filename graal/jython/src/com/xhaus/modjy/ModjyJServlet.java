...
public class ModjyJServlet extends HttpServlet {
    ...
    /**
     * Read configuration 1. Both context and servlet parameters are included in
     * the set, so that the definition of some parameters (e.g python.*) can be
     * shared between multiple WSGI servlets. 2. servlet params take precedence
     * over context parameters
     */
    protected Properties readConfiguration() {
        ...
        // Check if python home is relative, in which case find it in the
        // servlet context
    ...
    /**
     * Initialise the modjy servlet. 1. Read the configuration 2. Initialise the
     * jython runtime 3. Setup, in relation to the J2EE servlet environment 4.
     * Create the jython-implemented servlet 5. Initialise the
     * jython-implemented servlet
     */
    @Override
    public void init() throws ServletException {
        try {
            ...
            try {
                ...
                throw new ServletException("Unable to import '"
                        + MODJY_PYTHON_CLASSNAME
                        + "': maybe you need to set the '" + PYTHON_HOME_PARAM
                        + "' parameter?", ix);
            }
            PyObject pyServlet = ((PyType) interp.get(MODJY_PYTHON_CLASSNAME))
                    .__call__();
            Object temp = pyServlet.__tojava__(HttpServlet.class);
            if (temp == Py.NoConversion)
                throw new ServletException(
                        "Corrupted modjy file: cannot find definition of '"
                                + MODJY_PYTHON_CLASSNAME + "' class");
            modjyServlet = (HttpServlet) temp;
            modjyServlet.init(this);
        } catch (PyException pyx) {
            throw new ServletException("Exception creating modjy servlet: "
                    + pyx.toString(), pyx);
        }
    }

    /**
     * Actually service the incoming request. Simply delegate to the jython
     * servlet.
     * 
     * @param req
    ...
    /**
     * Setup the modjy environment, i.e. 1. Find the location of the modjy.jar
     * file and add it to sys.path 2. Process the WEB-INF/lib-python directory,
     * if it exists
     * 
     * @param interp
     ...
     * @param systemState
     *            - The PySystemState corresponding to the interpreter servicing
     *            requests
     */
    protected void setupEnvironment(PythonInterpreter interp, Properties props,
    ...
    /**
     * Check if the user has requested to initialise the jython installation
     * "site-packages".
     * 
     * @param props
     *            - The properties from which config options are found
    ...
    protected void checkSitePackages(Properties props) throws PyException {
        String loadSitePackagesParam = props
                .getProperty(LOAD_SITE_PACKAGES_PARAM);
        boolean loadSitePackages = true;
        if (loadSitePackagesParam != null
                && loadSitePackagesParam.trim().compareTo("0") == 0)
     ...
     */
    protected void processPythonLib(PythonInterpreter interp,
        ...
        for (String libPythonContent : libPythonContents)
            if (libPythonContent.endsWith(PTH_FILE_EXTENSION))
                processPthFile(interp, systemState, pythonLibPath,
                        libPythonContent);
    }
    ...
    protected void processPthFile(PythonInterpreter interp,
            PySystemState systemState, String pythonLibPath, String pthFilename) {
        try {
