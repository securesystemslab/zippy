...
public class GrammarActions {
    ...
    List<Name> makeDottedName(Token top, List<PythonTree> attrs) {
        ...
        if (attrs != null) {
            for (PythonTree attr : attrs) {
                ...
                result.add(new Name(token, token.getText(),
    ...
    expr makeDottedAttr(Token nameToken, List attrs) {
        expr current = new Name(nameToken, nameToken.getText(),
                expr_contextType.Load);
        for (Object o : attrs) {
            Token t = (Token) o;
    ...
    stmt makeWith(Token t, List<With> items, List<stmt> body) {
        ...
        for (int i = last; i >= 0; i--) {
            ...
            result = new With(current.getToken(),
                    current.getInternalContext_expr(),
    ...
    stmt makeTryExcept(Token t, List body, List<excepthandler> handlers,
            List orelse, List finBody) {
        List<stmt> b = castStmts(body);
    ...
    stmt makeFuncdef(Token t, Token nameToken, arguments args,
            List funcStatements, List decorators) {
        if (nameToken == null) {
            return errorHandler.errorStmt(new PythonTree(t));
        }
    ...
    void recurseSetContext(PythonTree tree, expr_contextType context) {
        ...
        } else if (!(tree instanceof ListComp) && (!(tree instanceof DictComp))
        ...
        if (args != null) {
            for (Object o : args) {
                ...
                if (k instanceof Name) {
                    Name arg = (Name) k;
                    keywords.add(new keyword(arg, arg.getInternalId(),
                            castExpr(v)));
                } else {
                    errorHandler
                            .error("keyword must be a name", (PythonTree) k);
                }
            }
        }
        return keywords;
    ...
    StringPair extractString(Token t, String encoding, boolean unicodeLiterals) {
        ...
        if (!ustring && encoding != null) {
            ...
            string = new PyUnicode(string.substring(start, end))
                    .encode(encoding);
            if (!raw) {
                // Handle escapes in non-raw strs
                string = PyString.decode_UnicodeEscape(string, 0,
                        string.length(), "strict", ustring);
            }
        } else if (raw) {
            ...
            if (ustring) {
                ...
                string = codecs.PyUnicode_DecodeRawUnicodeEscape(string,
                        "strict");
            }
        } else {
            // Plain unicode: already decoded, just handle escapes
            string = PyString.decode_UnicodeEscape(string, start, end,
                    "strict", ustring);
        }
    ...
    expr makeCall(Token t, expr func, List args, List keywords, expr starargs,
            expr kwargs) {
        if (func == null) {
    ...
    void checkAugAssign(expr e) {
        ...
        if (e instanceof Tuple) {
            errorHandler.error(
                    "assignment to tuple illegal for augmented assignment", e);
        } else if (e instanceof org.python.antlr.ast.List) {
            errorHandler.error(
                    "assignment to list illegal for augmented assignment", e);
        }
    }
    ...
    slice makeSubscript(PythonTree lower, Token colon, PythonTree upper,
            PythonTree sliceop) {
    ...
    BinOp makeBinOp(Token t, PythonTree left, operatorType op, List rights) {
        BinOp current = new BinOp(t, castExpr(left), op,
                castExpr(rights.get(0)));
    ...
    BinOp makeBinOp(Token t, PythonTree left, List ops, List rights, List toks) {
        BinOp current = new BinOp(t, castExpr(left), (operatorType) ops.get(0),
                castExpr(rights.get(0)));
