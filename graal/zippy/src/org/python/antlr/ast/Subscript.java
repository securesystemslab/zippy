...
public class Subscript extends expr implements Context {
    ...
    public void setValue(PyObject value) {
        ...
        if (Options.specialize) {
            this.value = adoptChild(AstAdapters.py2expr(value));
            // this.value.replace(AstAdapters.py2expr(value));
        } else {
    ...
    private final static PyString[] fields = new PyString[] {
            new PyString("value"), new PyString("slice"), new PyString("ctx") };
    ...
    public Subscript(Integer ttype, Token token, expr value, slice slice,
    ...
    public Subscript(PythonTree tree, expr value, slice slice,
            expr_contextType ctx) {
        super(tree);
        this.value = value;
        addChild(value);
    ...
    public Object executeGeneric(VirtualFrame frame) {
        assert ctx == expr_contextType.Load : "this method should only be invoked within the context of Load";
        PyObject val = (PyObject) value.executeGeneric(frame);

        if (slice instanceof Index) {
            PyObject key = (PyObject) slice.executeGeneric(frame);
            return val.__finditem__(key);
        } else if (slice instanceof Slice) {
            PySlice pySlice = (PySlice) slice.executeGeneric(frame);
            return val.__getitem__(pySlice);
        }
    ...
    /**
     * doUpdate should be called when a Subscript is on the left hand side.
     * Ideally, a different AST Node should be created for this scenario.
     * 
     * @param frame
     * @param newValue
     */
    public void doUpdate(VirtualFrame frame, PyObject newValue) {
        assert ctx == expr_contextType.Store : "this method should only be invoked within the context of Store";
        PyObject val = (PyObject) value.executeGeneric(frame);
        ...
        if (slice instanceof Index) {
            ...
            PySlice pySlice = (PySlice) slice.executeGeneric(frame);
            val.__setslice__(pySlice.start, pySlice.stop, newValue);
        }
    }
