...
public class If extends stmt {
    ...
    public If(Token token, expr test, java.util.List<stmt> body,
    ...
    public If(PythonTree tree, expr test, java.util.List<stmt> body,
            java.util.List<stmt> orelse) {
        super(tree);
        this.test = test;
        addChild(test);
    ...
    public void traverse(VisitorIF<?> visitor) throws Exception {
        ...
        if (orelse != null) {
            ...
            for (int i=0; i<orelse.size(); i++) {
                if (orelse.get(i) != null)
                    orelse.get(i).accept(visitor);
            }
        }
    }
    ...
    public void executeVoid(VirtualFrame frame) {
        PyObject testResult = (PyObject) test.executeGeneric(frame);
        
        if (testResult.__nonzero__()) {
            doBody(frame);
        } else {
            doOrElse(frame);
        }
    ...
    private void doBody(VirtualFrame frame) {
        for (stmt s : body) {
            s.executeVoid(frame);
        }
    }

    private void doOrElse(VirtualFrame frame) {
        for (stmt s : orelse) {
            s.executeVoid(frame);
        }
    }
