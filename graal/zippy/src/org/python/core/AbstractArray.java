...
/**
 * Abstract class that manages bulk structural and data operations on arrays,
 * defering type-specific element-wise operations to the subclass. Subclasses
 * supply the underlying array and the type-specific operations--greatly
 * reducing the need for casting (thus achieving array-like performances with
 * collection-like flexibility). Also includes functionality to support
 * integration with the the jdk's collections (via methods that return a
 * modification increment).
 * <P>
 * Subclasses will want to provide the following methods (which are not declared
 * in this class since subclasses should specify the explicit return type):
 * <UL>
 ...
 * </UL>
 * <P>
 * Clone cannot be supported since the array is not held locally. But the @link
 * #AbstractArray(AbstractArray) constructor can be used for suclasses that need
 * to support clone.
 * <P>
 * This "type-specific collections" approach was originally developed by Dennis
 * Sosnoski, who provides a more complete library at the referenced URL.
 * Sosnoski's library does not integrate with the jdk collection classes but
 * provides collection-like classes.
 * 
 * @author Clark Updike
 * @see <A href="http://www.sosnoski.com/opensrc/tclib/index.html"> Sosnoski's
 *      Type-Specific Collection Library</A>
 */
public abstract class AbstractArray implements Serializable {

    /**
     * Size of the current array, which can be larger than the <CODE>size</CODE>
     * field.
     */
     ...
     * over the array invalid. It is typically used by subclasses that extend
     * <CODE>AbstractList</CODE>, by adding the value to
     * <CODE>AbstractList.modCount</CODE> after performing a potentially
     * structure-altering operation. A value of 0 indicates that it is still
     * valid to iterate over the array. A value of 1 indicates it is no longer
     * valid to iterate over the range.
     * <P>
     * This class uses a somewhat stricter semantic for <CODE>modCount</CODE>.
     * Namely, <CODE>modCountIncr</CODE> is only set to 1 if a structural change
     * occurred. The jdk collections generally increment <CODE>modCount</CODE>
     * if a potentially structure-altering method is called, regardless of
     * whether or not a change actually occurred.
     * 
     ...
     * sublcasses that want to implement clone (poor man's cloning). Sublclasses
     * can then do this:
     * 
     * <PRE>
    ...
    /**
     * Construtor for multi-dimensional array types. For example,
     * <CODE>char[][]</CODE>. This class only manages the top level dimension of
     * the array. For single dimension arrays (the more typical usage), use the
     * other constructors.
     * <P>
     * 
     * @param type
     *            Array element type (primitive type or object class).
     * @param dimensions
     *            An int array specifying the dimensions. For a 2D array,
     *            something like <CODE>new int[] {10,0}</CODE> to create 10
     *            elements each of which can hold an reference to an array of
     *            the same type.
     ...
     * <CODE>this</CODE>, to the end of <CODE>this</CODE>.
     * <P>
    ...
    /**
     * Set the array to the empty state, clearing all the data out and nulling
     * objects (or "zero-ing" primitives).
     * <P>
     * Note: This method does not set <CODE>modCountIncr</CODE> to
     * <CODE>1</CODE> even though <CODE>java.util.ArrayList</CODE> would.
     * <p/>
     * <P>
     * <CODE>AbstractList</CODE> subclasses should update their
     * <CODE>modCount</CODE> after calling this method.
    ...
    /**
     * Clears out the values in the specified range. For object arrays, the
     * cleared range is nullified. For primitve arrays, it is "zero-ed" out.
     * <P>
     * Note: This method does not set <CODE>modCountIncr</CODE> to
     * <CODE>1</CODE> even though <CODE>java.util.ArrayList</CODE> would.
     * 
        ...
        if (start < stop && start >= 0 && stop <= this.size) {
            ...
            throw new ArrayIndexOutOfBoundsException(
                    "start and stop must follow: 0 <= start <= stop <= "
                            + (this.size) + ", but found start= " + start
                            + " and stop=" + stop);
        }
    }
    ...
    protected void ensureCapacity(int minCapacity) {
        ...
        if (minCapacity > this.capacity) {
            ...
            newCapacity = (newCapacity < minCapacity) ? minCapacity
                    : newCapacity;
     ...
     * increment so that the index value returned is always valid for the array
     * in use by the time of the return.
     * <P>
     * <CODE>AbstractList</CODE> subclasses should update their
     * <CODE>modCount</CODE> after calling this method.
     * 
     ...
     * <P>
     * <CODE>AbstractList</CODE> subclasses should update their
     * <CODE>modCount</CODE> after calling this method. Does not change the
     * <CODE>size</CODE> property of the array.
     * 
     * @param index
    ...
    protected void makeInsertSpace(int index, int length) {
        ...
        if (index >= 0 && index <= this.size) {
            ...
            throw new ArrayIndexOutOfBoundsException(
                    "Index must be between 0 and " + this.size + ", but was "
                            + index);
        }
    }
     ...
     * <P>
     * <CODE>AbstractList</CODE> subclasses should always increment their
     * <CODE>modCount</CODE> method after calling this, as <CODE>remove</CODE>
    ...
    public void remove(int index) {
        if (index >= 0 && index < this.size) {
            ...
            if (index < this.size) {
                Object base = getArray();
                System.arraycopy(base, index + 1, base, index, this.size
                        - index);
    ...
    /**
     * Allows an array type to overwrite a segment of the array. Will expand the
     * array if <code>(atIndex + 1) + ofArrayType</code>'s length is greater
    ...
    public void replaceSubArray(Object array, int atIndex) {
        ...
        replaceSubArray(atIndex, Math.min(this.size, atIndex + arrayLen),
                array, 0, arrayLen);
    }
     ...
     * 
     * @param thisStart
     *            the start index (inclusive) of the subarray in this array to
     *            be replaced
     * @param thisStop
     *            the stop index (exclusive) of the subarray in this array to be
     *            replaced
     * @param srcArray
    ...
    public void replaceSubArray(int thisStart, int thisStop, Object srcArray,
        ...
        if (srcStart < 0 || replacementLen < 0 || srcStop > srcLen) {
            ...
            throw new IllegalArgumentException(
                    "start, stop and array must follow:\n\t"
        ...
        try {
            this.modCountIncr = 1;
            System.arraycopy(srcArray, srcStart, getArray(), thisStart,
                    replacementLen);
        } catch (ArrayStoreException e) {
            throw new IllegalArgumentException(
                    "'ofArrayType' must be compatible with existing array type of "
                            + getArray().getClass().getName()
    ...
    /**
     * Replaces the existing base array in the subclass with a new base array
     * resized to the specified capacity.
     * 
     * @param newCapacity
     */
    private void setNewBase(int newCapacity) {
        ...
        System.arraycopy(base, 0, newBase, 0,
    ...
    /**
     * Removes any excess capacity in the backing array so it is just big enough
     * to hold the amount of data actually in the array.
     */
     ...
     * <CODE>AbstractList</CODE> uses it's <CODE>modCount</CODE> field to
     * invalidate concurrent operations (like iteration) that should fail if the
     * underlying array changes structurally during the operation.
     * 
    ...
    /**
     * @return an array of the given size for the type used by this abstract
     *         array.
     */
