...
import org.python.core.PyBytecode.PyBlock;
...
public class PyFrame extends PyObject {
    ...
    /**
     * ModularVM
     * f_blocks is used instead of f_exits to solve the NullPointerException
     */
    //public Stack<PyBlock> f_blocks;
    public ArrayList<PyBlock> f_blocks;
    ...
    public PyFrame(PyBaseCode code, PyObject locals, PyObject globals,
        ...
        if (locals == null && code != null) {
            ...
            if (code.co_flags.isFlagSet(CodeFlag.CO_OPTIMIZED)
                    || code.nargs > 0) {
    ...
    /**
     * Return the locals dict. First merges the fast locals into f_locals, then
     * returns the updated f_locals.
     * 
    ...
    public PyObject getTrace() {
        return tracefunc instanceof PythonTraceFunction ? ((PythonTraceFunction) tracefunc).tracefunc
                : Py.None;
    }
     ...
     * 
     * This is not to be confused with the CPython method frame_setlineno()
     * which causes the interpreter to jump to the given line.
     */
    ...
    public void setlocal(String index, PyObject value) {
        if (f_locals != null) {
            ...
            throw Py.SystemError(String.format(
                    "no locals found when storing '%s'", value));
        }
    }
    ...
    public void dellocal(int index) {
        if (f_fastlocals != null) {
            if (f_fastlocals[index] == null) {
                throw Py.UnboundLocalError(String.format(
                        UNBOUNDLOCAL_ERROR_MSG, f_code.co_varnames[index]));
            }
    ...
    public void dellocal(String index) {
        if (f_locals != null) {
            ...
            throw Py.SystemError(String.format("no locals when deleting '%s'",
                    index));
        }
    }
