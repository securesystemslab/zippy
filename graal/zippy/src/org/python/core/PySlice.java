...
public class PySlice extends PyObject {
    ...
    @ExposedNew
    static PyObject slice_new(PyNewWrapper new_, boolean init, PyType subtype,
            PyObject[] args, String[] keywords) {
        if (args.length == 0) {
            throw Py.TypeError("slice expected at least 1 arguments, got "
                    + args.length);
        } else if (args.length > 3) {
            throw Py.TypeError("slice expected at most 3 arguments, got "
                    + args.length);
        }
        ArgParser ap = new ArgParser("slice", args, keywords, "start", "stop",
                "step");
    ...
    public PyObject __eq__(PyObject o) {
        ...
        return Py.newBoolean(eq(getStart(), oSlice.getStart())
                && eq(getStop(), oSlice.getStop())
    ...
    final PyObject slice_indices(PyObject len) {
        ...
        return new PyTuple(Py.newInteger(indices[0]),
                Py.newInteger(indices[1]), Py.newInteger(indices[2]));
    }

    /**
     * Calculates the actual indices of a slice with this slice's start, stop,
     * step and slicelength values for a sequence of length <code>len</code>.
     * 
     * @return an array with the start at index 0, stop at index 1, step at
     *         index 2 and slicelength at index 3
     */
     ...
     */
    public static PyObject[] indices2(PyObject obj, PyObject start,
            PyObject stop) {
        PyObject[] indices = new PyObject[2];
        int istart = (start == null || start == Py.None) ? 0
                : calculateSliceIndex(start);
        int istop = (stop == null || stop == Py.None) ? PySystemState.maxint
    ...
    final String slice_toString() {
        return String.format("slice(%s, %s, %s)", getStart(), getStop(),
                getStep());
    }
