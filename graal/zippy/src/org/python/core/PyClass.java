...
public class PyClass extends PyObject {
    ...
    // Store these methods for performance optimization. These are only used by
    // PyInstance
    ...
    @ExposedNew
    public static PyObject classobj___new__(PyNewWrapper new_, boolean init,
            PyType subtype, PyObject[] args, String[] keywords) {
        ArgParser ap = new ArgParser("function", args, keywords, "name",
                "bases", "dict");
    ...
    public static PyObject classobj___new__(PyObject name, PyObject bases,
            PyObject dict) {
    ...
    public void noAttributeError(String name) {
        throw Py.AttributeError(String.format(
                "class %.50s has no attribute '%.400s'", __name__, name));
    }
    ...
    public String toString() {
        ...
        String modStr = (mod == null || !Py.isInstance(mod, PyString.TYPE)) ? "?"
                : mod.toString();
        return String.format("<class %s.%s at %s>", modStr, __name__,
                Py.idstr(this));
    }
    ...
    public void setDict(PyObject value) {
        if (value == null
                || !(value instanceof PyStringMap || value instanceof PyDictionary)) {
