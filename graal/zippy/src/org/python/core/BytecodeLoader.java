...
public class BytecodeLoader {
     ...
     *            the java byte code.
     */
    public static Class<?> makeClass(String name, List<Class<?>> referents,
            byte[] data) {
        if (referents != null) {
            return makeClass(name, data,
                    referents.toArray(new Class[referents.size()]));
        }
        return makeClass(name, data);
    }
    ...
    public static class Loader extends URLClassLoader {
        ...
        public Class<?> loadClassFromBytes(String name, byte[] data) {
            ...
            Class<?> c = defineClass(name, data, 0, data.length, getClass()
                    .getProtectionDomain());
            resolveClass(c);
            return c;
        }
