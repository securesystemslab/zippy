...
public class ClasspathPyImporter extends importer<String> {
    ...
    public static final PyType TYPE = PyType
            .fromClass(ClasspathPyImporter.class);
    ...
    protected String makeEntry(String filename) {
        ...
        if (Py.getSystemState().getClassLoader() != null) {
            is = tryClassLoader(filename, Py.getSystemState().getClassLoader(),
                    "sys");
        } else {
    ...
    private InputStream tryClassLoader(String fullFilename, ClassLoader loader,
            String name) {
        if (loader != null) {
            Py.writeDebug("import", "trying " + fullFilename + " in " + name
                    + " class loader");
    ...
    protected String makeFilename(String fullname) {
        return path.replace(PYCLASSPATH_PREFIX, "")
                + fullname.replace('.', '/');
    }
