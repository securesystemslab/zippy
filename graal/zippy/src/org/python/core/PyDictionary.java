...
public class PyDictionary extends PyObject implements ConcurrentMap {
    ...
    public PyDictionary(PyType type, int capacity) {
        super(type);
        map = new ConcurrentHashMap<PyObject, PyObject>(capacity,
     ...
     * factories, with different backing maps, at the cost that it prevents us
     * from making table be final.
     */
    // TODO we may want to revisit this API, but our chain calling of super
    // makes this tough
     ...
     * 
     * @param elements
     *            The initial elements that is inserted in the dictionary. Even
     *            numbered elements are keys, odd numbered elements are values.
     */
    ...
    /**
     * Insert all the key:value pairs from <code>d</code> into this dictionary.
     */
    ...
    private void updateCommon(PyObject[] args, String[] keywords,
            String methName) {
        int nargs = args.length - keywords.length;
        if (nargs > 1) {
            throw PyBuiltinCallable.DefaultInfo.unexpectedCall(nargs, false,
                    methName, 0, 1);
        }
        if (nargs == 1) {
            PyObject arg = args[0];
    ...
    private void merge(Map<Object, Object> other) {
        for (Entry<Object, Object> entry : other.entrySet()) {
            dict___setitem__(Py.java2py(entry.getKey()),
    ...
    /**
     * Merge any iterable object producing iterable objects of length 2 into
     * this dict.
     * 
     * @param other
     *            another PyObject
    ...
    /**
     * Return this[key] if the key exist, otherwise insert key with a None value
     * and return None.
     * 
     * @param key
     *            the key to lookup in the dictionary.
     ...
     * failobj and return failobj
     * 
     * @param key
     *            the key to lookup in the dictionary.
     * @param failobj
     *            the default value to insert in the dictionary if key does not
     *            already exist.
     */
    ...
    /**
     * Return a value based on key from the dictionary or default if that key is
     * not found.
     */
    ...
    /**
     * Return a random (key, value) tuple pair and remove the pair from the
     * dictionary.
     */
    ...
    final int dict___hash__() {
        throw Py.TypeError(String.format("unhashable type: '%.200s'", getType()
                .fastGetName()));
    }
     ...
     * PyDictionary to be passed to java methods that take java.util.Map as a
     * parameter. Basically, the Map methods are a wrapper around the
     * PyDictionary's Map container stored in member variable 'table'. These
    ...
    public void putAll(Map map) {
        for (Object o : map.entrySet()) {
            Entry entry = (Entry) o;
            getMap().put(Py.java2py(entry.getKey()),
                    Py.java2py(entry.getValue()));
        }
    }
    ...
    public boolean replace(Object key, Object oldValue, Object newValue) {
        return getMap().replace(Py.java2py(key), Py.java2py(oldValue),
                Py.java2py(newValue));
    }
 ...
 * turn is returned by the entrySet method of java.util.Map. This is needed to
 * correctly convert from PyObjects to java Objects. Note that we take care in
 * the equals and hashCode methods to make sure these methods are consistent
 * with Entry objects that contain java Objects for a value so that on the java
 ...
 * JavaToPyMapEntry and PyToJavaMapEntry. Note - The set interface is reliable
 * for standard objects like strings and integers, but may be inconsistent for
 * other types of objects since the equals method may return false for Entry
 * object that hold more elaborate PyObject types. However, we insure that this
...
class PyMapEntrySet extends PyMapSet {
    ...
    // We know that PyMapEntrySet will only contains entries, so if the object
    // being passed in is
    // null or not an Entry, then return null which will match nothing for
    // remove and contains
    // methods.
    @Override
    Object toPython(Object o) {
 ...
 * interface of PyDictionary. entrySet, values and keySet methods return this
 * type for the java.util.Map implementation. This class is necessary as a
 * wrapper to convert PyObjects to java Objects for methods that return values,
 * and convert Objects to PyObjects for methods that take values. The
 * translation is necessary to provide java access to jython dictionary objects.
 * This wrapper also provides the expected backing functionality such that
 * changes to the wrapper set or reflected in PyDictionary.
 */
