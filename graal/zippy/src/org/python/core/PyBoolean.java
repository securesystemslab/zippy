...
/**
 * The builtin python bool. It would be nice if it didn't extend PyInteger, but
 * too hard to avoid pre-Python 2.2 semantics here.
 */
...
public class PyBoolean extends PyInteger {
    ...
    @ExposedNew
    public static PyObject bool_new(PyNewWrapper new_, boolean init,
            PyType subtype, PyObject[] args, String[] keywords) {
        ArgParser ap = new ArgParser("bool", args, keywords,
                new String[] { "x" }, 0);
    ...
    final PyObject bool___and__(PyObject right) {
        if (right instanceof PyBoolean) {
            return Py.newBoolean(getBooleanValue()
