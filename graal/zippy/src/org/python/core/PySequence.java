...
public abstract class PySequence extends PyObject {
    ...
    /**
     * Returns the element of the sequence at the given index. This is an
     * extension point called by PySequence in its implementation of
     * {@link #__getitem__} It is guaranteed by PySequence that when it calls
     * <code>pyget(int)</code> the index is within the bounds of the array. Any
    ...
    /**
     * Returns a (concrete subclass of) PySequence that repeats the given
     * sequence, as in the implementation of <code>__mul__</code> for strings.
     * 
     * @param count
     ...
     * extension point called by PySequence in its implementation of
     * {@link #__setitem__} It is guaranteed by PySequence that when it calls
     * pyset(int) the index is within the bounds of the array. Any other clients
     * must make the same guarantee.
     * 
     * @param index
     ...
     * semantics. If the step size is one, it is a simple slice and the
     * operation is equivalent to deleting that slice, then inserting the value
     * at that position, regarding the value as a sequence (if possible) or as a
     * single element if it is not a sequence. If the step size is not one, but
     * <code>start==stop</code>, it is equivalent to insertion at that point. If
     * the step size is not one, and <code>start!=stop</code>, the slice defines
     * a certain number of elements to be replaced, and the value must be a
     * sequence of exactly that many elements (or convertible to such a
     * sequence).
     * 
     * @param start
     *            the position of the first element.
    ...
    protected void setslice(int start, int stop, int step, PyObject value) {
        throw Py.TypeError(String.format(
                "'%s' object does not support item assignment", getType()
                        .fastGetName()));
    }
    ...
    protected void del(int index) {
        throw Py.TypeError(String.format(
                "'%s' object does not support item deletion", getType()
                        .fastGetName()));
    }
     ...
     * 
     * @return value >= 0 is the index where the sequences differs. -1: reached
     *         the end of o1 without a difference -2: reached the end of both
     *         seqeunces without a difference -3: reached the end of o2 without
     *         a difference
     */
    ...
    /**
     * Return a copy of a sequence where the __len__() method is telling the
     * truth.
     */
     ...
     * <code>index</code> starts off negative, it's treated as an index from the
     * end of the sequence going back to the start.
     */
    ...
    final void seq___setslice__(PyObject start, PyObject stop, PyObject step,
            PyObject value) {
        if (value == null) {
            value = step;
            step = null;
