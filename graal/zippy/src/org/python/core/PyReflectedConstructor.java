...
public class PyReflectedConstructor extends PyReflectedFunction {
    ...
    PyObject make(PyObject[] args, String[] keywords) {
        ...
        if (nargs > 0) { // PyArgsKeywordsCall signature, if present, is the
                         // first
        ...
        PyObject obj;
        try {
            obj = (PyObject) ((Constructor<?>) method).newInstance(callData
                    .getArgsArray());
        } catch (Throwable t) {
    ...
    public PyObject __call__(PyObject self, PyObject[] args, String[] keywords) {
        ...
        if (javaClass == null) {
            throw Py.TypeError("self invalid - must be a Java subclass [self="
                    + self + "]");
        }
        Class<?> declaringClass = argslist[0] == null ? null
                : argslist[0].declaringClass;
        // If the declaring class is a pure Java type but we're instantiating a
        // Python proxy,
        // grab the proxy version of the constructor to instantiate the proper
        // type
        if ((declaringClass == null || !PyProxy.class
                .isAssignableFrom(declaringClass))
                && !(self.getType() instanceof PyJavaType)) {
            return PyType.fromClass(javaClass).lookup("__init__")
                    .__call__(self, args, keywords);
        }
        if (nargs == 0) {
            throw Py.TypeError("No visible constructors for class ("
                    + javaClass.getName() + ")");
        }
        ...
        // non-keyword args, and then use the keyword args to set bean
        // properties
        ...
        if (nkeywords > 0) {
            ...
            // Look for a constructor; the ReflectedArgs#matches() method exits
            // early in the case
        ...
        // Do the actual constructor call
        constructProxy(self, (Constructor<?>) method, callData.getArgsArray(),
                javaClass);
        // Do setattr's for keyword args. This convenience allows Java bean
        // properties to be set in
        // by a Python constructor call.
        // However, this is not done if the Java constructor accepts
        // (PyObject[], String[]) as its arguments,
        // in which case the intention is that the Java constructor will handle
    ...
    protected void constructProxy(PyObject obj, Constructor<?> ctor,
            Object[] args, Class<?> proxy) {
        // Do the actual constructor call
        Object jself = null;
