 ...
 * 
 * This differs from py3k TextIOWrapper, which currently handles both text mode
 * (py3k text mode is incompatible with Python 2.x's text mode) as well as
 * universal mode.
 * 
...
public class TextIOWrapper extends BinaryIOWrapper {
    ...
    public String readall() {
        ...
        int length = 0;
        if (readahead.hasRemaining()
                && readahead.get(readahead.limit() - 1) == CR_BYTE
                && remaining.hasRemaining()
                && remaining.get(remaining.position()) == LF_BYTE) {
        ...
        length += readLoop(remaining.array(), remaining.position(), all,
     ...
     * 
     * Converts CRLF to LF. src is assumed to be the end of the file; that is,
     * when src ends with CR, no attempt is made to peek into the underlying
     * file to detect a succeeding LF.
     * 
     * @param src
     ...
     */
    private int readLoop(byte[] src, int srcPos, char[] dest, int destPos,
            int length) {
