...
public abstract class BufferedIOBase extends IOBase {
     ...
     * 
     * The argument indicates a desired minimal number of bytes; we do at most
     * one raw read to satisfy it. We never return more than the size of the
     * underlying buffer;
     * 
     * @param size
     ...
     * 
     * Returns up to bytes.remaining() bytes. If at least one byte is buffered,
     * we only return buffered bytes. Otherwise, we do one raw read.
     * 
     * @param bytes
     *            a ByteBuffer to read bytes into
     * @return the amount of data read as an int
