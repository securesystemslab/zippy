...
public class PyFloat extends PyObject {
    ...
    @ExposedNew
    public static PyObject float_new(PyNewWrapper new_, boolean init,
            PyType subtype, PyObject[] args, String[] keywords) {
        ArgParser ap = new ArgParser("float", args, keywords,
                new String[] { "x" }, 0);
    ...
    public static PyObject float_fromhex(PyType type, PyObject o) {
        // XXX: I'm sure this could be shortened/simplified, but
        // Double.parseDouble() takes
        // non-hex strings and requires the form 0xNUMBERpNUMBER for hex input
        // which
    ...
    public Object __tojava__(Class<?> c) {
        if (c == Double.TYPE || c == Number.class || c == Double.class
    ...
    public PyObject __eq__(PyObject other) {
        // preclude _cmp_unsafe's this == other shortcut because NaN !=
        // anything, even
        // itself
    ...
    static PyObject formatImpl(double d, PyObject formatSpec) {
        ...
        try {
            ...
            switch (spec.type) {
            case '\0': /*
                        * No format code: like 'g', but with at least one
                        * decimal.
                        */
            case 'e':
            case 'E':
    ...
    public PyTuple as_integer_ratio() {
         ...
         * If FLT_RADIX != 2, the 300 steps may leave a tiny fractional part to
         * be truncated by PyLong_FromDouble().
         */
        ...
        if (exponent > 0) {
            numerator = new PyLong(numerator.getValue().multiply(
                    py_exponent.getValue()));
        } else {
    ...
    public static void float___setformat__(PyType type, String typestr,
            String format) {
