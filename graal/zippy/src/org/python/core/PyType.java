...
public class PyType extends PyObject implements Serializable {
    ...
    /**
     * The Java Class instances of this type will be represented as, or null if
     * it's determined by a base type.
     */
    ...
    /**
     * Whether this type has a __weakref__ slot (however all types are
     * weakrefable).
     */
    ...
    /**
     * Creates the PyType instance for type itself. The argument just exists to
     * make the constructor distinct.
     */
    ...
    @ExposedNew
    static final PyObject type___new__(PyNewWrapper new_, boolean init,
        ...
        // If that didn't trigger, we need 3 arguments. but ArgParser below may
        // give a msg
        ...
        if (!(dict instanceof PyDictionary || dict instanceof PyStringMap)) {
            throw Py.TypeError("type(): argument 3 must be dict, not "
                    + dict.getType());
        }
    ...
    public static PyObject newType(PyNewWrapper new_, PyType metatype,
        ...
        if (winner != metatype) {
            ...
            if (winnerNew != null && winnerNew != new_) {
                return invokeNew(winnerNew, winner, false, new PyObject[] {
                        new PyString(name), bases, dict }, Py.NoKeywords);
            }
            metatype = winner;
        }

        // Use PyType as the metaclass for Python subclasses of Java classes
        // rather than
        // PyJavaType. Using PyJavaType as metaclass exposes the
        ...
        type.name = name;
        type.bases = tmpBases.length == 0 ? new PyObject[] { PyObject.TYPE }
                : tmpBases;
        type.dict = dict;
        ...
        if (!base.isBaseType) {
            throw Py.TypeError(String.format(
                    "type '%.100s' is not an acceptable base type", base.name));
        }
    ...
    private void createAllSlots(boolean mayAddDict, boolean mayAddWeak) {
        ...
        if (slots == null) {
            ...
            // Check for valid slot names and create them. Handle two special
            // cases
            for (PyObject slot : slots.asIterable()) {
                ...
                if (slotName.equals("__dict__")) {
                    ...
                    if (dict.__finditem__(slotName) == null) {
                        dict.__setitem__(slotName, new PySlot(this, slotName,
                                numSlots++));
                    }
                }
            }
    ...
    private void createDictSlot() {
        ...
        dict.__setitem__("__dict__", new PyDataDescr(this, "__dict__",
                PyObject.class, doc) {
            @Override
    ...
    private void createWeakrefSlot() {
        ...
        dict.__setitem__("__weakref__", new PyDataDescr(this, "__weakref__",
                PyObject.class, doc) {
    ...
    /**
     * Ensure dict contains a __module__, retrieving it from the current frame
     * if it doesn't exist.
     * 
     * @param dict
     *            a PyObject mapping
    ...
    private static PyObject invokeNew(PyObject new_, PyType type, boolean init,
    ...
    /**
     * Called on builtin types for a particular class. Should fill in dict,
    ...
    protected void init(Class<?> forClass, Set<PyJavaType> needsInners) {
        ...
        if (Py.BOOTSTRAP_TYPES.contains(underlying_class)) {
            // init will be called again from addBuilder which also removes
            // underlying_class from
            // BOOTSTRAP_TYPES
            return;
        }
        ...
        // XXX: Can't create a __doc__ str until the PyBaseString/PyString types
        // are
        // created
        if (dict.__finditem__("__doc__") == null
                && forClass != PyBaseString.class && forClass != PyString.class) {
            PyObject docObj;
            if (doc != null) {
                docObj = new PyString(doc);
            } else {
                // XXX: Hack: Py.None may be null during bootstrapping. Most
                // types
    ...
    /**
     * Fills the base and bases of this type with the type of baseClass as sets
     * its mro to this type followed by the mro of baseClass.
     */
    ...
    /**
     * Ensures that the physical layout between this type and <code>other</code>
     ...
     * it has slots or an underlying_class. Can be this PyType.
     */
     ...
     */
    private static Class<?> getJavaLayout(PyObject[] bases,
        ...
        for (PyObject base : bases) {
            ...
            if (proxy.isInterface()) {
                ...
                if (baseProxy != null) {
                    ...
                    throw Py.TypeError(String.format(msg, proxy.getName(),
                            baseProxy.getName()));
                }
                baseProxy = proxy;
            }
    ...
    private void setupProxy(Class<?> baseProxyClass, List<Class<?>> interfaces) {
        ...
        Class<?> proxyClass = MakeProxies.makeProxy(baseProxyClass, interfaces,
                name, proxyName, dict);
        ...
        for (PyObject base : bases) {
            ...
            if (proxy == null) {
                ...
                if (!(base instanceof PyJavaType)) {
                    // python subclasses of proxy types need to be added as a
                    // base so their
                    // version of methods will show up
                    cleanedBases.add(base);
                } else if (!addedProxyType) {
                    // Only add a single Java type, since everything's going to
                    // go through the
                    // proxy type
            ...
            if (!(newBases[i] instanceof PyType)) {
                if (!(newBases[i] instanceof PyClass)) {
                    throw Py.TypeError(name
                            + ".__bases__ must be a tuple of old- or new-style "
    ...
    private void setIsBaseType(boolean isBaseType) {
        ...
        tp_flags = isBaseType ? tp_flags | Py.TPFLAGS_BASETYPE : tp_flags
                & ~Py.TPFLAGS_BASETYPE;
    }
    ...
    private void mro_internal() {
        if (getType() == TYPE) {
            ...
            PyObject[] result = Py.make_array(mroDescr.__get__(null, getType())
                    .__call__(this));
            ...
            for (PyObject cls : result) {
                ...
                if (!(cls instanceof PyType)) {
                    throw Py.TypeError(String.format(
                            "mro() returned a non-class ('%.500s')", cls
                                    .getType().fastGetName()));
                }
                PyType t = (PyType) cls;
    ...
    /**
     * Returns the Java Class that this type inherits from, or null if this type
     * is Python-only.
     */
    ...
    PyObject[] computeMro(MROMergeState[] toMerge, List<PyObject> mro) {
        ...
        PyType proxyAsType = javaProxy == null ? null : PyType.fromClass(
                ((Class<?>) javaProxy), false);
        scan: for (int i = 0; i < toMerge.length; i++) {
            ...
            if (!addedProxy
                    && !(this instanceof PyJavaType)
                    && candidate instanceof PyJavaType
                    && candidate.javaProxy != null
                    && PyProxy.class
                            .isAssignableFrom(((Class<?>) candidate.javaProxy))
                    && candidate.javaProxy != javaProxy) {
                // If this is a subclass of a Python class that subclasses a
                // Java class, slip the
                // proxy for this class in before the proxy class in the
                // superclass' mro.
                // This exposes the methods from the proxy generated for this
                // class in addition to
                // those generated for the superclass while allowing methods
    ...
    /**
     * Must either throw an exception, or bring the merges in
     * <code>toMerge</code> to completion by finishing filling in
     * <code>mro</code>.
     */
    void handleMroError(MROMergeState[] toMerge, List<PyObject> mro) {
        ...
        for (PyObject unmerged : set) {
            ...
            msg.append(name == null ? "?" : name.toString()
                    + new PyList(((PyType) unmerged).bases));
        }
    ...
    private static boolean isSolidBase(PyType type) {
        return type.underlying_class != null
                || (type.numSlots != 0 && !type.needs_userdict);
    }
     ...
     * cached, and should generally only be used during the bootstrapping of a
     * type.
     * 
     * @param name
     *            attribute name (must be interned)
    ...
    /**
     * Attribute lookup for name through mro objects' dicts. This isn't cached,
    ...
    public synchronized static void addBuilder(Class<?> forClass,
            TypeBuilder builder) {
        ...
        if (class_to_type.containsKey(forClass)) {
            if (!Py.BOOTSTRAP_TYPES.remove(forClass)) {
                Py.writeWarning("init",
                        "Bootstrapping class not in Py.BOOTSTRAP_TYPES[class="
                                + forClass + "]");
            }
            // The types in Py.BOOTSTRAP_TYPES are initialized before their
            // builders are assigned,
            // so do the work of addFromClass & fillFromClass after the fact
            fromClass(builder.getTypeClass())
                    .init(builder.getTypeClass(), null);
        }
    }

    private synchronized static PyType addFromClass(Class<?> c,
            Set<PyJavaType> needsInners) {
    ...
    private static TypeBuilder getBuilder(Class<?> c) {
        if (classToBuilder == null) {
            // PyType itself has yet to be initialized. This should be a
            // bootstrap type, so it'll
            // go through the builder process in a second
            return null;
        }
        if (c.isPrimitive() || !PyObject.class.isAssignableFrom(c)) {
            // If this isn't a PyObject, don't bother forcing it to be
            // initialized to load its
            // builder
            return null;
        }

        // This is a PyObject, call forName to force static initialization on
        // the class so if it has
        ...
        try {
            ...
            throw new RuntimeException(
                    "Got ClassNotFound calling Class.forName on an already "
                            + " found class.", e);
    ...
    private synchronized static PyType createType(Class<?> c,
            Set<PyJavaType> needsInners) {
        PyType newtype;
        if (c == PyType.class) {
        ...
        // If filling in the type above filled the type under creation, use that
        // one
    ...
    public static synchronized PyType fromClass(Class<?> c, boolean hardRef) {
        ...
        // We haven't seen this class before, so its type needs to be created.
        // If it's being
        // exposed as a Java class, defer processing its inner types until it's
        // completely
        // created in case the inner class references a class that references
        // this class.
        ...
        for (PyJavaType javaType : needsInners) {
            ...
            for (Class<?> inner : forClass.getClasses()) {
                // Only add the class if there isn't something else with that
                // name and it came from this
                // class
                if (inner.getDeclaringClass() == forClass
                        && javaType.dict.__finditem__(inner.getSimpleName()) == null) {
                    // If this class is currently being loaded, any exposed
                    // types it contains won't have
                    // set their builder in PyType yet, so add them to
                    // BOOTSTRAP_TYPES so they're
    ...
    static PyType fromClassSkippingInners(Class<?> c,
            Set<PyJavaType> needsInners) {
        PyType type = class_to_type.get(c);
        if (type != null) {
            return type;
     ...
     * should be used before looking for the object on the actual object.
     */
    ...
    /**
     * Adds the given method to this type's dict under its name in its
     * descriptor. If there's an existing item in the dict, it's replaced.
     */
    ...
    void type___setattr__(String name, PyObject value) {
        if (builtin) {
            throw Py.TypeError(String
                    .format("can't set attributes of built-in/extension type "
                            + "'%s'", this.name));
        }
     ...
     * modification to __dict__ (or anything else affecting attribute lookups).
     */
    ...
    final PyObject type___call__(PyObject[] args, String[] keywords) {
        ...
        if (!instantiable || new_ == null) {
            throw Py.TypeError(String.format(
                    "cannot create '%.100s' instances", name));
        }

        PyObject obj = invokeNew(new_, this, true, args, keywords);
        // When the call was type(something) or the returned object is not an
        // instance of
    ...
    /**
     * Returns the actual dict underlying this type instance. Changes to Java
     * types should go through {@link #addMethod} and {@link #removeMethod}, or
     * unexpected mro errors can occur.
     */
    ...
    public void setDict(PyObject newDict) {
        ...
        throw Py.AttributeError(String.format(
                "attribute '__dict__' of '%s' objects is not " + "writable",
                getType().fastGetName()));
    }
    ...
    final String type_toString() {
        ...
        if (module instanceof PyString
            ...
            return String.format("<%s '%s.%s'>", kind, module.toString(),
                    getName());
        }
    ...
    public void noAttributeError(String name) {
        throw Py.AttributeError(String.format(
                "type object '%.50s' has no attribute '%.400s'", fastGetName(),
                name));
    }
    ...
    private static String confirmIdentifier(PyObject obj) {
        ...
        if (!(obj instanceof PyString)) {
            throw Py.TypeError(String.format(
                    "__slots__ items must be strings, not '%.200s'", obj
                            .getType().fastGetName()));
    ...
    static class TypeResolver implements Serializable {
        ...
        TypeResolver(Class<?> underlying_class, String module, String name) {
            // Don't store the underlying_class for PyProxies as the proxy type
            // needs to fill in
            // based on the class, not be the class
            if (underlying_class != null
                    && !PyProxy.class.isAssignableFrom(underlying_class)) {
    ...
    static class MROMergeState {
        ...
        /**
         * The index of the next item to be merged from mro, or mro.length if
         * this base has been completely merged.
         */
        public int next;
        ...
        /**
         * Marks candidate as merged for this base if it's the next item to be
         * merged.
         */
        ...
        /**
         * Returns true if candidate is in the items past this state's next item
         * to be merged.
         */
        ...
        /**
         * Removes the given item from this state's mro if it isn't already
         * finished.
         */
    ...
    static class MethodCache {
        ...
        public PyObject lookup_where(PyType type, String name, PyObject where[]) {
            ...
            if (isCacheableName(name)) {
                // CAS isn't totally necessary here but is possibly more
                // correct. Cache by
                // the original version before the lookup, if it's changed since
                // then
                // we'll cache a bad entry. Bad entries and CAS failures aren't
                // a concern
                // as subsequent lookups will sort themselves out
                table.compareAndSet(index, entry, new MethodCacheEntry(
                        versionTag, name, where[0], value));
            }
            return value;
        }
         ...
         * 
         * Since the cache can keep references to names alive longer than usual,
         * it avoids caching unusually large strings.
         */
        ...
        static class MethodCacheEntry extends WeakReference<PyObject> {
            ...
            public MethodCacheEntry(Object version, String name,
                    PyObject where, PyObject value) {
                super(value);
