...
public class ReflectedArgs {
    ...
    public ReflectedArgs(Object data, Class<?>[] args, Class<?> declaringClass,
        ...
        this.isVarArgs = isVarArgs; // only used for varargs matching; it should
                                    // be added after the unboxed form
    ...
    public boolean matches(PyObject self, PyObject[] pyArgs, String[] keywords,
        ...
        // we need to do box (wrap with an array) the last pyArgs.length - n
        // args
        ...
        // test(List... x)
        //
        // in this last example, don't worry if someone is overly clever in
        // calling this,
        // they can always write their own version of PyReflectedFunction and
        // put it in the proxy
        // if that's what they need to do ;)

        if (isVarArgs) {
            if (pyArgs.length == 0
                    || !(pyArgs[pyArgs.length - 1] instanceof PySequenceList)) {
    ...
    public int compareTo(ReflectedArgs other) {
        ...
        boolean replace = other.declaringClass
                .isAssignableFrom(this.declaringClass);
    ...
    public String toString() {
        String s = declaringClass + ", static=" + isStatic + ", varargs="
                + isVarArgs + ",flags=" + flags + ", " + data + "\n";
        s = s + "\t(";
        for (Class<?> arg : args) {
            s += arg.getName() + ", ";
        }
