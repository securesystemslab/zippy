...
public class FunctionThread extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            if (exc.match(Py.SystemExit)
                    || exc.match(_systemrestart.SystemRestart)) {
                return;
            }
            Py.stderr.println("Unhandled exception in thread started by "
                    + func);
    ...
    public String toString() {
        ...
        if (group != null) {
            return String.format("FunctionThread[%s,%s,%s]", getName(),
                    getPriority(), group.getName());
        } else {
            return String.format("FunctionThread[%s,%s]", getName(),
                    getPriority());
        }
    }
}
