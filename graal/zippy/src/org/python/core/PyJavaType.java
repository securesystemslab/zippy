...
public class PyJavaType extends PyType {
    ...
    private final static Set<String> BAD_AWT_METHODS = Generic.set("layout",
            "insets", "size", "minimumSize", "preferredSize", "maximumSize",
            "bounds", "enable");
    ...
    private final static Set<Class<?>> immutableClasses = Generic.set(
            ...
            String.class, java.net.InetAddress.class,
            java.net.Inet4Address.class, java.net.Inet6Address.class,
            java.net.InetSocketAddress.class, java.net.Proxy.class,
            java.net.URI.class, java.util.concurrent.TimeUnit.class);

    private static Map<Class<?>, PyBuiltinMethod[]> collectionProxies;

    /**
     * Other Java classes this type has MRO conflicts with. This doesn't matter
     * for Java method resolution, but if Python methods are added to the type,
     * the added methods can't overlap with methods added to any of the types in
     * this set. If this type doesn't have any known conflicts, this is null.
     */
    ...
    public static PyObject wrapJavaObject(Object o) {
        PyObject obj = new PyObjectDerived(
                PyType.fromClass(o.getClass(), false));
        obj.javaProxy = o;
        return obj;
    }
    ...
    protected boolean useMetatypeFirst(PyObject attr) {
        return !(attr instanceof PyReflectedField
                || attr instanceof PyReflectedFunction || attr instanceof PyBeanEventProperty);
    }
    ...
    void type___setattr__(String name, PyObject value) {
        PyObject field = lookup(name);// If we have a static field that takes
                                      // this, go with that
        if (field != null) {
    ...
    void handleMroError(MROMergeState[] toMerge, List<PyObject> mro) {
        ...
        for (MROMergeState mergee : toMerge) {
            for (int i = mergee.next; i < mergee.mro.length; i++) {
                ...
                if (winner == null) {
                    // Pick an arbitrary class to be added to the mro next and
                    // break the conflict.
                    // If method name conflicts were allowed between methods
                    // added to Java types,
                    // it would go first, but that's prevented, so being a
                    // winner doesn't actually
                    // get it anything
        ...
        PyJavaType[] conflictedAttributes = inConflict
                .toArray(new PyJavaType[inConflict.size()]);
        for (PyJavaType type : conflictedAttributes) {
            if (type.modified == null) {
                continue;
            }
            for (String method : type.modified) {
                if (!allModified.add(method)) { // Another type in conflict has
                                                // this method, fail
                    PyList types = new PyList();
                    for (PyJavaType othertype : conflictedAttributes) {
                        if (othertype.modified != null
                                && othertype.modified.contains(method)) {
                            types.add(othertype);
                        }
                    }
                    throw Py.TypeError(String
                            .format("Supertypes that share a modified attribute "
                                    + " have an MRO conflict[attribute=%s, types=%s]",
                                    method, types));
                }
            }
        }

        // We can keep trucking, there aren't any existing method name
        // conflicts. Mark the
        // conflicts in all the classes so further method additions can check
        // for trouble
        for (PyJavaType type : conflictedAttributes) {
    ...
    protected void init(Class<?> forClass, Set<PyJavaType> needsInners) {
        ...
        if (PyObject.class.isAssignableFrom(forClass)) {
            // Non-exposed subclasses of PyObject use a simple linear mro to
            // PyObject that ignores
            // their interfaces
            ...
            // Wrapped Java types fill in their mro first using all of their
            // interfaces then their
            // super class.
            ...
            for (Class<?> iface : forClass.getInterfaces()) {
                if (iface == PyProxy.class || iface == ClassDictInit.class) {
                    // Don't show the interfaces added by proxy type
                    // construction; otherwise Python
                    // subclasses of proxy types and another Java interface
                    // can't make a consistent
                    // mro
                    continue;
                }
                if (baseClass != null && iface.isAssignableFrom(baseClass)) {
                    // Don't include redundant interfaces. If the redundant
                    // interface has methods
                    // that were combined with methods of the same name from
                    // other interfaces higher
                    // in the hierarchy, adding it here hides the forms from
                    // those interfaces.
                    continue;
                }
                visibleBases.add(PyType.fromClassSkippingInners(iface,
                        needsInners));
            }
            if (javaProxy == Object.class) {
                base = PyType.fromClassSkippingInners(PyObject.class,
                        needsInners);
            } else if (baseClass == null) {
                base = PyType
                        .fromClassSkippingInners(Object.class, needsInners);
            } else if (javaProxy == Class.class) {
                base = PyType
                        .fromClassSkippingInners(PyType.class, needsInners);
            } else {
                base = PyType.fromClassSkippingInners(baseClass, needsInners);
            }
            visibleBases.add(base);
            this.bases = visibleBases
                    .toArray(new PyObject[visibleBases.size()]);
            mro = computeMro();
        }

        // PyReflected* can't call or access anything from non-public classes
        // that aren't in
        // org.python.core
        ...
        if (Options.respectJavaAccessibility) {
            ...
            // Grab all methods on this class and all of its superclasses and
        ...
        boolean isInAwt = name.startsWith("java.awt.")
                && name.indexOf('.', 9) == -1;
        for (Method meth : methods) {
            ...
            // Special case a few troublesome methods in java.awt.*. These
            // methods are all
            // deprecated and interfere too badly with bean properties to be
            // tolerated. This is
            // totally a hack but a lot of code that uses java.awt will break
            // without it.
            ...
            // Now check if this is a bean method, for which it must be an
            // instance method
            ...
            if ((methname.startsWith("add") || methname.startsWith("set"))
                    && methname.endsWith("Listener")
                    && n == 1
                    && meth.getReturnType() == Void.TYPE
                    && EventListener.class.isAssignableFrom(meth
                            .getParameterTypes()[0])) {
            ...
            } else if (methname.startsWith("is") && methname.length() > 2
                    && n == 0 && meth.getReturnType() == Boolean.TYPE) {
                beanPropertyName = methname.substring(2);
            } else if (methname.startsWith("set") && methname.length() > 3
                    && n == 1) {
                beanPropertyName = methname.substring(3);
                get = false;
            }
            if (beanPropertyName != null) {
                beanPropertyName = normalize(StringUtil
                        .decapitalize(beanPropertyName));
                PyBeanProperty prop = props.get(beanPropertyName);
                if (prop == null) {
                    prop = new PyBeanProperty(beanPropertyName, null, null,
                            null);
                ...
                if (get) {
                    ...
                    prop.setMethod = meth;
                    // Needed for readonly properties. Getter will be used
                    // instead
                    // if there is one. Only works if setX method has exactly
                    // one
                    // param, which is the only reasonable case.
                    // XXX: should we issue a warning if setX and getX have
                    // different
                    // types?
                    if (prop.myType == null) {
        ...
        for (Method meth : methods) {
            ...
            PyReflectedFunction reflfunc = (PyReflectedFunction) dict
                    .__finditem__(nmethname);
            if (reflfunc != null) {
                // The superclass method has the same name as one declared on
                // this class, so add
                // the superclass version's arguments
                reflfunc.addMethod(meth);
            } else if (PyReflectedFunction.isPackagedProtected(meth
                    .getDeclaringClass()) && lookup(nmethname) == null) {
                // This method must be a public method from a package protected
                // superclass. It's
                // visible from Java on this class, so do the same for Python
                // here. This is the
                // flipside of what handleSuperMethodArgCollisions does for
        ...
        for (Field field : fields) {
            ...
            if (dict.__finditem__(normalize(fldname)) == null) {
                dict.__setitem__(normalize(fldname),
        ...
        for (PyBeanEvent ev : events.values()) {
            ...
            for (Method meth : ev.eventClass.getMethods()) {
                ...
                dict.__setitem__(methodName, new PyBeanEventProperty(
                        methodName, ev.eventClass, ev.addMethod, meth));
            }
        }
        ...
        for (PyBeanProperty prop : props.values()) {
            ...
            if (prev != null) {
                if (!(prev instanceof PyReflectedField)
                        || !Modifier.isStatic(((PyReflectedField) prev).field
                                .getModifiers())) {
                    // Any methods or non-static fields take precedence over the
                    // bean property
                    continue;
                } else {
            ...
            // If one of our superclasses has something defined for this name,
            // check if its a bean
            // property, and if so, try to fill in any gaps in our property from
            // there
            ...
            if (superForName instanceof PyBeanProperty) {
                ...
                // If it has a set method and we don't, take it regardless. If
                // the types don't line
                // up, it'll be rejected below
                if (prop.setMethod == null) {
                    prop.setMethod = superProp.setMethod;
                } else if (prop.getMethod == null
                        && superProp.myType == prop.setMethod
                                .getParameterTypes()[0]) {
                    // Only take a get method if the type on it agrees with the
                    // set method
                    // we already have. The bean on this type overrides a
                    // conflicting one
                    // of the parent
                ...
                if (prop.field == null) {
                    // If the parent bean is hiding a static field, we need it
                    // as well.
                ...
                // It came from a type which is not @this; it came from a
                // superclass
                // It is not a bean event
                // Do not override methods defined in superclass
                continue;
            }
            // If the return types on the set and get methods for a property
            // don't agree, the get
        ...
        // No matter the security manager, trying to set the constructor on
        // class to accessible
        // blows up
        ...
        for (Map.Entry<Class<?>, PyBuiltinMethod[]> entry : getCollectionProxies()
                .entrySet()) {
        ...
        if (ClassDictInit.class.isAssignableFrom(forClass)
                && forClass != ClassDictInit.class) {
            try {
        ...
        if (forClass == Object.class) {
            ...
            addMethod(new PyBuiltinMethodNarrow("__repr__") {
                ...
                public PyObject __call__() {
                    ...
                    return toString == null ? Py.EmptyString : Py
                            .newString(toString);
                }
            });
        ...
        if (immutableClasses.contains(forClass)) {
            ...
            // __deepcopy__ just works for these objects since it uses
        ...
        if (forClass == Cloneable.class) {
            addMethod(new PyBuiltinMethodNarrow("__copy__") {
                ...
                public PyObject __call__() {
                    ...
                    Method clone;
                    // TODO we could specialize so that for well known objects
                    // like collections. This would avoid needing to use
                    // reflection
                    // in the general case, because Object#clone is protected
                    // (but most subclasses are not).
                    //
                    // Lastly we can potentially cache the method handle in the
                    // proxy instead of looking it up each time
                    try {
     ...
     * interfaces or extend public classes can't have their implementations of
     * the methods of their supertypes called through reflection due to Sun VM
     * bug 4071957(http://tinyurl.com/le9vo). They can be called through the
     * supertype version of the method though. Unfortunately we can't just let
     * normal mro lookup of those methods handle routing the call to the correct
     * version as a class can implement interfaces or classes that each have
     * methods with the same name that takes different number or types of
     * arguments. Instead this method goes through all interfaces implemented by
     * this class, and combines same-named methods into a single
     * PyReflectedFunction.
     * 
     * Prior to Jython 2.5, this was handled in PyJavaClass.setMethods by
     * setting methods in package protected classes accessible which made them
     * callable through reflection. That had the drawback of failing when
     * running in a security environment that didn't allow setting
    ...
    private void handleSuperMethodArgCollisions(Class<?> forClass) {
        ...
        if (forClass.getSuperclass() != null) {
            ...
            if (!Modifier.isPublic(forClass.getSuperclass().getModifiers())) {
                // If the superclass is also not public, it needs to get the
                // same treatment as we
    ...
    private void mergeMethods(Class<?> parent) {
        for (Method meth : parent.getMethods()) {
            if (!Modifier.isPublic(meth.getDeclaringClass().getModifiers())) {
                // Ignore methods from non-public interfaces as they're
                // similarly bugged
                continue;
            }
            String nmethname = normalize(meth.getName());
            ...
            if (obj == null) {
                // Nothing in our supertype hierarchy defines something with
                // this name, so it
                // must not be visible there.
                continue;
            } else if (where[0] == this) {
                // This method is the only thing defining items in this class'
                // dict, so it must
                // be a PyReflectedFunction created here. See if it needs the
                // current method
                // added to it.
                ...
                // There's something in a superclass with the same name. Add an
                // item to this type's
                // dict to hide it. If it's this method, nothing's changed. If
                // it's a field, we
                // want to make the method visible. If it's a different method,
                // it'll be added to
    ...
    private static boolean declaredOnMember(Class<?> base, Member declaring) {
        return base == null
                || (declaring.getDeclaringClass() != base && base
                        .isAssignableFrom(declaring.getDeclaringClass()));
    }
    ...
    private static Method getDescrMethod(Class<?> c, String name,
            Class<?>... parmtypes) {
        Method meth;
        try {
        ...
        if (!Modifier.isStatic(meth.getModifiers())
                && meth.getDeclaringClass() != PyObject.class) {
            return meth;
        }
        return null;
    }
    ...
    private static class EnumerationIter extends PyIterator {
        ...
        public PyObject __iternext__() {
            return proxy.hasMoreElements() ? Py.java2py(proxy.nextElement())
                    : null;
        }
    }
    ...
    private static abstract class ComparableMethod extends
        ...
        public PyObject __call__(PyObject arg) {
            ...
            int compare;
            try {
                compare = ((Comparable<Object>) self.getJavaProxy())
                        .compareTo(asjava);
    ...
    private static Map<Class<?>, PyBuiltinMethod[]> getCollectionProxies() {
        if (collectionProxies == null) {
            ...
            PyBuiltinMethodNarrow iterableProxy = new PyBuiltinMethodNarrow(
                    "__iter__") {
                @Override
                public PyObject __call__() {
            ...
            collectionProxies.put(Iterable.class,
                    new PyBuiltinMethod[] { iterableProxy });

            PyBuiltinMethodNarrow lenProxy = new PyBuiltinMethodNarrow(
                    "__len__") {
                @Override
                public PyObject __call__() {
                    return Py.newInteger(((Collection<?>) self.getJavaProxy())
                            .size());
                }
            };

            PyBuiltinMethodNarrow containsProxy = new PyBuiltinMethodNarrow(
                    "__contains__") {
                @Override
                public PyObject __call__(PyObject obj) {
                    Object other = obj.__tojava__(Object.class);
                    boolean contained = ((Collection<?>) self.getJavaProxy())
                            .contains(other);
                    return contained ? Py.True : Py.False;
                }
            };
            collectionProxies.put(Collection.class, new PyBuiltinMethod[] {
                    lenProxy, containsProxy });

            PyBuiltinMethodNarrow iteratorProxy = new PyBuiltinMethodNarrow(
                    "__iter__") {
                @Override
                public PyObject __call__() {
            ...
            collectionProxies.put(Iterator.class,
                    new PyBuiltinMethod[] { iteratorProxy });

            PyBuiltinMethodNarrow enumerationProxy = new PyBuiltinMethodNarrow(
                    "__iter__") {
                @Override
                public PyObject __call__() {
                    return new EnumerationIter(
                            ((Enumeration) self.getJavaProxy()));
                }
            };
            collectionProxies.put(Enumeration.class,
                    new PyBuiltinMethod[] { enumerationProxy });

            // Map doesn't extend Collection, so it needs its own version of
            // len, iter and contains
            ...
            PyBuiltinMethodNarrow mapContainsProxy = new MapMethod(
                    "__contains__", 1) {
                @Override
                public PyObject __call__(PyObject obj) {
                    Object other = obj.__tojava__(Object.class);
            ...
            PyBuiltinMethodNarrow mapGetProxy = new MapMethod("__getitem__", 1) {
                ...
                public PyObject __call__(PyObject key) {
                    return Py
                            .java2py(asMap().get(Py.tojava(key, Object.class)));
                }
            };
            ...
            PyBuiltinMethodNarrow mapRemoveProxy = new MapMethod("__delitem__",
                    1) {
                @Override
                public PyObject __call__(PyObject key) {
                    return Py.java2py(asMap().remove(
                            Py.tojava(key, Object.class)));
                }
            };
            collectionProxies.put(Map.class, new PyBuiltinMethod[] {
                    mapLenProxy, mapIterProxy, mapContainsProxy, mapGetProxy,
                    mapPutProxy, mapRemoveProxy });

            PyBuiltinMethodNarrow listGetProxy = new ListMethod("__getitem__",
                    1) {
                @Override
                public PyObject __call__(PyObject key) {
                    return new ListIndexDelegate(asList())
                            .checkIdxAndGetItem(key);
                }
            };
            PyBuiltinMethodNarrow listSetProxy = new ListMethod("__setitem__",
                    2) {
                @Override
                public PyObject __call__(PyObject key, PyObject value) {
                    new ListIndexDelegate(asList()).checkIdxAndSetItem(key,
                            value);
                    return Py.None;
                }
            };
            PyBuiltinMethodNarrow listRemoveProxy = new ListMethod(
                    "__delitem__", 1) {
                @Override
                public PyObject __call__(PyObject key) {
            ...
            collectionProxies.put(List.class, new PyBuiltinMethod[] {
                    listGetProxy, listSetProxy, listRemoveProxy });
        }
    ...
    protected static class ListIndexDelegate extends SequenceIndexDelegate {
        ...
        public void setSlice(int start, int stop, int step, PyObject value) {
            ...
            if (value.javaProxy == this.list) {
                ...
                if (valueList != null && valueList != Py.NoConversion) {
                    ...
                    setsliceIterator(start, stop, step, value.asIterable()
                            .iterator());
                }
            }
        }

        final private void setsliceList(int start, int stop, int step,
                List<Object> value) {
            if (step == 1) {
        ...
        final private void setsliceIterator(int start, int stop, int step,
        ...
        final private void setslicePyList(int start, int stop, int step,
                PyList value) {
            if (step == 1) {
                list.subList(start, stop).clear();
                int n = value.getList().size();
                for (int i = 0, j = start; i < n; i++, j++) {
                    Object item = value.getList().get(i)
