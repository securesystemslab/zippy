...
final class InternalFormatter {
    ...
    private void checkPrecision(String type) {
        if (precision > 250) {
            ...
            throw Py.OverflowError("formatted " + type
                    + " is too long (precision too long?)");
        }
    ...
    public String format(double value) {
        ...
        if (Double.isNaN(value)) {
            ...
            case 'g':
            case 'G':
                int exponent = (int) ExtraMath.closeFloor(Math.log10(Math
                        .abs(value == 0 ? 1 : value)));
                ...
                if (exponent >= -4 && exponent < precision) {
                    ...
                    // Exponential precision is the number of digits after the
                    // decimal
                    // point, whereas 'g' precision is the number of significant
                    // digits --
                    // and exponential always provides one significant digit
                    // before the
                    // decimal point
                    precision--;
                    string = formatFloatExponential(value,
                            (char) (spec.type - 2), !spec.alternate);
                }
                if (spec.type == 'G') {
                    string = string.toUpperCase();
                }
