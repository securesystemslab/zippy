...
public class MarkupIterator extends PyObject {
    ...
    final PyObject formatteriterator___iternext__() {
        ...
        elements[1] = chunk.fieldName.length() == 0 ? Py.None : new PyString(
                chunk.fieldName);
        if (chunk.fieldName.length() > 0) {
            elements[2] = chunk.formatSpec == null ? Py.EmptyString
                    : new PyString(chunk.formatSpec);
        } else {
            elements[2] = Py.None;
        }
        elements[3] = chunk.conversion == null ? Py.None : new PyString(
                chunk.conversion);
