...
public class PyList extends PySequenceList implements List {
    ...
    final void list___init__(PyObject[] args, String[] kwds) {
        ArgParser ap = new ArgParser("list", args, kwds,
                new String[] { "sequence" }, 0);
    ...
    final synchronized PyObject list___getslice__(PyObject start,
            PyObject stop, PyObject step) {
        return seq___getslice__(start, stop, step);
    }

    @ExposedMethod(defaults = "null", doc = BuiltinDocs.list___setslice___doc)
    final synchronized void list___setslice__(PyObject start, PyObject stop,
    ...
    final synchronized void list___delslice__(PyObject start, PyObject stop,
            PyObject step) {
     ...
     * arecompared with the <code>==</code> operator. <br>
    ...
    /**
     * Reverses the items of s in place. The reverse() methods modify the list
     * in place for economy of space when reversing a large list. It doesn't
     * return the reversed list to remind you of this side effect.
     */
    public void reverse() {
        list_reverse();
    }
    ...
    /**
     * Sort the items of the list in place. The compare argument is a function
     * of two arguments (list items) which should return -1, 0 or 1 depending on
     * whether the first argument is considered smaller than, equal to, or
     * larger than the second argument. Note that this slows the sorting process
     * down considerably; e.g. to sort a list in reverse order it is much faster
     * to use calls to the methods sort() and reverse() than to use the built-in
     * function sort() with a comparison function that reverses the ordering of
     * the elements.
     * 
     * @param compare
    ...
    final synchronized void list_sort(PyObject[] args, String[] kwds) {
        ArgParser ap = new ArgParser("list", args, kwds, new String[] { "cmp",
                "key", "reverse" }, 0);
    ...
    // a bunch of optimized paths for sort to avoid unnecessary work, such as
    // DSU or checking compare functions for null
    ...
    private synchronized void sort(boolean reverse) {
        ...
        if (reverse) {
            Collections.reverse(list); // maintain stability of sort by
                                       // reversing first
        }
    ...
    private static class PyObjectDefaultComparator implements
            Comparator<PyObject> {
    ...
    private synchronized void sort(PyObject cmp, PyObject key, boolean reverse) {
        ...
        if (reverse) {
            Collections.reverse(decorated); // maintain stability of sort by
                                            // reversing first
        }
    ...
    public ListIterator listIterator(final int index) {
        return new ListIterator() {
            ...
            private final ListIterator<PyObject> iter = list
                    .listIterator(index);
