 ...
 * compiled, package and module items in the right order, and creating and
 * filling in modules.
 */
public abstract class importer<T> extends PyObject {
    ...
    /**
     * Returns the separator between directories and files used by this type of
     * importer.
     */
    protected abstract String getSeparator();

    /**
     * Returns the value to fill in __path__ on a module with the given full
     * module name created by this importer.
     */
     ...
     * such that this importer can make a bundle with it, or null if
     * fullFilename doesn't exist in this importer.
     */
    protected abstract T makeEntry(String filenameAndSuffix);

    /**
     * Returns a Bundle for fullFilename and entry, the result from a makeEntry
     * call for fullFilename.
     */
    ...
    private SearchOrderEntry[] makeSearchOrder() {
        return new SearchOrderEntry[] {
                ...
                new SearchOrderEntry("$py.class",
                        EnumSet.of(EntryType.IS_BYTECODE)),
    ...
    protected final PyObject importer_find_module(String fullname, String path) {
        ...
        if (moduleInfo == ModuleInfo.ERROR
                || moduleInfo == ModuleInfo.NOT_FOUND) {
            return Py.None;
        }
        return this;
    }
    ...
    protected final PyObject importer_load_module(String fullname) {
        ...
        // the module *must* be in sys.modules before the loader executes the
        // module code; the
        ...
        Py.writeDebug("import", "import " + fullname + " # loaded from "
                + moduleCodeData.path);
        return mod;
    }

    /**
     * Bundle is an InputStream, bundled together with a method that can close
     * the input stream and whatever resources are associated with it when the
    ...
    protected abstract static class Bundle {
        ...
        /**
         * Close the underlying resource if necessary. Raises an IOError if a
         * problem occurs.
         */
    ...
    /**
     * Given a path to a compiled file in the archive, return the modification
     * time of the matching .py file.
     * 
     * @param path
    ...
    /**
     * Return the code object and its associated data for the module with the
    ...
    protected final ModuleCodeData getModuleCode(String fullname) {
        ...
        for (SearchOrderEntry entry : searchOrder) {
            ...
            try {
                if (isBytecode) {
                    try {
                        codeBytes = imp.readCode(fullname, bundle.inputStream,
                                true, mtime);
                    } catch (IOException ioe) {
                        throw Py.ImportError(ioe.getMessage() + "[path="
                                + fullSearchPath + "]");
                    }
                    if (codeBytes == null) {
                        // bad magic number or non-matching mtime in byte code,
                        // try next
                        continue;
                    }
                } else {
                    codeBytes = imp.compileSource(fullname, bundle.inputStream,
                            fullSearchPath);
                }
            } finally {
                bundle.close();
            ...
            PyCode code = BytecodeLoader.makeCode(fullname + "$py", codeBytes,
                    fullSearchPath);
    ...
    /**
     * Container for PyModule code - whether or not it's a package - and its
     * path.
     */
