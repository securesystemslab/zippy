...
public class ParserFacade {
    ...
    private static String getLine(ExpectedEncodingBufferedReader reader,
            int line) {
        if (reader == null) {
            return "";
        }
    ...
    public static PyException fixParseError(
            ExpectedEncodingBufferedReader reader, Throwable t, String filename) {
        if (reader != null) {
            try {
                reader.reset();
            } catch (IOException e) {
        ...
        if (t instanceof ParseException) {
            ...
            if (reader.encoding == null) {
                ...
                msg = "Illegal character in file '" + filename
                        + "' for encoding '" + reader.encoding + "'";
            }
     ...
     * distinguish between scripts which return values and those which do not,
     * nor do they make the corresponding distinction between evaluating or
     * executing objects." (SCR.4.2.1)
     */
    public static mod parseExpressionOrModule(Reader reader, String filename,
    ...
    private static mod parse(ExpectedEncodingBufferedReader reader,
            CompileMode kind, String filename, CompilerFlags cflags)
            throws Throwable {
    ...
    public static mod parse(Reader reader, CompileMode kind, String filename,
            CompilerFlags cflags) {
        ExpectedEncodingBufferedReader bufReader = null;
        try {
            bufReader = prepBufReader(reader, cflags, filename);
    ...
    public static mod parse(InputStream stream, CompileMode kind,
    ...
    public static mod parse(String string, CompileMode kind, String filename,
            CompilerFlags cflags) {
        ExpectedEncodingBufferedReader bufReader = null;
        try {
    ...
    public static mod partialParse(String string, CompileMode kind,
    ...
    private static boolean validPartialSentence(BufferedReader bufreader,
            CompileMode kind, String filename) {
    ...
    private static class ExpectedEncodingBufferedReader extends BufferedReader {
        ...
        /**
         * The encoding from the source file, or null if none was specified and
         * ascii is being used.
         */
    ...
    private static ExpectedEncodingBufferedReader prepBufReader(Reader reader,
            CompilerFlags cflags, String filename) throws IOException {
    ...
    private static ExpectedEncodingBufferedReader prepBufReader(
            InputStream input, CompilerFlags cflags, String filename,
            boolean fromString) throws IOException {
        return prepBufReader(input, cflags, filename, fromString, true);
    }

    private static ExpectedEncodingBufferedReader prepBufReader(
            InputStream input, CompilerFlags cflags, String filename,
            boolean fromString, boolean universalNewlines) throws IOException {
        ...
        if (universalNewlines) {
            ...
            org.python.core.io.BufferedReader bufferedIO = new org.python.core.io.BufferedReader(
                    rawIO, 0);
        ...
        try {
            ...
            throw new PySyntaxError("Unknown encoding: " + encoding, 1, 0, "",
                    filename);
        }
        CharsetDecoder dec = cs.newDecoder();
        dec.onMalformedInput(CodingErrorAction.REPORT);
        dec.onUnmappableCharacter(CodingErrorAction.REPORT);
        return new ExpectedEncodingBufferedReader(new InputStreamReader(input,
                dec), encoding);
    }
    ...
    private static ExpectedEncodingBufferedReader prepBufReader(String string,
        ...
        return prepBufReader(new ByteArrayInputStream(stringBytes), cflags,
                filename, true, false);
    }

    /**
     * Check for a BOM mark at the beginning of stream. If there is a BOM mark,
     * advance the stream passed it. If not, reset() to start at the beginning
     * of the stream again.
     * 
    ...
    private static String readEncoding(InputStream stream) throws IOException {
        ...
        // XXX: reset() can still raise an IOException if a line exceeds our
        // large mark
        // limit
        stream.reset();
     ...
     * declaration.
     * 
     * Note that reseting the reader (if needed) is responsibility of the
     * caller.
     * 
    ...
    private static final Pattern pep263EncodingPattern = Pattern
            .compile("#.*coding[:=]\\s*([-\\w.]+)");
