...
public class PyFile extends PyObject {
    ...
    /**
     * Indicator dictating whether a space should be written to this file on the
     * next print statement (not currently implemented in print )
     */
     ...
     * builtin method <code>file</code> doesn't expose this functionality (
     * <code>open</code> does albeit deprecated) as it isn't available to
     * regular Python code. To wrap an InputStream in a file from Python, use
    ...
    public PyFile(InputStream istream, int bufsize) {
        this(istream, "<Java InputStream '" + istream + "' as file>", "r",
                bufsize, true);
    }
    ...
    PyFile(OutputStream ostream, String name, String mode, int bufsize,
            boolean closefd) {
        parseMode(mode);
     ...
     * builtin method <code>file</code> doesn't expose this functionality (
     * <code>open</code> does albeit deprecated) as it isn't available to
     * regular Python code. To wrap an OutputStream in a file from Python, use
    ...
    public PyFile(OutputStream ostream, int bufsize) {
        this(ostream, "<Java OutputStream '" + ostream + "' as file>", "w",
                bufsize, true);
    }
    ...
    final void file___init__(PyObject[] args, String[] kwds) {
        ...
        if (!(name instanceof PyString)) {
            throw Py.TypeError("coercing to Unicode: need string, '"
                    + name.getType().fastGetName() + "' type found");
        }
        String mode = ap.getString(1, "r");
        int bufsize = ap.getInt(2, -1);
        file___init__(new FileIO((PyString) name, parseMode(mode)), name, mode,
                bufsize);
        closer = new Closer(file, Py.getSystemState());
    }

    private void file___init__(RawIOBase raw, String name, String mode,
            int bufsize) {
        file___init__(raw, new PyString(name), mode, bufsize);
    }

    private void file___init__(RawIOBase raw, PyObject name, String mode,
            int bufsize) {
        this.name = name;
        this.mode = mode;
    ...
    /**
     * Wrap the given RawIOBase with a BufferedIOBase according to the mode and
     * given bufsize.
     * 
     * @param raw
     *            a RawIOBase value
     * @param bufsize
     *            an int size of the buffer
    ...
    private BufferedIOBase createBuffer(RawIOBase raw, int bufsize) {
        ...
        if (updating) {
            buffer = lineBuffered ? new LineBufferedRandom(raw)
                    : new BufferedRandom(raw, bufsize);
        } else if (writing || appending) {
            buffer = lineBuffered ? new LineBufferedWriter(raw)
                    : new BufferedWriter(raw, bufsize);
        } else if (reading) {
            // Line buffering is for output only
            buffer = new BufferedReader(raw,
                    lineBuffered ? IOBase.DEFAULT_BUFFER_SIZE : bufsize);
        } else {
            // Should never happen
    ...
    final synchronized void file_writelines(PyObject lines) {
        checkClosed();
        PyObject iter = Py.iter(lines,
                "writelines() requires an iterable argument");
        for (PyObject item = null; (item = iter.__iternext__()) != null;) {
            softspace = false;
            file.write(asWritable(item,
    ...
    private String asWritable(PyObject obj, String message) {
        ...
        if (message == null) {
            message = String.format(
                    "argument 1 must be string or %sbuffer, not %.200s",
                    binary ? "" : "read-only character ", obj.getType()
                            .fastGetName());
        }
    ...
    final String file_toString() {
        ...
        if (name instanceof PyUnicode) {
            String escapedName = PyString.encode_UnicodeEscape(name.toString(),
                    false);
            return String.format("<%s file u'%s', mode '%s' at %s>", state,
                    escapedName, mode, id);
        }
        return String.format("<%s file '%s', mode '%s' at %s>", state, name,
                mode, id);
    }
     ...
     * creation Closer adds itself to a list of Closers that will be run by
     * PyFileCloser on JVM shutdown. When a PyFile's close or finalize methods
     * are called, PyFile calls its Closer.close which clears Closer out of the
     * shutdown queue.
     * 
     * We use a regular object here rather than WeakReferences and their ilk as
     * they may be collected before the shutdown hook runs. There's no guarantee
     * that finalize will be called during shutdown, so we can't use it. It's
     * vital that this Closer has no reference to the PyFile it's closing so the
     * PyFile remains garbage collectable.
     */
