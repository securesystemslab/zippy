 ...
 * be called, if any.
 */
public abstract class ExposedMethodFinder extends MethodVisitor implements
        PyTypes, Opcodes {
    ...
    public ExposedMethodFinder(String typeName, Type onType, int access,
    ...
    public AnnotationVisitor visitAnnotation(String desc, boolean visible) {
        if (desc.equals(EXPOSED_NEW.getDescriptor())) {
            if ((access & ACC_STATIC) != 0) {
                newExp = new NewExposer(onType, access, methodName, methodDesc,
                        exceptions);
            } else {
                newExp = new OverridableNewExposer(onType, Type.getType("L"
                        + onType.getInternalName() + "Derived;"), access,
                        methodName, methodDesc, exceptions);
            }
    ...
    private abstract class StringArrayBuilder extends
    ...
    public void visitEnd() {
        if (methVisitor != null) {
            handleResult(new InstanceMethodExposer(onType, access, methodName,
                    methodDesc, typeName, methVisitor.names,
                    methVisitor.defaults, methVisitor.type, methVisitor.doc));
        }
        if (newExp != null) {
            handleNewExposer(newExp);
        }
        if (classMethVisitor != null) {
            handleResult(new ClassMethodExposer(onType, access, methodName,
                    methodDesc, typeName, classMethVisitor.names,
                    classMethVisitor.defaults, classMethVisitor.doc));
        }
        super.visitEnd();
    }
}
