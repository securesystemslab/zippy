...
public abstract class MethodExposer extends Exposer {
    ...
    public MethodExposer(Type onType, String methodName, Type[] args,
            Type returnType, String typeName, String[] asNames,
            String[] defaults, Class superClass, String doc) {
    ...
    protected void throwInvalid(String msg) {
        throw new InvalidExposingException(msg + "[method="
    ...
    private void generateFullConstructor() {
        ...
        mv.visitVarInsn(ALOAD, 0);
        mv.visitLdcInsn(doc);
        mv.visitFieldInsn(PUTFIELD, BUILTIN_FUNCTION.getInternalName(), "doc",
    ...
    private void generateCall(int numDefaults) {
        ...
        int offset = needsThreadState ? 1 : 0;
        // We always have one used local for self, and possibly one for
        // ThreadState
        ...
        loadSelfAndThreadState();
        // Push the passed in callArgs onto the stack, and convert them if
        // necessary
        int i;
    ...
    private void generateCallNoThreadState(Type[] callArgs) {
        ...
        System.arraycopy(callArgs, 1, noThreadStateArgs, 0,
                noThreadStateArgs.length);
    ...
    private void pushDefault(String def, Type arg) {
        if (def.equals("Py.None")) {
            ...
            // For primitive types, parse using the Java wrapper for that type
            // and push onto the
            // stack as a constant. If the default is malformed, a
            ...
            // byte, char, boolean and short go as int constants onto the stack,
            // so convert them
    ...
    protected static boolean isWide(Type[] args) {
        ...
        return args.length == 2 + offset && args[offset].equals(APYOBJ)
                && args[offset + 1].equals(ASTRING);
    }
