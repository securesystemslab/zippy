...
public class DescriptorExposer extends Exposer {
    ...
    private String getterMethodName, getterFieldName, setterMethodName,
            setterFieldName, deleterMethodName;
    ...
    public DescriptorExposer(Type onType, String descrName) {
        super(PyDataDescr.class, onType.getClassName() + "$" + descrName
                + "_descriptor", ASSUPER);
        this.onType = onType;
        name = descrName;
    }
    ...
    public String toString() {
        return "DescriptorExposer[class=" + onType.getClassName() + ", name="
                + name + "]";
    }
    ...
    protected void generate() {
        ...
        generateImplement("Get", getterMethodName != null
        ...
        generateImplement("Set", setterMethodName != null
    ...
    private void generateFieldGetter() {
        ...
        mv.visitVarInsn(ALOAD, 1);
        mv.visitTypeInsn(CHECKCAST, onType.getInternalName());
        mv.visitFieldInsn(GETFIELD, onType.getInternalName(), getterFieldName,
    ...
    private void generateMethodSetter() {
        ...
        if (PRIMITIVES.containsKey(ofType)) {
            mv.visitTypeInsn(CHECKCAST, PRIMITIVES.get(ofType)
                    .getInternalName());
            call(PRIMITIVES.get(ofType), ofType.getClassName() + "Value",
                    ofType);
        } else {
    ...
    private void generateFieldSetter() {
        ...
        mv.visitFieldInsn(PUTFIELD, onType.getInternalName(), setterFieldName,
                ofType.getDescriptor());
        endMethod(RETURN);
    }
    ...
    private void error(String reason) {
        throw new InvalidExposingException(reason + "[class="
                + onType.getClassName() + ", name=" + name + "]");
    }
}
