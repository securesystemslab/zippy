...
public class ClassMethodExposer extends MethodExposer {
    ...
    public ClassMethodExposer(Type onType, int access, String methodName,
            String desc, String typeName, String[] asNames, String[] defaults,
            String doc) {
        super(onType, methodName, getArgs(onType, methodName, desc), Type
                .getReturnType(desc), typeName, asNames, defaults,
                PyBuiltinClassMethodNarrow.class, doc);
    ...
    private static Type[] getArgs(Type onType, String methodName, String desc) {
        ...
        if (args.length == offset || !args[offset].equals(PYTYPE)) {
            String msg = String.format(
                    "ExposedClassMethod's first argument %smust be "
                            + "PyType[method=%s.%s]",
                    needsThreadState ? "(following ThreadState) " : "", onType
                            .getClassName(), methodName);
            throw new InvalidExposingException(msg);
        }

        // Remove PyType from the exposed __call__'s args, it'll be already
        // bound as self
    ...
    protected void loadSelfAndThreadState() {
        // ThreadState precedes self for ClassMethods, load it first if
        // necessary
        loadThreadState();
