 ...
 * another directory, possibly the same one.
 */
public class JycompileAntTask extends GlobMatchingTask {
     ...
     * <code>moduleName</code> as its name, and stores it in
     * <code>compiled</code>. This is called by process for every file that's
     * compiled, so subclasses can override this method to affect or track the
     * compilation.
     */
    protected void compile(File src, File compiled, String moduleName) {
        ...
        if (!dir.exists() && !compiled.getParentFile().mkdirs()) {
            throw new BuildException(
                    "Unable to make directory for compiled file: " + compiled);
        }
        imp.cacheCompiledSource(src.getAbsolutePath(),
                compiled.getAbsolutePath(), bytes);
    }
