 ...
 * <code>javax.servlet.Filter</code>, and to insert them in your servlet
 * container's filter chain, like any Java <code>Filter</code>.
 * 
 * <p>
 * Example:
 * 
    ...
    public void init(FilterConfig config) throws ServletException {
        if (config.getServletContext().getAttribute(PyServlet.INIT_ATTR) == null) {
            throw new ServletException(
                    "Jython has not been initialized.  Add "
        ...
        if (filterPath == null) {
            throw new ServletException("Missing required param '"
                    + FILTER_PATH_PARAM + "'");
        }
        source = new File(getRealPath(config.getServletContext(), filterPath));
        if (!source.exists()) {
            throw new ServletException(source.getAbsolutePath()
                    + " does not exist.");
        }
