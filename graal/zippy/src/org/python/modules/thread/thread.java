...
public class thread implements ClassDictInit {
    ...
    private static volatile long stack_size = 0; // XXX - can we figure out the
                                                 // current stack size?
    ...
    public static long stack_size(PyObject[] args) {
        ...
        case 1:
            ...
            if (proposed_stack_size != 0 && proposed_stack_size < 32768) {
                ...
                throw Py.ValueError("size not valid: " + proposed_stack_size
                        + " bytes");
            }
            stack_size = proposed_stack_size;
            return old_stack_size;
        default:
            throw Py.TypeError("stack_size() takes at most 1 argument ("
                    + args.length + "given)");
        }
    }
}
