...
public class PosixModule implements ClassDictInit {
    ...
    public static void chdir(PyObject path) {
        ...
        Py.getSystemState().setCurrentWorkingDir(
                realpath.__call__(path).asString());
    }
    ...
    public static PyList listdir(PyObject path) {
        ...
        if (names == null) {
            // Can't read the path for some reason. stat will throw an error if
            // it can't
            // read it either
    ...
    public static FileIO open(PyObject path, int flag, int mode) {
        ...
        String fileIOMode = (reading ? "r" : "")
                + (!appending && writing ? "w" : "")
                + (appending && (writing || updating) ? "a" : "")
                + (updating ? "+" : "");
        if (sync && (writing || updating)) {
            try {
                ...
                throw Py.OSError(file.isDirectory() ? Errno.EISDIR
                        : Errno.ENOENT, path);
            }
        }
    ...
    public static PyObject popen(PyObject[] args, String[] kwds) {
        // XXX: popen lives in posix in 2.x, but moves to os in 3.x. It's easier
        // for us to
        // keep it in os
    ...
    public static PyObject read(PyObject fd, int buffersize) {
        try {
            return new PyString(StringUtil.fromBytes(FileDescriptors.get(fd)
                    .read(buffersize)));
        } catch (PyException pye) {
            throw badFD();
        }
    ...
    public static void rename(PyObject oldpath, PyObject newpath) {
        if (!new File(absolutePath(oldpath)).renameTo(new File(
                absolutePath(newpath)))) {
            PyObject args = new PyTuple(Py.Zero, new PyString(
    ...
    public static void rmdir(PyObject path) {
        ...
        if (!file.exists()) {
            ...
            PyObject args = new PyTuple(Py.Zero, new PyString(
                    "Couldn't delete directory"), path);
            throw new PyException(Py.OSError, args);
        }
    }
    ...
    public static PyObject strerror(int code) {
        ...
        if (errno.name() == errno.toString()) {
            ...
            errno = Enum.valueOf(
                    com.kenai.constantine.platform.linux.Errno.class,
                    errno.name());
        }
    ...
    public static PyObject system(PyObject command) {
        ...
        return imp.load("subprocess").invoke("call", command,
                new PyObject[] { Py.True }, new String[] { "shell" });
    }
    ...
    /**
     * Convert seconds (with a possible fraction) from epoch to a 2 item array
     * of seconds, microseconds from epoch as longs.
     * 
     * @param seconds
     *            a PyObject number
    ...
    private static long[] extractTimeval(PyObject seconds) {
        long[] timeval = new long[] {
                Platform.IS_32_BIT ? seconds.asInt() : seconds.asLong(), 0L };
    ...
    public static int write(PyObject fd, String string) {
        try {
            return FileDescriptors.get(fd).write(
     ...
     * commands for this OS.
     * 
     * @return a tuple of lists of command line arguments. E.g. (['/bin/sh',
     *         '-c'])
     */
    ...
    /**
     * Initialize the environ dict from System.getenv. environ may be empty when
     * the security policy doesn't grant us access.
     */
    private static PyObject getEnviron() {
        ...
        for (Map.Entry<String, String> entry : env.entrySet()) {
            environ.__setitem__(Py.newString(entry.getKey()),
                    Py.newString(entry.getValue()));
        }
        return environ;
    }
     ...
     * 
     * @param path
     *            a PyObject, raising a TypeError if an invalid path type
    ...
    private static String asPath(PyObject path) {
        ...
        throw Py.TypeError(String.format(
                "coercing to Unicode: need string, %s type found", path
                        .getType().fastGetName()));
    }
