...
public class Structure extends CData implements Pointer {
    ...
    Structure(PyType pyType, StructLayout layout) {
        this(pyType, layout, AllocatedNativeMemory
    ...
    @ExposedNew
    public static PyObject Structure_new(PyNewWrapper new_, boolean init,
    ...
    static final StructLayout getStructLayout(PyType subtype) {
        ...
        if (!(jffi_type instanceof StructLayout)) {
            throw Py.TypeError("invalid _jffi_type for "
                    + subtype.fastGetName()
                    + "; should be instance of jffi.StructLayout");
        }
    ...
    public static final PyObject from_address(PyType subtype, PyObject address) {
        return new Structure(subtype, getStructLayout(subtype),
                Util.getMemoryForAddress(address));
    }
    ...
    StructLayout.Field getField(PyObject key) {
        ...
        if (f == null) {
            throw Py.NameError(String.format("struct %s has no field '%s'",
                    getType().fastGetName(), key.toString()));
        }
        return f;
    }
