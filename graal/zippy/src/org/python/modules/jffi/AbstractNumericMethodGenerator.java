...
abstract class AbstractNumericMethodGenerator implements JITMethodGenerator {
    ...
    public void generate(AsmClassBuilder builder, String functionName,
            JITSignature signature) {
        SkinnyMethodAdapter mv = new SkinnyMethodAdapter(
                builder.getClassVisitor(), ACC_PUBLIC | ACC_FINAL,
                functionName, sig(PyObject.class,
                        params(PyObject.class, signature.getParameterCount())),
                null, null);
    ...
    public void generate(AsmClassBuilder builder, SkinnyMethodAdapter mv,
            JITSignature signature) {
        ...
        mv.getstatic(p(JITInvoker.class), "jffiInvoker",
                ci(com.kenai.jffi.Invoker.class));
        // [ stack now contains: Invoker ]
        mv.aload(0);
        mv.getfield(p(JITInvoker.class), "jffiFunction",
        ...
        for (int i = 0; i < signature.getParameterCount(); ++i) {
            if (signature.hasParameterConverter(i)) {
                mv.aload(0); // this
                mv.getfield(builder.getClassName(),
                        builder.getParameterConverterFieldName(i),
                        ci(NativeDataConverter.class));
                mv.aload(firstParam + i); // PyObject parameter
                mv.invokevirtual(p(NativeDataConverter.class), "toNative",
        ...
        for (int i = 0; i < signature.getParameterCount(); ++i) {
            ...
            case POINTER:
                ...
                mv.aload(paramVar);
                mv.invokestatic(p(JITRuntime.class), "other2ptr",
                        sig(PyObject.class, PyObject.class));
                mv.label(converted);
                mv.dup();
                mv.astore(paramVar);
        ...
        if (maxPointerIndex >= 0) {
            ...
            mv.aload(0);
            mv.getfield(p(JITInvoker.class), "fallbackInvoker",
                    ci(Invoker.class));
    ...
    private void emitResultConversion(SkinnyMethodAdapter mv,
            AsmClassBuilder builder, JITSignature signature) {
        if (signature.hasResultConverter()) {
            mv.aload(0); // [ result, this ]
            mv.getfield(builder.getClassName(),
                    builder.getResultConverterFieldName(),
                    ci(NativeDataConverter.class));
            mv.swap(); // [ converter, result ]
            mv.invokevirtual(p(NativeDataConverter.class), "fromNative",
                    sig(PyObject.class, PyObject.class));
        }
    }
    ...
    private void boxResult(SkinnyMethodAdapter mv, NativeType type) {
        ...
        default:
            throw new UnsupportedOperationException(
                    "native return type not supported: " + type);

        }
    }

    private void unbox(SkinnyMethodAdapter mv, String method) {
        mv.invokestatic(p(JITRuntime.class), getRuntimeMethod(method),
                sig(getInvokerIntType(), PyObject.class));
    }
    ...
    public static boolean isPrimitiveInt(Class c) {
        return byte.class == c || char.class == c || short.class == c
                || int.class == c || boolean.class == c;
    }
    ...
    protected static String[] buildSignatures(Class nativeIntClass,
            int maxParameters) {
