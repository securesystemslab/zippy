...
class NativeMemory implements Memory, DirectMemory {
    protected static final com.kenai.jffi.MemoryIO IO = com.kenai.jffi.MemoryIO
            .getInstance();
    final NativeMemory parent; // keep a reference to avoid the memory being
                               // freed
    final long address;
    ...
    public final boolean equals(Object obj) {
        return (obj instanceof DirectMemory)
                && ((DirectMemory) obj).getAddress() == address;
    }
    ...
    public final long getNativeLong(long offset) {
        return Platform.getPlatform().longSize() == 32 ? IO.getInt(address
                + offset) : IO.getLong(address + offset);
    }
    ...
    public final int indexOf(long offset, byte value) {
        return value == 0 ? (int) IO.getStringLength(address + offset)
