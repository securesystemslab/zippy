...
final class JITHandle {
    ...
    private static final int THRESHOLD = Integer.getInteger(
            "jython.ctypes.compile.threshold", 100);
    ...
    JITHandle(JITCompiler compiler, JITSignature signature,
    ...
    final Invoker compile(com.kenai.jffi.Function function,
            NativeDataConverter resultConverter,
        ...
        synchronized (this) {
            if (compiledClassRef == null
                    || (compiledClass = compiledClassRef.get()) == null) {
                ...
                compiledClassRef = new WeakReference<Class<? extends Invoker>>(
                        compiledClass);
            }
        }

        try {
            Constructor<? extends Invoker> cons = compiledClass
                    .getDeclaredConstructor(com.kenai.jffi.Function.class,
                            NativeDataConverter.class,
                            NativeDataConverter[].class, Invoker.class);
            return cons.newInstance(function, resultConverter,
                    parameterConverters,
    ...
    Class<? extends Invoker> newInvokerClass(JITSignature jitSignature) {
        ...
        JITMethodGenerator[] generators = { new FastIntMethodGenerator(),
    ...
    static Invoker createFallbackInvoker(com.kenai.jffi.Function function,
            JITSignature signature) {
        NativeType[] parameterTypes = new NativeType[signature
                .getParameterCount()];
        for (int i = 0; i < parameterTypes.length; i++) {
            parameterTypes[i] = signature.getParameterType(i);
        }

        return DefaultInvokerFactory.getFactory().createInvoker(function,
                parameterTypes, signature.getResultType());
    }
}
