...
public class DynamicLibrary extends PyObject {
    ...
    DynamicLibrary(String libname, int mode) {
        ...
        if (lib == null) {
            throw Py.RuntimeError("Could not open " + libname != null ? libname
                    : "[current process]" + " " + Library.getLastError());
        }
    }
    ...
    private long findSymbol(PyObject name) {
        ...
        if (address == 0) {
            throw Py.NameError("Could not locate symbol '" + name.asString()
                    + "' in " + this.name);
        }
    ...
    public static final class TextSymbol extends Symbol implements
            ExposeAsSuperclass {
    ...
    public static final class DataSymbol extends Symbol implements
            ExposeAsSuperclass {
