...
public class PointerCData extends AbstractMemoryCData implements Pointer {
    ...
    PointerCData(PyType subtype, CType type, DirectMemory memory,
    ...
    @ExposedNew
    public static PyObject PointerCData_new(PyNewWrapper new_, boolean init,
        ...
        if (args.length == 0) {
            return new PointerCData(subtype, pointerType, NullMemory.INSTANCE,
                    pointerType.componentMemoryOp);
        }

        PyObject value = args[0];
        if (value instanceof CData
                && value.getType().isSubType(pointerType.pyComponentType)) {

            return new PointerCData(subtype, pointerType,
                    ((CData) value).getReferenceMemory(),
                    pointerType.componentMemoryOp);

        } else {
            throw Py.TypeError("expected "
                    + pointerType.pyComponentType.getName() + " instead of "
    ...
    public static final PyObject from_address(PyType subtype, PyObject address) {
        ...
        PointerCData cdata = new PointerCData(subtype, pointerType,
                m.getMemory(0), pointerType.componentMemoryOp);
