...
public class StringCData extends AbstractMemoryCData {
    ...
    @ExposedNew
    public static PyObject StringCData_new(PyNewWrapper new_, boolean init,
            PyType subtype, PyObject[] args, String[] keywords) {

        // No args == create NULL pointer
        if (args.length == 0) {
            return new StringCData(subtype, CType.typeOf(subtype),
                    NullMemory.INSTANCE);
        }
    ...
    public static final PyObject from_address(PyType subtype, PyObject address) {
        ...
        StringCData cdata = new StringCData(subtype, CType.typeOf(subtype),
                m.getMemory(0));
    ...
    public PyObject getValue() {
        Memory m = getMemory();
        return !m.isNull() ? Py.newString(new String(m
                .getZeroTerminatedByteArray(0))) : Py.None;
    }
    ...
    public String asString() {
        Memory m = getMemory();
        return !m.isNull() ? new String(m.getZeroTerminatedByteArray(0)) : null;
    }
