 ...
 * represented as Python strings. It uses <i>format strings</i> (explained
 ...
 * <dd>
 * Return a string containing the values <tt><i>v1</i>, <i>v2</i>,  ...</tt>
 * packed according to the given format. The arguments must match the values
 ...
 *    ...)</tt>) according to the given format. The result is a tuple even if it
 * contains exactly one item. The string must contain exactly the amount of data
 * required by the format (i.e. <tt>len(<i>string</i>)</tt> must equal
 * <tt>calcsize(<i>fmt</i>)</tt>).
 * </dl>
 * 
 * <P>
 * <dl>
 ...
 * 
 * <P>
 * A format character may be preceded by an integral repeat count; e.g. the
 * format string <tt>'4h'</tt> means exactly the same as <tt>'hhhh'</tt>.
 * 
 * <P>
 ...
 * 
 * <P>
 * For the "<tt>s</tt>" format character, the count is interpreted as the size
 * of the string, not a repeat count like for the other format characters; e.g.
 * <tt>'10s'</tt> means a single 10-byte string, while <tt>'10c'</tt> means 10
 * characters. For packing, the string is truncated or padded with null bytes as
 * appropriate to make it fit. For unpacking, the resulting string always has
 * exactly the specified number of bytes. As a special case, <tt>'0s'</tt> means
 * a single, empty string (while <tt>'0c'</tt> means 0 characters).
 * 
 * <P>
 * The "<tt>p</tt>" format character can be used to encode a Pascal string. The
 * first byte is the length of the stored string, with the bytes of the string
 * following. If count is given, it is used as the total number of bytes used,
 * including the length byte. If the string passed in to <tt>pack()</tt> is too
 * long, the stored representation is truncated. If the string is too short,
 * padding is used to ensure that exactly enough bytes are used to satisfy the
 * count.
 * 
 * <P>
 * For the "<tt>I</tt>" and "<tt>L</tt>" format characters, the return value is
 ...
 * 
 * <P>
 * Native size and alignment are defined as follows: <tt>short</tt> is 2 bytes;
 * <tt>int</tt> and <tt>long</tt> are 4 bytes; <tt>float</tt> are 4 bytes and
 * <tt>double</tt> are 8 bytes. Native byte order is chosen as big-endian.
 * 
 * <P>
 * Standard size and alignment are as follows: no alignment is required for any
 * type (so you have to use pad bytes); <tt>short</tt> is 2 bytes; <tt>int</tt>
 * and <tt>long</tt> are 4 bytes. <tt>float</tt> and <tt>double</tt> are 32-bit
 * and 64-bit IEEE floating point numbers, respectively.
 * 
 * <P>
 * Note the difference between "<tt>@</tt>" and "<tt>=</tt>": both use native
 * byte order, but the size and alignment of the latter is standardized.
 * 
 * <P>
 * The form "<tt>!</tt>" is available for those poor souls who claim they can't
 * remember whether network byte order is big-endian or little-endian.
 * 
 * <P>
 * There is no way to indicate non-native byte order (i.e. force byte-swapping);
 * use the appropriate choice of "<tt>&lt;</tt>" or "<tt>&gt;</tt>".
 * 
 * <P>
 ...
 * count of zero, e.g. the format <tt>'llh0l'</tt> specifies two pad bytes at
 ...
 * <p>
 * 
 * The module is based on the original structmodule.c except that all mistakes
 * and errors are my own. Original author unknown.
 * <p>
 * 
...
public class struct {
     ...
     * what is wrong.
     */
    public static final PyObject error = Py.makeClass("error", Py.Exception,
            exceptionNamespace());
    ...
    static class ByteStream {
        ...
        ByteStream(String s, int offset) {
            ...
            // System.out.println("s.length()=" + s.length() + ",offset=" +
            // offset + ",size=" + size + ",data=" + Arrays.toString(data));
        }
    ...
    static class PascalStringFormatDef extends StringFormatDef {
        int doPack(ByteStream buf, int count, int pos, PyObject[] args) {
            ...
            buf.writeByte(Math.min(0xFF,
                    Math.min(value.toString().length(), count - 1)));
    ...
    static class LEUnsignedLongFormatDef extends FormatDef {
        void pack(ByteStream buf, PyObject value) {
            ...
            long lvalue = bi.longValue(); // underflow is OK -- the bits are
                                          // correct
        ...
        Object unpack(ByteStream buf) {
            ...
            result = result
                    .multiply(java.math.BigInteger.valueOf(0x100000000L));
    ...
    static void pack_into(String format, FormatDef[] f, int size, int argstart,
            PyObject[] args) {
        if (args.length - argstart < 2)
            Py.TypeError("illegal argument type for built-in operation");
        if (!(args[argstart] instanceof PyArray)) {
            throw Py.TypeError("pack_into takes an array arg"); // as well as a
                                                                // buffer, what
                                                                // else?
        }
        ...
        if (res.pos > buffer.__len__()) {
            throw StructError("pack_into requires a buffer of at least "
                    + res.pos + " bytes, got " + buffer.__len__());
        }
    ...
    static ByteStream pack(String format, FormatDef[] f, int size, int start,
            PyObject[] args) {
     ...
     * (i.e. len(string) must equal calcsize(fmt)).
     */
