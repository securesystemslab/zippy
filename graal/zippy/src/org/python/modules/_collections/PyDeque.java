 ...
 * 
 * collections.deque([iterable]) - returns a new deque object initialized
 * left-to-right (using append()) with data from iterable. If iterable is not
    ...
    private Node getNode(PyObject index) {
        ...
        if (!index.isIndex()) {
            throw Py.TypeError(String.format(
                    "sequence index must be integer, not '%.200s'", index
                            .getType().fastGetName()));
        }
