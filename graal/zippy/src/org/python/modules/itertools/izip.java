...
public class izip extends PyObject {
    ...
    /**
     * Create an iterator whose <code>next()</code> method returns a tuple where
     * the i-th element comes from the i-th iterable argument. Continues until
     * the shortest iterable is exhausted. (Code in this method is based on
     * __builtin__.zip()).
     * 
     */
    @ExposedNew
    ...
    final void izip___init__(PyObject[] args, String[] kwds) {
        if (kwds.length > 0) {
            throw Py.TypeError(String
                    .format("izip does not take keyword arguments"));
        }
        izip___init__(args);
    }
