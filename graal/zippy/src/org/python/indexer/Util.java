...
public class Util {
    ...
    /**
     * Returns the parent qname of {@code qname} -- everything up to the last
     * dot (exclusive), or if there are no dots, the empty string.
     */
    ...
    /**
     * Given an absolute {@code path} to a file (not a directory), returns the
     * module name for the file. If the file is an __init__.py, returns the last
     * component of the file's parent directory, else returns the filename
    ...
    /**
     * Return absolute path for {@code path}. Make sure path ends with "/" if
     * it's a directory. Does _not_ resolve symlinks, since the caller may need
     * to play symlink tricks to produce the desired paths for loaded modules.
     */
