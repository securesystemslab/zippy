...
public class AstConverter extends Visitor {
    ...
    private List<NExceptHandler> convertListExceptHandler(List<excepthandler> in)
            throws Exception {
        List<NExceptHandler> out = new ArrayList<NExceptHandler>(in == null ? 0
                : in.size());
        if (in != null) {
    ...
    private List<NKeyword> convertListKeyword(List<keyword> in)
            throws Exception {
        List<NKeyword> out = new ArrayList<NKeyword>(in == null ? 0 : in.size());
        if (in != null) {
            for (keyword e : in) {
                NKeyword nkw = new NKeyword(e.getInternalArg(),
                        convExpr(e.getInternalValue()));
                if (nkw != null) {
                    out.add(nkw);
                }
            }
    ...
    public Object visitBinOp(BinOp n) throws Exception {
        return new NBinOp(convExpr(n.getInternalLeft()),
                convExpr(n.getInternalRight()), convOp(n.getInternalOp()),
                start(n), stop(n));
    }
    ...
    public Object visitBoolOp(BoolOp n) throws Exception {
        ...
        return new NBoolOp(op, convertListExpr(n.getInternalValues()),
                start(n), stop(n));
    }
    ...
    public Object visitCall(Call n) throws Exception {
        return new NCall(convExpr(n.getInternalFunc()),
                ...
                convExpr(n.getInternalStarargs()), start(n), stop(n));
    }
    ...
    public Object visitClassDef(ClassDef n) throws Exception {
        return new NClassDef((NName) convExpr(n.getInternalNameNode()),
                ...
                convertListStmt(n.getInternalBody()), start(n), stop(n));
    }

    @Override
    public Object visitCompare(Compare n) throws Exception {
        return new NCompare(convExpr(n.getInternalLeft()), null, // XXX: why
                                                                 // null?
                convertListExpr(n.getInternalComparators()), start(n), stop(n));
    }
    ...
    public Object visitDelete(Delete n) throws Exception {
        return new NDelete(convertListExpr(n.getInternalTargets()), start(n),
                stop(n));
    }
    ...
    public Object visitDict(Dict n) throws Exception {
        return new NDict(convertListExpr(n.getInternalKeys()),
                convertListExpr(n.getInternalValues()), start(n), stop(n));
    }
    ...
    public Object visitExec(Exec n) throws Exception {
        return new NExec(convExpr(n.getInternalBody()),
                ...
                convExpr(n.getInternalLocals()), start(n), stop(n));
    }
    ...
    public Object visitGeneratorExp(GeneratorExp n) throws Exception {
        List<NComprehension> generators = new ArrayList<NComprehension>(n
                .getInternalGenerators().size());
        for (comprehension c : n.getInternalGenerators()) {
            generators.add(new NComprehension(convExpr(c.getInternalTarget()),
                    convExpr(c.getInternalIter()), convertListExpr(c
                            .getInternalIfs()), start(c), stop(c)));
        }
        return new NGeneratorExp(convExpr(n.getInternalElt()), generators,
                start(n), stop(n));
    }

    @Override
    public Object visitGlobal(Global n) throws Exception {
        return new NGlobal(convertListName(n.getInternalNameNodes()), start(n),
                stop(n));
    }
    ...
    public Object visitIfExp(IfExp n) throws Exception {
        return new NIfExp(convExpr(n.getInternalTest()),
                convExpr(n.getInternalBody()), convExpr(n.getInternalOrelse()),
                start(n), stop(n));
    }

    @Override
    public Object visitImport(Import n) throws Exception {
        List<NAlias> aliases = new ArrayList<NAlias>(n.getInternalNames()
                .size());
        for (alias e : n.getInternalNames()) {
            aliases.add(new NAlias(e.getInternalName(), convertQname(e
                    .getInternalNameNodes()), (NName) convExpr(e
                    .getInternalAsnameNode()), start(e), stop(e)));
        }
    ...
    public Object visitImportFrom(ImportFrom n) throws Exception {
        ...
        return new NImportFrom(n.getInternalModule(),
                convertQname(n.getInternalModuleNames()), aliases, start(n),
                stop(n));
    }
    ...
    public Object visitList(org.python.antlr.ast.List n) throws Exception {
        return new NList(convertListExpr(n.getInternalElts()), start(n),
                stop(n));
    }
    ...
    public Object visitListComp(ListComp n) throws Exception {
        ...
        return new NListComp(convExpr(n.getInternalElt()), generators,
                start(n), stop(n));
    }

    @Override
    public Object visitModule(Module n) throws Exception {
        return new NModule(convertListStmt(n.getInternalBody()), start(n),
                stop(n));
    }
    ...
    public Object visitRaise(Raise n) throws Exception {
        return new NRaise(convExpr(n.getInternalType()),
                convExpr(n.getInternalInst()), convExpr(n.getInternalTback()),
                start(n), stop(n));
    }
    ...
    public Object visitSlice(Slice n) throws Exception {
        return new NSlice(convExpr(n.getInternalLower()),
                convExpr(n.getInternalStep()), convExpr(n.getInternalUpper()),
                start(n), stop(n));
    }
    ...
    public Object visitSubscript(Subscript n) throws Exception {
        return new NSubscript(convExpr(n.getInternalValue()),
                convExpr(n.getInternalSlice()), start(n), stop(n));
    }

    @Override
    public Object visitTryExcept(TryExcept n) throws Exception {
        return new NTryExcept(
                convertListExceptHandler(n.getInternalHandlers()),
                convertListStmt(n.getInternalBody()),
                convertListStmt(n.getInternalOrelse()), start(n), stop(n));
    }
    ...
    public Object visitTryFinally(TryFinally n) throws Exception {
        return new NTryFinally(convertListStmt(n.getInternalBody()),
                convertListStmt(n.getInternalFinalbody()), start(n), stop(n));
    }

    @Override
    public Object visitTuple(Tuple n) throws Exception {
        return new NTuple(convertListExpr(n.getInternalElts()), start(n),
                stop(n));
    }
    ...
    public Object visitUnaryOp(UnaryOp n) throws Exception {
        return new NUnaryOp(null, // XXX: why null for operator?
                convExpr(n.getInternalOperand()), start(n), stop(n));
    }
