...
public class NLambda extends NFunctionDef {
    ...
    protected void bindFunctionName(Scope owner) throws Exception {
        NameBinder.make(NBinding.Kind.FUNCTION).bindName(owner, fname,
                getType());
    }
    ...
    public NType resolve(Scope s) throws Exception {
        if (!getType().isFuncType()) {
            org.python.indexer.Indexer.idx.reportFailedAssertion("Bad type on "
                    + this + ": type=" + getType() + " in file " + getFile()
                    + " at " + start());
        }
        ...
        if (getType().isFuncType()) { // else warning logged at method entry
                                      // above
