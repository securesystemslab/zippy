...
public abstract class NNode implements java.io.Serializable {
    ...
    /**
     * Returns the type for this node. It is never {@code null}. If the node has
     * not been resolved, the type will default to
    ...
    /**
     * Adds a new type for the node, creating a union of the previous type and
     * the new type.
     * 
     * @param newType
     *            the new type
    ...
    /**
     * Returns {@code true} if this is a name-binding node. Includes
     * functions/lambdas, function/lambda params, classes, assignments, imports,
     * and implicit assignment via for statements and except clauses.
     * 
    ...
    private static NType handleExceptionInResolve(NNode n, Throwable t) {
        Indexer.idx.handleException(
                "Unable to resolve: " + n + " in " + n.getFile(), t);
    ...
    public static NType resolveExpr(NNode n, Scope s) {
        ...
        try {
            ...
            String msg = "Unable to resolve: " + n + " in " + n.getFile()
                    + " (stack overflow)";
    ...
    /**
     * Node should set the resolved type in its {@link #type} field and also
     * return it.
     */
     ...
     * 
     * @param visitor
     *            the object to call with this node. If the visitor returns
     *            {@code true}, the node also passes its children to the
     *            visitor.
     */
     ...
     * lookups. If the current node defines a scope, it returns the parent scope
     * for name lookups.
     * 
    ...
    /**
     * Resolves each element of a node list in the passed scope. Node list may
     ...
     * @return the deepest AST node whose start is greater than or equal to the
     *         offset, and whose end is less than or equal to the offset.
     *         Returns {@code null} if no node overlaps {@code sourceOffset}.
     */
    public NNode getDeepestNodeAtOffset(int sourceOffset) {
        NNode ast = getAstRoot();
        DeepestOverlappingNodeFinder finder = new DeepestOverlappingNodeFinder(
                sourceOffset);
        try {
            ast.visit(finder);
