...
public class NFunctionDef extends NNode {
    ...
    public NFunctionDef(NName name, List<NNode> args, NBlock body,
            List<NNode> defaults, NName varargs, NName kwargs) {
        this(name, args, body, defaults, kwargs, varargs, 0, 1);
    }

    public NFunctionDef(NName name, List<NNode> args, NBlock body,
            List<NNode> defaults, NName varargs, NName kwargs, int start,
            int end) {
        super(start, end);
        this.name = name;
        this.args = args;
    ...
    /**
     * Returns the name of the function for indexing/qname purposes. Lambdas
     * will return a generated name.
     */
    ...
    protected void bindNames(Scope s) throws Exception {
        ...
        Scope funcTable = new Scope(s.getEnclosingLexicalScope(),
                Scope.Type.FUNCTION);
        getType().setTable(funcTable);
        funcTable.setPath(owner.extendPath(getBindingName(owner)));

        // If we already defined this function in this scope, don't try it
        // again.
        NType existing = owner.lookupType(getBindingName(owner), true /*
                                                                       * local
                                                                       * scope
                                                                       */);
     ...
     * For instance, in
     * {@code def foo(converter=lambda name: name.upper()): ...} the lambda is a
    ...
    protected NBinding addSpecialAttr(String name, NType atype,
            NBinding.Kind kind) {
        NBinding b = getTable().update(name,
                Builtins.newDataModelUrl("the-standard-type-hierarchy"), atype,
                kind);
        b.markSynthetic();
        b.markStatic();
        return b;
    ...
    protected NBinding addReadOnlyAttr(String name, NType type,
            NBinding.Kind kind) {
    ...
    public NType resolve(Scope outer) throws Exception {
        ...
        if (selfBinding != null) {
            if (args.size() < 1) {
                addWarning(name,
                        "method should have at least one argument (self)");
    ...
    private void bindParamsToDefaults(NBinding selfBinding, NTupleType fromType)
            throws Exception {
        NameBinder param = NameBinder.make(PARAMETER);
        Scope funcTable = getTable();
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            NType argtype = ((i == 0 && selfBinding != null) ? selfBinding
                    .getType() : getArgType(args, defaults, i));
