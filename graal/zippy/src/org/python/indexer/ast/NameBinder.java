...
public class NameBinder {
    ...
    static private final NameBinder CONSTRUCTOR_BINDER = new NameBinder(
            CONSTRUCTOR);
    ...
    public void bind(Scope s, List<NNode> xs, NType rvalue) throws Exception {
        ...
        if (!rvalue.isUnknownType()) {
            Indexer.idx.putProblem(xs.get(0).getFile(), xs.get(0).start(), xs
                    .get(xs.size() - 1).end(), "unpacking non-iterable: "
                    + rvalue);
        }
        for (int i = 0; i < xs.size(); i++) {
    ...
    public NBinding bindName(Scope s, NName name, NType rvalue)
            throws Exception {
        NBinding b;
    ...
    public void bindIter(Scope s, NNode target, NNode iter) throws Exception {
        ...
        if (iterType.isListType()) {
            ...
            bind(s, target, iterType.asTupleType().toListType()
                    .getElementType());
        } else {
