 ...
 * {@code from moduleA import a, b as c, d} and
 * {@code from foo.bar.moduleB import *}.
 * <p>
 * 
 * The indexer's implementation of import * uses different semantics from all
 * the other forms of import. It's basically a bug, although the jury is still
 * out as to which implementation is better.
 * <p>
 * 
 * For the others we define name bindings anywhere an actual name is introduced
 * into the scope containing the import statement, and references to the
 * imported module or name everywhere else. This mimics the behavior of Python
 * at runtime, but it may be confusing to anyone with only a casual
 * understanding of Python's data model, who might think it works more like
 * Java.
 * <p>
 * 
 * For import * we just enter the imported names into the symbol table, which
 * lets other code reference them, but the references "pass through"
 * automatically to the module from which the names were imported.
 * <p>
 * 
 ...
 * </pre>
 * 
 * To make import * work like the others, we need only create bindings for the
 * imported names. But where would the bindings be located? Assuming that we
 * were to co-locate them all at the "*" name node, clicking on a reference to
 * any of the names would jump to the "*". It's not clear that this is a better
 * user experience.
 * <p>
 * 
...
public class NImportFrom extends NNode {
    ...
    /**
     * Resolve "foo [as bar]" in "from x[.y] import foo [as bar]". There are
     * several possibilities for "foo": it could be a file in the directory
     * "x[.y]", or it could be a name exported by the module "x[.y]" (from its
     * __init__.py), or it could be a public name in the file "x/y.py".
     * 
     * @param scope
     *            the scope into which names should be imported
     * @param mt
     *            the non-{@code null} module "x[.y]". Could refer to either
     *            x/y.py or x/y/__init__.py.
     * @param a
     *            the "foo [as bar]" component of the import statement
     */
    private void resolveAlias(Scope scope, NModuleType mt, NAlias a)
            throws Exception {
