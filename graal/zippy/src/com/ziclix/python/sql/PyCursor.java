...
public class PyCursor extends PyObject implements ClassDictInit,
        WarningListener, ContextManager {
    ...
    private static final DataHandler DATAHANDLER = DataHandler
    ...
    /**
     * Create the cursor, optionally choosing the type of fetch (static or
     * dynamic). If dynamicFetch is true, then use a dynamic fetch.
     * 
     * @param connection
     * @param dynamicFetch
     */
    ...
    /**
     * Create the cursor, optionally choosing the type of fetch (static or
     * dynamic). If dynamicFetch is true, then use a dynamic fetch. rsType and
     * rsConcur are used to create the Statement if both are non-None
     * 
     * @param connection
     * @param dynamicFetch
     * @param rsType
     * @param rsConcur
     */
    PyCursor(PyConnection connection, boolean dynamicFetch, PyObject rsType,
            PyObject rsConcur) {
    ...
    static public void classDictInit(PyObject dict) {
        dict.__setitem__("fetchmany", new CursorFunc("fetchmany", 0, 0, 1,
                "fetch specified number of rows"));
        dict.__setitem__("close", new CursorFunc("close", 1, 0,
                "close the cursor"));
        dict.__setitem__("fetchall", new CursorFunc("fetchall", 2, 0,
                "fetch all results"));
        dict.__setitem__("fetchone", new CursorFunc("fetchone", 3, 0,
                "fetch the next result"));
        dict.__setitem__("nextset", new CursorFunc("nextset", 4, 0,
                "return next set or None"));
        dict.__setitem__("execute", new CursorFunc("execute", 5, 1, 4,
                "execute the sql expression"));
        dict.__setitem__("setinputsizes", new CursorFunc("setinputsizes", 6, 1,
                "not implemented"));
        dict.__setitem__("setoutputsize", new CursorFunc("setoutputsize", 7, 1,
                2, "not implemented"));
        dict.__setitem__("callproc", new CursorFunc("callproc", 8, 1, 4,
                "executes a stored procedure"));
        dict.__setitem__("executemany", new CursorFunc("executemany", 9, 1, 3,
                "execute sql with the parameter list"));
        dict.__setitem__(
                "scroll",
                new CursorFunc("scroll", 10, 1, 2,
                        "scroll the cursor in the result set to a new position according to mode"));
        dict.__setitem__("write", new CursorFunc("write", 11, 1,
                "execute the sql written to this file-like object"));
        dict.__setitem__("prepare", new CursorFunc("prepare", 12, 1,
                "prepare the sql statement for later execution"));
        dict.__setitem__("__enter__", new CursorFunc("__enter__", 13, 0, 0,
                "__enter__"));
        dict.__setitem__("__exit__", new CursorFunc("__exit__", 14, 3, 3,
                "__exit__"));
     ...
     * will be unusable from this point forward; an Error (or subclass)
    ...
    /**
     * Returns the next row from the currently executing SQL statement using the
     * same semantics as .fetchone(). A StopIteration exception is raised when
     * the result set is exhausted for Python versions 2.2 and later.
     * 
     * @return PyObject
     * 
     * @since Jython 2.2, DB API 2.0+
     ...
     * @param prepared
     *            if true, prepare the statement, otherwise create a normal
     *            statement
     * 
     * @return PyStatement
     */
    private PyStatement prepareStatement(PyObject sql, PyObject maxRows,
            boolean prepared) {
        ...
        try {
            if (sql instanceof PyStatement) {
                ...
                String sqlString = sql instanceof PyUnicode ? sql.toString()
                        : sql.__str__().toString();
                ...
                if (normal) {
                    if (prepared) {
                        sqlStatement = this.connection.connection
                    ...
                    if (prepared) {
                        sqlStatement = this.connection.connection
                                .prepareStatement(sqlString, t, c);
                    } else {
                        sqlStatement = this.connection.connection
                                .createStatement(t, c);
                    }
                ...
                int style = prepared ? PyStatement.STATEMENT_PREPARED
                        : PyStatement.STATEMENT_STATIC;
     ...
     * made available through the standard fetchXXX() methods.
     * 
     * @param name
     * @param params
     * @param bindings
     * @param maxRows
     */
    public void callproc(PyObject name, final PyObject params,
            PyObject bindings, PyObject maxRows) {
        this.clear();
        ...
        try {
            if (getMetaData().supportsStoredProcedures()) {
                ...
                final Procedure procedure = datahandler
                        .getProcedure(this, name);
                Statement stmt = procedure.prepareCall(this.rsType,
                        this.rsConcur);
     ...
     * seq_of_parameters. Modules are free to implement this method using
     * multiple calls to the execute() method or by using array operations to
     * have the database process the sequence as a whole in one call.
     * 
     ...
     * @param maxRows
     */
    public void executemany(PyObject sql, PyObject params, PyObject bindings,
            PyObject maxRows) {
     ...
     * time. It is legal for a parameter to not match the predefined
     ...
     */
    public void execute(final PyObject sql, PyObject params, PyObject bindings,
            PyObject maxRows) {
        int rowIndex = -1;
        this.clear();
        ...
        try {
            synchronized (this.statement) {
                if (hasParams) {
                    ...
                    // if we have a sequence of sequences, let's run through
                    // them and finish
                    if (isSeqSeq(params)) {
    ...
    /**
     * Execute the current sql statement. Some generic functionality such as
     * updating the lastrowid and updatecount occur as well.
     */
    ...
    public PyObject nextset() {
        ...
        // If the fetch is exhausted and multiple ResultSets are supported, try
        // addding a
        // next ResultSet. XXX: DynamicFetch currently isn't so tailored for
        // this
        if (!nextset.__nonzero__() && connection.supportsMultipleResultSets
                && !dynamicFetch) {
            ...
            try {
                ...
                if ((hasMoreResults = stmt.getMoreResults())
                    ...
                    updateAttributes(!hasMoreResults ? updateCount : stmt
                            .getUpdateCount());
    ...
    public void warning(WarningEvent event) {
        ...
        while (warning != null) {
            ...
            PyObject[] warn = Py.javas2pys(warning.getMessage(),
    ...
    /**
     * Resets the cursor state. This includes flushing the warnings and any
     * previous results.
     * 
     */
    ...
    public static boolean isSeq(PyObject object) {
        ...
        // originally checked for __getitem__ and __len__, but this is true for
        // PyString
