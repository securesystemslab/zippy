...
public class PyConnection extends PyObject implements ClassDictInit,
    ...
    public PyConnection(Connection connection) throws SQLException {
        ...
        this.supportsTransactions = this.connection.getMetaData()
                .supportsTransactions();
        this.supportsMultipleResultSets = this.connection.getMetaData()
    ...
    public String toString() {
        try {
            return String.format(
                    "<PyConnection object at %s user='%s', url='%s'>",
                    Py.idstr(this), connection.getMetaData().getUserName(),
    ...
    static public void classDictInit(PyObject dict) {
        ...
        dict.__setitem__("close",
                new ConnectionFunc("close", 0, 0, 0, zxJDBC.getString("close")));
        dict.__setitem__(
                "commit",
                new ConnectionFunc("commit", 1, 0, 0, zxJDBC
                        .getString("commit")));
        dict.__setitem__(
                "cursor",
                new ConnectionFunc("cursor", 2, 0, 4, zxJDBC
                        .getString("cursor")));
        ...
        dict.__setitem__("__enter__", new ConnectionFunc("__enter__", 5, 0, 0,
                "__enter__"));
        dict.__setitem__("__exit__", new ConnectionFunc("__exit__", 6, 3, 3,
                "__exit__"));
    ...
    public PyObject __findattr_ex__(String name) {
        if ("autocommit".equals(name)) {
            ...
            try {
                return Py.newString(this.connection.getMetaData()
     ...
     * connection will be unusable from this point forward; an Error (or
    ...
    public void close() {
        ...
        // mark ourselves closed now so that any callbacks we get from closing
        // down
    ...
    public PyObject nativesql(PyObject nativeSQL) {
        ...
        try {
            if (nativeSQL instanceof PyUnicode) {
                return Py.newUnicode(this.connection.nativeSQL(nativeSQL
                        .toString()));
            }
            return Py.newString(this.connection.nativeSQL(nativeSQL.__str__()
                    .toString()));
        } catch (SQLException e) {
            throw zxJDBC.makeException(e);
        }
     ...
     * @return a new cursor using this connection
     */
    public PyCursor cursor(boolean dynamicFetch, PyObject rsType,
            PyObject rsConcur) {
        if (closed) {
            throw zxJDBC.makeException(zxJDBC.ProgrammingError,
                    "connection is closed");
        ...
        PyCursor cursor = new PyExtendedCursor(this, dynamicFetch, rsType,
                rsConcur);
