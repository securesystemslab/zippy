...
public class PyExtendedCursor extends PyCursor {
     ...
     */
    PyExtendedCursor(PyConnection connection, boolean dynamicFetch,
            PyObject rsType, PyObject rsConcur) {
    ...
    static public void classDictInit(PyObject dict) {
        ...
        dict.__setitem__("tables", new ExtendedCursorFunc("tables", 100, 4, 4,
                "query for table information"));
        dict.__setitem__("columns", new ExtendedCursorFunc("columns", 101, 4,
                4, "query for column information"));
        dict.__setitem__("primarykeys", new ExtendedCursorFunc("primarykeys",
                102, 3, 3, "query for primary keys"));
        dict.__setitem__("foreignkeys", new ExtendedCursorFunc("foreignkeys",
                103, 6, 6, "query for foreign keys"));
        dict.__setitem__("procedures", new ExtendedCursorFunc("procedures",
                104, 3, 3, "query for procedures"));
        dict.__setitem__("procedurecolumns", new ExtendedCursorFunc(
                "procedurecolumns", 105, 4, 4, "query for procedures columns"));
        dict.__setitem__("statistics", new ExtendedCursorFunc("statistics",
                106, 5, 5, "description of a table's indices and statistics"));
        dict.__setitem__("gettypeinfo", new ExtendedCursorFunc("gettypeinfo",
                107, 0, 1, "query for sql type info"));
        dict.__setitem__("gettabletypeinfo", new ExtendedCursorFunc(
                "gettabletypeinfo", 108, 0, 1, "query for table types"));
        dict.__setitem__("bestrow", new ExtendedCursorFunc("bestrow", 109, 3,
                3, "optimal set of columns that uniquely identifies a row"));
        dict.__setitem__(
                "versioncolumns",
                new ExtendedCursorFunc("versioncolumns", 110, 3, 3,
     ...
     * @param type
     */
    protected void tables(PyObject qualifier, PyObject owner, PyObject table,
            PyObject type) {
        clear();
        ...
        if (type != Py.None) {
            ...
            if (isSeq(type)) {
                ...
                y = new String[] { (typeName == null) ? null : typeName
                        .toUpperCase() };
            }
        }
     ...
     * @param column
     */
    protected void columns(PyObject qualifier, PyObject owner, PyObject table,
            PyObject column) {
        clear();
    ...
    /**
     * Gets a description of the stored procedures available for the qualifier
     * and owner.
     * 
     * @param qualifier
     * @param owner
     * @param procedure
     */
    protected void procedures(PyObject qualifier, PyObject owner,
            PyObject procedure) {
        clear();
     ...
     * @param column
     */
    protected void procedurecolumns(PyObject qualifier, PyObject owner,
            PyObject procedure, PyObject column) {
        clear();
     ...
     *            a table name
     */
    protected void primarykeys(PyObject qualifier, PyObject owner,
            PyObject table) {
     ...
     * @param foreignTable
     */
    protected void foreignkeys(PyObject primaryQualifier,
            PyObject primaryOwner, PyObject primaryTable,
            PyObject foreignQualifier, PyObject foreignOwner,
        ...
        try {
            this.fetch.add(getMetaData().getCrossReference(pq, po, pt, fq, fo,
                    ft));
        } catch (SQLException e) {
            throw zxJDBC.makeException(e);
        }
     ...
     * @param accuracy
     */
    protected void statistics(PyObject qualifier, PyObject owner,
            PyObject table, PyObject unique, PyObject accuracy) {
        clear();
    ...
    protected void typeinfo(PyObject type) {
        ...
        // if the type is non-null, then trim the result list down to that type
        // only
        // if(type != null &&!Py.None.equals(type)) {
        // for(int i = 0; i < ((PyObject) this.results.get(0)).__len__(); i++) {
        // PyObject row = ((PyObject) this.results.get(0)).__getitem__(new
        // PyInteger(i));
     ...
     *            a table name
     */
    protected void versioncolumns(PyObject qualifier, PyObject owner,
            PyObject table) {
        clear();
...
class ExtendedCursorFunc extends PyBuiltinMethodSet {
    ...
    ExtendedCursorFunc(String name, int index, int minargs, int maxargs,
            String doc) {
    ...
    public PyObject fancyCall(PyObject[] args) {
        ...
        switch (index) {
        case 103:
            cursor.foreignkeys(args[0], args[1], args[2], args[3], args[4],
                    args[5]);
            return Py.None;
