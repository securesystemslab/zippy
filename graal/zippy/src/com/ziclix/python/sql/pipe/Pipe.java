...
/**
 * Manager for a Sink and Source. The Pipe creates a Queue through which the
 * Source can feed data to the Sink. Both Sink and Source run in their own
 * thread and can are completely independent of the other. When the Source
 * pushes None onto the Queue, the piping is stopped and the Sink finishes
 * processing all the remaining data. This class is especially useful for
 * loading/copying data from one database or table to another.
 * 
 * @author brian zimmer
 */
public class Pipe {
    ...
    public PyObject pipe(Source source, Sink sink) {
        ...
        /*
         * This is interesting territory. I originally tried to store the the
         * Throwable in the Thread instance and then re-throw it here, but
         * whenever I tried, I would get an NPE in the construction of the
         * PyTraceback required for the PyException. I tried calling
         * .fillInStackTrace() but that didn't work either. So I'm left with
         * getting the String representation and throwing that. At least it
         * gives the relevant error messages, but the stack is lost. This might
         * have something to do with a Java issue I don't completely understand,
         * such as what happens for an Exception whose Thread is no longer
         * running? Anyways, if anyone knows what to do I would love to hear
         * about it.
         */
        ...
        // Assert that both sides handled the same number of rows. I know doing
        // the check up front kinda defeats
        // the purpose of the assert, but there's no need to create the buffer
        // if I don't need it and I still
