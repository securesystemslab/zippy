...
public class zxJDBC extends PyObject implements ClassDictInit {
    ...
    static {
        try {
            resourceBundle = ResourceBundle
                    .getBundle("com.ziclix.python.sql.resource.zxJDBCMessages");
    ...
    public static void classDictInit(PyObject dict) {
        ...
        dict.__setitem__("Timestamp", new zxJDBCFunc("Timestamp", 3, 6, 6,
                "construct a Timestamp from year, month, day, hour, "
                        + "minute, second"));
        dict.__setitem__("DateFromTicks", new zxJDBCFunc("DateFromTicks", 4, 1,
                1, "construct a Date from seconds since the epoch"));
        dict.__setitem__("TimeFromTicks", new zxJDBCFunc("TimeFromTicks", 5, 1,
                1, "construct a Time from seconds since the epoch"));
        dict.__setitem__("TimestampFromTicks", new zxJDBCFunc(
                "TimestampFromTicks", 6, 1, 1,
                "construct a Timestamp from seconds since the epoch"));
        dict.__setitem__("Binary", new zxJDBCFunc("Binary", 7, 1, 1,
    ...
    protected static void _addSqlTypes(PyObject dict) throws PyException {
        ...
        dict.__setitem__("DATETIME",
                dict.__getitem__(Py.newString("TIMESTAMP")));
    }
    ...
    protected static void _addConnectors(PyObject dict) throws PyException {
        ...
        while (names.hasMoreElements()) {
            ...
            try {
                ...
                Py.writeComment("zxJDBC", "loaded connector [" + className
                        + "] as [" + name + "]");
            } catch (Throwable t) {
    ...
    /**
     * Create the exception classes and get their descriptions from the resource
     * bundle.
     * 
     * @param dict
     */
    ...
    /**
     * Return the string associated with the key for the default resource
     * bundle. It first checks for 'key.N' where N starts at 0 and increments by
     * one. If any indexed key is found, the results of all the indexed values
     * are concatenated with the line separator. If no indexed key is found, it
     * defaults to checking the bundle by the key value alone.
     * 
     * @param key
     * @return String
     */
    ...
    /**
     * Return a formatted string. The key is used to get the format and the
     * values are passed, along with the format, to a MessageFormat who formats
     * it appropriately.
     * 
     * @param key
     * @param values
    ...
    public static PyException makeException(PyObject type, String msg) {
        return Py.makeException(type,
                msg == null ? Py.EmptyString : Py.newString(msg));
    }
...
class zxJDBCFunc extends PyBuiltinFunctionSet {
    ...
    public PyObject fancyCall(PyObject[] args) {
        ...
        case 3:
            ...
            return zxJDBC.datefactory.Timestamp(year, month, day, hour, minute,
                    second);
        default:
