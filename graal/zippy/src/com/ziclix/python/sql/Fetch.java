 ...
 * <p/>
 * <p>
 * <b>Static</b> The static variety iterates the entire set immediately,
 ...
 * from a database resource perspective (it does clear the results list
 * however). This approach also allows for the correct rowcount to be determined
 * since the entire result set has been iterated.
 * </p>
 * <p/>
 * <p>
 * <b>Dynamic</b> The dynamic variety iterates the result set only as requested.
 * This holds a bit truer to the intent of the API as the fetch*() methods
 * actually fetch when instructed. This is especially useful for managing
 * exeedingly large results, but is unable to determine the rowcount without
 * having worked through the entire result set. The other disadvantage is the
 * ResultSet remains open throughout the entire iteration. So the tradeoff is in
 * open database resources versus JVM resources since the application can keep
 * constant space if it doesn't require the entire result set be presented as
 * one.
 * </p>
 * 
 * @author brian zimmer
...
abstract public class Fetch {
     ...
     * <p/>
     * Note: since JDBC provides no means to get this information without
     * iterating the entire result set, only those fetches which build the
     * result statically will have an accurate row count.
     */
    ...
    /**
     * Create the results after a successful execution and manages the result
     * set.
     * 
     * @param resultSet
     */
    ...
    /**
     * Create the results after a successful execution and manages the result
     * set. Optionally takes a set of JDBC-indexed columns to automatically set
     * to None primarily to support getTypeInfo() which sets a column type of a
     * number but doesn't use the value so a driver is free to put anything it
     * wants there.
     * 
     * @param resultSet
     * @param skipCols
     ...
     * @param params
     */
    abstract public void add(CallableStatement callableStatement,
            Procedure procedure, PyObject params);
     ...
     * 
     * @return a single sequence from the result set, or None when no more data
     *         is available
     */
     ...
     * executeXXX() did not produce any result set or no call was issued yet.
     * 
     * @return a sequence of sequences from the result set, or None when no more
     *         data is available
     */
     ...
     * result set. In this case, the cursor position is left undefined (ideal
     * would be to not move the cursor at all).
     * <p/>
     * Note: This method should use native scrollable cursors, if available, or
     * revert to an emulation for forward-only scrollable cursors. The method
     * may raise NotSupportedErrors to signal that a specific operation is not
    ...
    protected PyObject createDescription(Procedure procedure)
        ...
        for (int i = 0, len = procedure.columns.__len__(); i < len; i++) {
            ...
            case DatabaseMetaData.procedureColumnReturn:
                ...
                a[6] = (nullable == DatabaseMetaData.procedureNullable) ? Py.One
                        : Py.Zero;
                ((PyList) metadata).append(new PyTuple(a));
                break;
            }
     ...
     * @throws SQLException
     */
    protected PyObject createResults(CallableStatement callableStatement,
            Procedure procedure, PyObject params) throws SQLException {
        ...
        for (int i = 0, j = 0, len = procedure.columns.__len__(); i < len; i++) {
            ...
            case DatabaseMetaData.procedureColumnInOut:
                obj = datahandler.getPyObject(callableStatement, i + 1,
                        dataType);
    ...
    /**
     * Creates the results of a query. Iterates through the list and builds the
     * tuple.
     * 
     * @param set
     *            result set
     ...
     * @throws SQLException
     */
    protected PyList createResults(ResultSet set, Set<Integer> skipCols,
            PyObject metaData) throws SQLException {
     ...
     * @throws SQLException
     */
    protected PyTuple createResult(ResultSet set, Set<Integer> skipCols,
            PyObject metaData) throws SQLException {
        ...
        for (int i = 0; i < descriptionLength; i++) {
            if (skipCols != null && skipCols.contains(i + 1)) {
                ...
                int type = ((PyInteger) metaData.__getitem__(i).__getitem__(1))
                        .getValue();
...
/**
 * This version of fetch builds the results statically. This consumes more
 * resources but allows for efficient closing of database resources because the
 * contents of the result set are immediately consumed. It also allows for an
 * accurate rowcount attribute, whereas a dynamic query is unable to provide
 * this information until all the results have been consumed.
 */
class StaticFetch extends Fetch {
    ...
    /**
     * Construct a static fetch. The entire result set is iterated as it is
     * added and the result set is immediately closed.
     */
    ...
    public void add(ResultSet resultSet, Set<Integer> skipCols) {
        try {
            if (resultSet != null && resultSet.getMetaData() != null) {
                PyObject metadata = this.createDescription(resultSet
                        .getMetaData());
                PyObject result = this.createResults(resultSet, skipCols,
                        metadata);
    ...
    public void add(CallableStatement callableStatement, Procedure procedure,
            PyObject params) {
        try {
            PyObject result = this.createResults(callableStatement, procedure,
                    params);
     ...
     * executeXXX() did not produce any result set or no call was issued yet.
     * 
     * @return a sequence of sequences from the result set, or an empty sequence
    ...
    public void scroll(int value, String mode) {
        ...
        if (pos >= 0 && pos < this.rowcount) {
            ...
            throw zxJDBC.makeException(Py.IndexError, "cursor index [" + pos
                    + "] out of range");
        }
    }
...
/**
 * Dynamically construct the results from an execute*(). The static version
 * builds the entire result set immediately upon completion of the query,
 * however in some circumstances, this requires far too many resources to be
 * efficient. In this version of the fetch the resources remain constant. The
 * dis-advantage to this approach from an API perspective is its impossible to
 * generate an accurate rowcount since not all the rows have been consumed.
 */
class DynamicFetch extends Fetch {
    ...
    /**
     * Add the result set to the results. If more than one result set is
     * attempted to be added, an Error is raised since JDBC requires that only
     * one ResultSet be iterated for one Statement at any one time. Since this
     * is a dynamic iteration, it precludes the addition of more than one result
     * set.
     */
    @Override
    public void add(ResultSet resultSet) {
        ...
        try {
            if (resultSet != null && resultSet.getMetaData() != null) {
                if (this.description == Py.None) {
                    this.description = this.createDescription(resultSet
                            .getMetaData());
                }
    ...
    /**
     * Internal use only. If <i>all</i> is true, return everything that's left
     * in the result set, otherwise return up to size. Fewer than size may be
     * returned if fewer than size results are left in the set.
     */
        ...
        try {
            ...
            if (type != ResultSet.TYPE_FORWARD_ONLY || value > 0) {
                if ("relative".equals(mode)) {
                    ...
                    if (value < 0) {
                        throw zxJDBC.makeException(Py.IndexError,
                                "cursor index [" + value + "] out of range");
                    }
                } else {
                    throw zxJDBC.makeException(zxJDBC.ProgrammingError,
                            "invalid cursor scroll mode [" + mode + "]");
                ...
                // since .rownumber is the *next* row, then the JDBC value suits
                // us fine
                this.rownumber = this.resultSet.getRow();
            } else {
