...
public class ModjyTestEnviron extends ModjyTestBase {
    ...
    // This test doesn't really achieve much, because mockrunner doesn't
    // actually do any computation
    // on the incoming URI: you have to set the servletContextPath(SCRIPT_NAME)
    // and
    ...
    // which actually does computation on the incoming URI to work out the
    // pathInfo.
    public void doScriptNamePathInfoTest(String contextPath,
            String servletPath, String URI, String pathInfo,
        ...
        assertEquals("ScriptName '" + results[0] + "' != '"
                + expectedScriptName + "'", expectedScriptName, results[0]);
        String actualPathInfo = "";
        if (results.length > 1) {
            actualPathInfo = results[1];
        }
        assertEquals("PathInfo '" + actualPathInfo + "' != '"
                + expectedPathInfo + "'", actualPathInfo, expectedPathInfo);
    }

    public void testScriptNamePathInfo() throws Exception {
        // Check that the environment contains correct values for SCRIPT_NAME
        // and PATH_INFO
        String[][] testData = { { "", "/", "" }, { "", "/", "info" },
                { "/ctx", "/srv", "" }, { "/ctx", "/srv", "/info" },
                { "/ctx", "/srv/sub", "" }, { "/ctx", "/srv/sub", "/info" }, };
        for (String[] element : testData) {
            ...
            doScriptNamePathInfoTest(contextPath, servletPath, uri, pathInfo,
                    contextPath + servletPath, expectedPathInfo, false);
            doScriptNamePathInfoTest(contextPath, servletPath, uri, pathInfo,
                    contextPath + servletPath, expectedPathInfo, true);
        }
    }

    // Again, this test doesn't achieve a lot under mockrunner, because you have
    // to
    ...
    public void testMandatoryWSGIVars_WSGIVersion() throws Exception {
        ...
        assertTrue("'wsgi.version' != PyTuple",
                result instanceof org.python.core.PyTuple);
        assertEquals("'wsgi.version[0]' != '1'",
                ((PyInteger) result.__getitem__(0)).getValue(), 1);
        assertEquals("'wsgi.version[1]' != '0'",
                ((PyInteger) result.__getitem__(1)).getValue(), 0);
    }
    ...
    public void testUserSpecifiedEnv() throws Exception {
        // Set a user specified environment in the servlet config, then test
        // that it is passed
        // through.
    ...
    public void testUserConfigVars() {
        // Set some environment vars in the servlet config, then test that they
        // are passed through.
    }
