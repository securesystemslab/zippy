...
public class PyFilterTest extends PyServletTest {
    ...
    public void testFilter() {
        getWebMockObjectFactory().getMockRequest().setServletPath(
                getTestPath("basic"));
        doGet();
        assertFalse(((HttpServletResponse) getFilteredResponse())
                .containsHeader("X-LookMaNoJava"));
        clearOutput();
        ...
        // Set that PyServlet initialization has run as mockrunner's context
        // doesn't indicate that
        // it happened.
        getWebMockObjectFactory().getMockServletContext().setAttribute(
                PyServlet.INIT_ATTR, "true");
