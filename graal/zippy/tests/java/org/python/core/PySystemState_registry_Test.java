...
public class PySystemState_registry_Test extends TestCase {
    ...
    /**
     * make sure a property from the user registry overrides the same property
    ...
    /**
     * make sure a pre property overrides any registry properties with the same
     * name
     * 
     * @throws Exception
     */
    ...
    /**
     * make sure a post property overrides any registry properties with the same
     * name, and the pre property as well
     * 
     * @throws Exception
     */
    ...
    private void findRoot() throws Exception {
        ...
        String classFileName = "/"
                .concat(thisClass.getName().replace('.', '/')).concat(".class");
        URL url = thisClass.getResource(classFileName);
        assertNotNull(url);
        String path = URLDecoder.decode(url.getPath(), "UTF-8");
        assertTrue(path.endsWith(classFileName));
        String classesDirName = path.substring(0,
                path.length() - classFileName.length());
     ...
     * @throws IOException
     */
    private void appendToInstalledRegistry(String contentToAppend)
            throws IOException {
        if (contentToAppend != null && contentToAppend.length() > 0) {
            String content = getOriginalRegistryContent().concat(
                    contentToAppend);
     ...
     * @return <code>false</code> if newResult or existingResult are false,
