...
public class PythonTreeTester {
    ...
    public PythonTree parse(String[] args) throws Exception {
        ...
        if (!isParseOnly()) {
             ...
             * nodes.setTokenStream(tokens); PythonWalker walker = new
             * PythonWalker(nodes); walker.setErrorHandler(eh); switch (_block)
             * { case MODULE : result = walker.module(); break; case INTERACTIVE
             * : result = walker.interactive(); break; case EXPRESSION : result
             * = walker.expression(); break; }
             * 
             * if (args.length > 1) { System.out.println(result.toStringTree());
             * }
             */
        }
        return result;
    ...
    /**
     * If set to <code>true</code>, only <code>PythonParser</code> is called.
     * 
     * @param parseOnly
     */
