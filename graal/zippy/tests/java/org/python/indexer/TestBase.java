...
public class TestBase extends TestCase {
    ...
    static {
        TEST_SOURCE_DIR = System.getProperties().getProperty(
                "python.test.source.dir")
                + "/org/python/indexer/";
        TEST_DATA_DIR = TEST_SOURCE_DIR + "data/";
        TEST_LIB_DIR = System.getProperties().getProperty("python.home")
                + "/Lib/";
    }
    ...
    /**
     * Return absolute path for {@code file}, a relative path under the data/
     * directory.
     */
    ...
    /**
     * Build an index out of the specified file and content lines, and return
     * the resulting module source.
     */
     ...
     *             if the {@code n}th occurrence does not exist
     */
    ...
    public void testDataFileFindable() throws Exception {
        assertTrue("Test file not found",
                new java.io.File(TEST_DATA_DIR).exists());
    }
    ...
    public void testStringModule() throws Exception {
        idx.loadString("test-string-module.py",
                makeModule("def foo():", "  pass"));
        idx.ready();
    ...
    public NBinding assertBinding(String qname, NBinding.Kind kind)
            throws Exception {
    ...
    public Def getDefinition(String qname, int offset, int length)
            throws Exception {
        NBinding b = getBinding(qname);
    ...
    public void assertDefinition(String qname, int offset, int length)
            throws Exception {
        Def def = getDefinition(qname, offset, length);
        if (def == null) {
            fail("No definition for " + qname + " at " + offset + " of len "
                    + length);
        }
    }

    public void assertNoDefinition(String msg, String qname, int pos, int len)
            throws Exception {
    ...
    public void assertDefinition(String qname, String name, int offset)
            throws Exception {
        assertDefinition(qname, offset, name.length());
    }

    public Ref getRefOrNull(String qname, int offset, int length)
            throws Exception {
        NBinding b = getBinding(qname);
    ...
    public Ref getRefOrFail(String qname, int offset, int length)
            throws Exception {
        Ref ref = getRefOrNull(qname, offset, length);
        assertNotNull("No reference to " + qname + " at offset " + offset
                + " of length " + length, ref);
        return ref;
    }

    public void assertReference(String qname, int offset, int length)
            throws Exception {
        assertTrue(getRefOrFail(qname, offset, length).isRef());
    }

    public void assertReference(String qname, int offset, String refname)
            throws Exception {
    ...
    public void assertNoReference(String msg, String qname, int pos, int len)
            throws Exception {
        assertNull(msg, getRefOrNull(qname, pos, len));
    }

    public void assertCall(String qname, int offset, int length)
            throws Exception {
        assertTrue(getRefOrFail(qname, offset, length).isCall());
    }

    public void assertCall(String qname, int offset, String refname)
            throws Exception {
    ...
    public void assertConstructed(String qname, int offset, int length)
            throws Exception {
        assertTrue(getRefOrFail(qname, offset, length).isNew());
    }

    public void assertConstructed(String qname, int offset, String refname)
            throws Exception {
    ...
    public NBinding assertBindingType(String bindingQname, String typeQname)
            throws Exception {
        NBinding b = getBinding(bindingQname);
        NType expected = getTypeBinding(typeQname);
        assertEquals("Wrong binding type", expected,
                NUnknownType.follow(b.getType()));
        return b;
    }

    public NBinding assertBindingType(String bindingQname, Class type)
            throws Exception {
        NBinding b = getBinding(bindingQname);
    ...
    /**
     * Asserts that the binding named by {@code bindingQname} exists and its
     * type is a List type. If {@code eltTypeQname} is non-{@code null}, asserts
     * that the List type's element type is an existing binding with
     * {@code eltTypeQname}.
     */
    public void assertListType(String bindingQname, String eltTypeQname)
            throws Exception {
        NBinding b = getBinding(bindingQname);
        ...
        if (eltTypeQname != null) {
            ...
            assertEquals(eltType,
                    NUnknownType.follow(btype.asListType().getElementType()));
        }
    }
    ...
    public void assertInstanceType(String bindingQname, String classQname)
            throws Exception {
        if (true) {
